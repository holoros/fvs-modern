!     Subroutines to calculate diameter inside and ouside bark diameter between 0 and 4.5 and also stump volume.
!     From Raile, G 1982. Estimating stump volume. Res. Pap. NC-224 St Paul, USDA FS North Central Forest Experimental Station
!     Created by Y. Wang 03/13/2012
  ...
  DATA (splist(I),I=1,23)/129,125,105,090,094,095,012,261,241,802, &
                          ...
                          950,740,970/
!     Jenkins species group 1,2,3 use 261
...
!     Jenkins species group 10 use 833
...
5 CONTINUE
  ...
  IF (idx .eq. 0) THEN
    ...
    IF(DONE .GT. 0)THEN
      ...
      GOTO 5
    ELSE
!         Not in group 1,2,3 and continue search group 4
      DONE = 0
      LAST = 34
      ...
      IF(DONE .GT. 0)THEN
        ...
        GOTO 5
      ELSE
!           Not find in group 4 and continue search group 5
        DONE = 0
        LAST = 9
        ...
        IF(DONE .GT. 0)THEN
          ...
          GOTO 5
        ELSE
!             Not find in group 5 and continue search group 6
          DONE = 0
          LAST = 27
          ...
          IF(DONE .GT. 0)THEN
            ...
            GOTO 5
          ELSE
!               Not find in group 6 and continue search group 7
            DONE = 0
            LAST = 18
            ...
            IF(DONE .GT. 0)THEN
              ...
              GOTO 5
            ELSE
!                 Not find in group 7 and continue search group 9
              DONE = 0
              LAST = 59
              ...
              IF(DONE .GT. 0)THEN
                ...
                GOTO 5
              ELSE
!                   Not find in group 9 and continue search group 10
                DONE = 0
                LAST = 38
...
!  Cubic foot volume of stump including wood and bark based on
!  integration formula provided by Raile
  ...
  RETURN
  END
!  Cubic foot volume of stump inside and outside bark. DIB/DOB pridicated by
!  Raile computed for half way up stump and then volume computed for cylinder
  ...
  END
!  Cubic foot volume of stump inside and outside bark. DIB/DOB pridicated by
!  Raile and sum of segments using Smailian formula
...
!  This rubroutine searches an array to find the maching element
