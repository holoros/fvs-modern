...
! Calculates costs & revenues associated with the FVS/ECON extension.
!   May throw runtime exceptions if array bounds exceeded for number of harvest costs or
!      revenues or maximum number of investment years
!   Routine is complicated because calculating SEV with appreciated costs and revenues requires
!      that values be kept over time because the aapreciation period can be variable.

! Called by GRADD during cycling, once for each FVS cycle.

! 09/08/2011 Refactored computation of bfTotal and ft3Total in calcEcon() to a single place.
! 10/12/2012 Removed saved variables & created new commons

! Variables from FVS
!  JSP    - species codes. Loaded in **BLOCK DATA**.
!  IACTK  - activityCode, code for an Event Monitor activity
!  ICYC   - cycle number of current cycle
!  IDT    - eventYear, year an Event Monitor activity is scheduled to take place
!  ISTAT  - year an Event Monitor activity was accomplished, if -1=event deleted, 0=yet to do
!  IY     - array storing beginning year of each cycle
!  KODE   - return code, where: 0 implies no errors, 1 implies the referenced activity could not be found.
!  MAXCY1 - maximum number of cycles plus 1, from PRGPRM.F77
!  MAXSP  - maximum number of species codes used by a specific variant, from PRGPRM.F77
!  MGMID  - management code
!  NPLT   - stand label
!  NTIMES - the number of times the activity exists within the the range of dates.
!  ITITLE - STDIDENT title

implicit none

include 'PRGPRM.f90'
include 'ECNCOM.f90'
include 'ECNCOMSAVES.f90'
