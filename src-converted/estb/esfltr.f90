SUBROUTINE ESFLTR
IMPLICIT NONE
!----------
! ESTB $Id$
!----------
!
!      CALLED AFTER CRATET TO COLLECT INVENTORY INFORMATION FOR
!      UNDERSTORY/OVERSTORY DENSITIES AND TO FLAG BEST TREES
!
!OMMONS
!
INCLUDE 'PRGPRM.f90'
!
!
INCLUDE 'ARRAYS.f90'
!
!
INCLUDE 'CONTRL.f90'
!
!
INCLUDE 'PLOT.f90'
!
!
INCLUDE 'ESPARM.f90'
!
!
INCLUDE 'ESTREE.f90'
!
!
INCLUDE 'ESHAP.f90'
!
!
INCLUDE 'PDEN.f90'
!
!
INCLUDE 'CALDEN.f90'
!
!
INCLUDE 'VARCOM.f90'
!
!
!OMMONS
!
INTEGER IDENWK(MAXTRE),NOTE(30),ICHOIS(30),NULL(MAXSP), &
             ITALL(MAXSP),IPLOT(MAXTRE)
REAL TALL(MAXSP)
INTEGER I,N,KODE,NPS,IDT,IFLAG,IBGIN,NNN,III,NUM,J,IDENT,IS,KT
INTEGER IHOLD
REAL PIX,D,ZPROB,CHEK,HEIGHT
EQUIVALENCE (WK5,IDENWK),(WK6,IPLOT)
DO 10 I=1,MAXPLT
BAAINV(I)=0.0
TPAAINV(I)=0.0
10 CONTINUE
TPACRE=0.0
IF(ITRN.LT.1) GO TO 201
!
!     PIX IS NUMBER OF STOCKABLE PLOTS
!
PIX=PI-FLOAT(NONSTK)
DO 30 I=1,ITRN
IPLOT(I)=ITRE(I)
D=DBH(I)
N=ITRE(I)
ZPROB=PROB(I)
IF(D.LT.REGNBK) GO TO 20
BAAINV(N)= BAAINV(N) + 0.005454154*D*D*ZPROB*PIX
TPAAINV(N)= TPAAINV(N) + ZPROB*PIX
GO TO 30
20 CONTINUE
TPACRE=TPACRE +ZPROB
30 CONTINUE
!
!     FLAG INCOMING BEST TREE RECORDS. IGNORE PROB. ASSUMPTIONS:
!     1. SPECIES CODES ARE 1 TO MAXSP.  2. HEIGHTS, DIAMETERS, ETC. ARE
!     SET.  3. BEST TREE RECORDS ARE WITHIN THE FIRST 30 RECORDS OF TREES
!     TALLIED ON A PLOT.  4. IESTAT IS ZEROED OUT (DONE IN INTREE).
!     5. PLOT NUMBERS ARE INTEGERS BETWEEN 1 AND 200.
!
CALL OPGET2 (428,IDT,IY(1)-20,IY(1)+20,1,1,NPS,WK4,KODE)
IF(KODE.NE.0) GO TO 201
IDSDAT=IFIX(WK4(1))
CALL IAPSRT(ITRN,IPLOT,IDENWK,.TRUE.)
IFLAG=0
IBGIN=ITRE(IDENWK(1))
N=0
NNN=0
DO 60 I=1,30
ICHOIS(I)=0
60 CONTINUE
DO 61 I=1,MAXSP
NULL(I)=0
ITALL(I)=0
TALL(I)=0.001
61 CONTINUE
!
!     BEGIN WORKING WITH THE TREE LIST. FLAG BEST TREES BY PLOT.
!
DO 200 III=1,ITRN
NUM=ITRE(IDENWK(III))
IF(NUM.EQ.IBGIN) GO TO 190
62 CONTINUE
IF(N.GT.0) GO TO 63
IBGIN=ITRE(IDENWK(III))
GO TO 190
63 CONTINUE
!
!     STEP 1:  PICK 2 TALLEST RECORDS REGARDLESS OF SPECIES.
!
DO 65 J=1,2
IF(NNN.GE.N) GO TO 180
CHEK=0.001
DO 64 I=1,N
IF(ICHOIS(I).EQ.1) GO TO 64
IDENT=NOTE(I)
IS=ISP(IDENT)
HEIGHT=HT(IDENT)
IF(HEIGHT.LT.CHEK) GO TO 64
CHEK=HEIGHT
KT=IS
IHOLD=I
64 CONTINUE
NULL(KT)=1
ICHOIS(IHOLD)=1
NNN=NNN+1
65 CONTINUE
!
!     STEP 2:  PICK TALLEST TREE OF EACH ADDITIONAL SPECIES.
!
DO 71 J=1,MAXSP
IF(NULL(J).EQ.1) GO TO 71
DO 70 I=1,N
IDENT=NOTE(I)
IS=ISP(IDENT)
IF(IS.NE.J) GO TO 70
IF(ICHOIS(I).EQ.1) GO TO 70
HEIGHT=HT(IDENT)
IF(HEIGHT.LT.TALL(J)) GO TO 70
TALL(J)=HEIGHT
ITALL(J)=I
70 CONTINUE
71 CONTINUE
DO 80 J=1,MAXSP
IF(NULL(J).EQ.1) GO TO 80
KT=ITALL(J)
IF(KT.EQ.0) GO TO 80
ICHOIS(KT)=1
NNN=NNN+1
80 CONTINUE
IF(NNN.GE.N) GO TO 180
IF(NNN.GE.4) GO TO 180
!
!     STEP 3:  PICK 4 TREES BY DESCENDING HEIGHTS, IF POSSIBLE.
!
90 CONTINUE
CHEK=0.001
DO 100 I=1,N
IF(ICHOIS(I).EQ.1) GO TO 100
IDENT=NOTE(I)
HEIGHT=HT(IDENT)
IF(HEIGHT.LT.CHEK) GO TO 100
CHEK=HEIGHT
IHOLD=I
100 CONTINUE
ICHOIS(IHOLD)=1
NNN=NNN+1
IF(NNN.GE.N) GO TO 180
IF(NNN.LT.4) GO TO 90
180 CONTINUE
DO 181 I=1,N
IDENT=NOTE(I)
IESTAT(IDENT)=ICHOIS(I)* (IDSDAT+20)
181 CONTINUE
!
!     RE-INITIALIZE
!
IF(IFLAG.EQ.1) GO TO 200
IBGIN=ITRE(IDENWK(III))
N=0
NNN=0
DO 185 I=1,30
ICHOIS(I)=0
185 CONTINUE
DO 186 I=1,MAXSP
NULL(I)=0
ITALL(I)=0
TALL(I)=0.001
186 CONTINUE
190 CONTINUE
IF(N.GT.29) GO TO 195
D=DBH(IDENWK(III))
IF(D.GE.REGNBK) GO TO 200
N=N+1
NOTE(N)=IDENWK(III)
195 CONTINUE
IF(III.EQ.ITRN) IFLAG=1
IF(IFLAG.EQ.1) GO TO 62
200 CONTINUE
201 CONTINUE
RETURN
END
