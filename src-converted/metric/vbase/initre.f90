...
134 FORMAT(T12,'STAND ATTRIBUTES ARE CALCULATED PER HA OF ', &
...
139     FORMAT (/'STDINFO    FOREST-LOCATION CODE=',I8, &
        '; HABITAT TYPE=',A10,' (CODE',I3,')', &
        '; AGE=',I5,'; ASPECT AZIMUTH IN DEGREES= ',F4.0, &
        ';',/T12,' SLOPE= ' ,F4.0,'%', &
        '; ELEVATION (M)=',F5.1,'; REFERENCE CODE= ',A4)
!
  CASE ('AK')
    WRITE (JOSTND,131) KODFOR,KODTYP,IAGE,ASPECT, &
       SLOPE,ELEV*10.*FTtoM,ADJUSTL(CPVREF)
131     FORMAT (/'STDINFO    FOREST-LOCATION CODE=',I8, &
        '; HABITAT TYPE=',I3, &
        '; AGE=',I5,'; ASPECT AZIMUTH IN DEGREES= ',F4.0, &
          '; SLOPE= ' ,F4.0,'%'/ &
       T12,'ELEVATION (M)=',F5.1,'; REFERENCE CODE= ',A4)
!
  CASE ('SN')
...
!
  CASE ('ON','BC')
  WRITE (JOSTND,136) KODFOR,KODTYP,IAGE,ASPECT, &
       SLOPE,ELEV*100.*FTtoM,ADJUSTL(CPVREF)
136   FORMAT (/'STDINFO    FOREST-LOCATION CODE=',I8, &
      '; HABITAT TYPE=',I3, &
      '; AGE=',I5,'; ASPECT AZIMUTH IN DEGREES= ',F4.0, &
      '; SLOPE= ' ,F4.0,'%'/ &
      T12,'ELEVATION (M)=',F5.1,'; REFERENCE CODE= ',A4)
  ...
  WRITE (JOSTND,138) KODFOR,KODTYP,IAGE,ASPECT, &
       SLOPE,ELEV*100.0*FTtoM,ADJUSTL(CPVREF)
...
IF((LNOTBK(3) .OR. LNOTBK(4)).AND.LKECHO) &
     &RITE(JOSTND,1405) SDLO*INtoCM,SDHI*INtoCM &
     &ORMAT(T12,'ONLY RECORDS WITH DIAMETERS GE',F6.1,' CM OR ', &
...
IF(LKECHO)WRITE(JOSTND,1925) TC*INtoCM,(WK6(I),I=1,8) &
     &ORMAT(T12,'TRANSITION SIZE = ',F10.2,' CM DBH.', &
     &   EQUATION FORM: C0 + C1*DBH +C2*DBH**C3'/ &
     &12,'COEFFICIENTS FOR TREES THAT ARE SMALLER THAN', &
...
!
  CASE ('BC','ON')
    IF(LKECHO)WRITE (JOSTND,2414) KEYWRD,KODFOR,KODTYP, &
         IAGE,ASPECT,SLOPE,ELEV*100.*FTtoM,ADJUSTL(CPVREF)
2414     FORMAT (/A8,'   FOREST-LOCATION CODE=',I8, &
       '; HABITAT TYPE=',I3, &
       '; AGE=',I5,'; ASPECT AZIMUTH IN DEGREES= ',F4.0, &
       '; SLOPE= ',F4.0,'%'/ &
       T12,'ELEVATION (M)=',F5.1,'; REFERENCE CODE= ',A4)
...
IF (IPRMPT.GT.0) THEN
   ...
   GOTO 10
ENDIF
IF (.NOT.LNOTBK(3)) ARRAY(3)=999.*CMtoIN
...
IF(((ARRAY(6).GT.0.0).OR.(ARRAY(7).GT.0.0)) &
     &AND.LKECHO)WRITE(JOSTND,3930) &
       ARRAY(6)/ACRtoHA, &
       ARRAY(7)*FT2pACRtoM2pHA &
     &ORMAT(T12,'RESIDUAL CLASS TPH =',F8.1, &
             ' RESIDUAL CLASS BA =',F8.1)
GOTO 10
!
!  ==========  OPTION NUMBER 30: SALVAGE  ===========================SALVAGE
!
     &ONTINUE
ICFLAG=229
GO TO 3805
...
ENDIF &
     &ORMAT (/A8,'   DATE/CYCLE=',I5,'; RESIDUAL=',F8.2, &
      ...
      F5.1,' M.') &
    FORMAT (/A8,'   DATE/CYCLE=',I5,'; RESIDUAL=',F8.2,
>        '; PROPORTION OF SELECTED TREES REMOVED=',F6.3/
>   T12,'DBH OF REMOVED TREES WILL RANGE FROM ',F5.1,' TO ',
>   F5.1,' CM, AND'/
>   T12,'HEIGHT OF REMOVED TREES WILL RANGE FROM ',F5.1,' TO ',
...
!
  CASE ('ON')
    IF(LKECHO)WRITE(JOSTND,4111)
4111     FORMAT(T12,'DEFECT CORRECTIONS WILL BE APPLIED TO ', &
       'GROSS TOTAL VOLUME.')
!
  CASE DEFAULT
...
4312     FORMAT(/A8,T12,'PULPWOOD VOLUME DEFECT', &
       ...
       /T12,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2, &
       '; 38.1 CM TREES=',F6.2,/T12,'50.8 CM TREES=',F6.2, &
       '; 63.5 CM AND LARGER TREES=',F6.2)
!
  CASE ('ON')
    IF(LKECHO)WRITE(JOSTND,4313) KEYWRD,(ARRAY(I),I=3,7)
4313     FORMAT(/A8,T12,'GROSS TOTAL VOLUME DEFECT', &
       ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:', &
       /T12,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2, &
       '; 38.1 CM TREES=',F6.2,/T12,'50.8 CM TREES=',F6.2, &
       '; 63.5 CM AND LARGER TREES=',F6.2)
...
4320     FORMAT(/A8,T12,'CUBIC METRE VOLUME DEFECT', &
       ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:', &
       /T12,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2, &
       '; 38.1 CM TREES=',F6.2,/T12,'50.8 CM TREES=',F6.2, &
       '; 63.5 CM AND LARGER TREES=',F6.2)
...
!
    CASE ('ON')
      ILEN=3
      IF(LKECHO)WRITE(JOSTND,4333) KEYWRD,KARD(2)(1:ILEN),IS, &
           (ARRAY(I),I=3,7)
4333       FORMAT(/A8,T12,'GROSS TOTAL VOLUME DEFECT PROPORTIONS ', &
...
!
    CASE DEFAULT
      ...
      IF(LKECHO)WRITE(JOSTND,4340) KEYWRD,KARD(2)(1:ILEN),IS, &
           (ARRAY(I),I=3,7)
4340       FORMAT(/A8,T12,'CUBIC METRE VOLUME DEFECT PROPORTIONS ', &
         'HAVE BEEN CHANGED FOR SPECIES:',A,' (CODE=',I3,')', &
         /T12,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2, &
         '; 38.1 CM TREES=',F6.2,/T12,'50.8 CM TREES=',F6.2, &
         '; 63.5 CM AND LARGER TREES=',F6.2)
...
!
!  SET ITOPD8=1 TO DISSALLOW MODIFICATION OF TOP MERCH. SPEC.
!  IN R8 VARIANTS
!
SELECT CASE (VARACD)
!
...
4534     FORMAT (/A8,'   MERCHANTABILITY STANDARDS FOR ', &
         ...
         'LOG LENGTH=',F6.2)
  ELSEIF(ITOPD8.GT.0)THEN
    IF(LKECHO)WRITE(JOSTND,4537) KEYWRD,KARD(2)(1:ILEN),IS, &
         DBHMIN(IS)*INtoCM, &
         STMP(IS)*FTtoCM, &
         FRMCLS(IS)
  ELSE
...
IF(IS .LT. 0) THEN
  ...
  IF (VARACD .EQ. 'AK') THEN
    IF(LKECHO)WRITE(JOSTND,4634) KEYWRD,KARD(2)(1:ILEN),IGSP, &
          ...
          FRMCLS(IGSP)
  ELSEIF(ITOPD8.GT.0)THEN &
     &RITE(JOSTND,4537) KEYWRD,KARD(2)(1:ILEN),IGSP, &
          BFMIND(IGSP)*INtoCM, &
          BFSTMP(IGSP)*FTtoCM, &
          FRMCLS(IGSP)
  ELSE
...
5010   FORMAT (/A8,'   DATE/CYCLE=',I5,'; CUTTING MUST EXCEED ', &
       'THE FOLLOWING:',/,29X,F8.1,' SAWLOG M3,',F8.1, &
       ' SAWLOG BDFT,',F6.1,' M2 BASAL AREA, AND', &
       F8.1,' TOTAL MERCH M3.')
!
  CASE ('ON')
  IF(LKECHO)WRITE(JOSTND,5012) KEYWRD,IDT, &
       ARRAY(2)*FT3pACRtoM3pHA, &
       ARRAY(4)*FT2pACRtoM2pHA, &
       ARRAY(5)*FT3pACRtoM3pHA
5012   FORMAT (/A8,'   DATE/CYCLE=',I5,'; CUTTING MUST EXCEED ', &
       'THE FOLLOWING:',/,29X,F8.1,' GROSS MERCHANTABLE M3,', &
       F8.1,' M2 BASAL AREA, AND', &
       F8.1,' TOTAL MERCH M3.')
!
  CASE DEFAULT
  IF(LKECHO)WRITE(JOSTND,5011) KEYWRD,IDT, &
       ARRAY(2)*FT3pACRtoM3pHA, &
       ARRAY(3), &
       ARRAY(4)*FT2pACRtoM2pHA, &
       ARRAY(5)*FT3pACRtoM3pHA
5011   FORMAT (/A8,'   DATE/CYCLE=',I5,'; CUTTING MUST EXCEED ', &
       'THE FOLLOWING:',/,29X,F8.1,' MERCH M3,',F8.1, &
       ' MERCH BDFT,',F6.1,' M2 BASAL AREA, AND', &
       F8.1,' TOTAL M3.')
END SELECT
GO TO 10
!
...
IF (IS.EQ.-999) GOTO 10
IF(.NOT.LNOTBK(3)) ARRAY(3)=1.0
IF(.NOT.LNOTBK(5)) ARRAY(5)=999.0*CMtoIN
ARRAY(6)=0.
...
IF(LKECHO)WRITE(JOSTND,8020) KEYWRD,IDT,(ARRAY(I),I=2,3) &
     ...
     &12,'TREE-CLASS BOUNDARIES WHERE THE LARGEST ', &
     &DIFFERENCES ARE BETWEEN TREES.  DEFINE THE REMAINING'/ &
     &12,'CLASSES BY SPLITTING THOSE WHICH HAVE THE LARGEST ', &
     &RANGES.')
GO TO 10
!
...
IF(LKECHO)WRITE(JOSTND,8720) KEYWRD,I &
     &ORMAT (/A8,'   DATA SET REFERENCE NUMBER = ',I5)
CLOSE (UNIT=I)
GOTO 10
!
...
9030    FORMAT (/A8,'   DATE/CYCLE=',I5,'; SPECIES=',A,' (CODE=', &
       ...
       F7.2,' CM DBH ARE AFFECTED.'/T12,'TYPE CODE = ', &
...
9616     CONTINUE
9617     CONTINUE
    IF(LKECHO)WRITE(JOSTND,9618) KEYWRD, &
         ARRAY(2)*FTtoM
...
9908   FORMAT(/A8,T12,'DEFAULT COEFFICIENTS FOR CUBIC METER VOLUME'/ &
...
9918   FORMAT(/A8,T12,'DEFAULT COEFFICIENTS FOR CUBIC METER VOLUME'/ &
       T12,'EQUATIONS REPLACED FOR SPECIES= ',A,' (CODE= ',I3,').' )
ENDIF
!
IF(LKECHO)WRITE(JOSTND,9925) ITC,TC,(WK6(I),I=1,14) &
     &ORMAT(T12,'TRANSITION SIZE CODE = ',I4,';   TRANSITION SIZE = ', &
     &10.2/T12,'COEFFICIENTS FOR TREE THAT IS SMALLER THAN', &
...
9958   FORMAT(/A8,T12,'DEFAULT COEFFICIENTS FOR BOARD VOLUME'/ &
       T12,'EQUATIONS REPLACED FOR ALL SPECIES.')
!----------
!     SPECIES CODE IS SPECIFIED (ISPC>0).
!----------
...
9965   CONTINUE
  ILEN=3
  IF(LKECHO)WRITE(JOSTND,9968) KEYWRD,KARD(1)(1:ILEN),ISPC
9968   FORMAT(/A8,T12,'DEFAULT COEFFICIENTS FOR BOARD VOLUME'/ &
       T12,'EQUATIONS REPLACED FOR SPECIES= ',A,' (CODE= ',I3,').' )
ENDIF
!
...
!----------
!  FOR CENTRAL ROCKIES VARIANT, ONLY DEAL WITH MODEL TYPE
...
GOTO 10
!
!  ==========  OPTION NUMBER 117: 2222 ============================DATABASE
!
11700 CONTINUE
...
11912       FORMAT(/A8,T12,'PULPWOOD VOLUME DEFECT', &
          ...
          /T12,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2, &
          '; 38.1 CM TREES=',F6.2,/T12,'50.8 CM TREES=',F6.2, &
          '; 63.5 CM TREES=',F6.2,'; 76.2 CM TREES=',F6.2,/T12, &
...
!
    CASE ('ON')
    IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.1)THEN
      IF(LKECHO)WRITE(JOSTND,11930) KEYWRD,(WK6(I),I=1,8)
11930       FORMAT(/A8,T12,'GROSS TOTAL VOLUME DEFECT', &
          ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:', &
          /T12,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2, &
          '; 38.1 CM TREES=',F6.2,/T12,'50.8 CM TREES=',F6.2, &
          '; 63.5 CM TREES=',F6.2,'; 76.2 CM TREES=',F6.2,/T12, &
    ...
    ENDIF
    IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.2)THEN
      IF(LKECHO)WRITE(JOSTND,11931) KEYWRD,(WK6(I),I=1,8)
11931       FORMAT(/A8,T12,'GROSS MERCHANTABLE VOLUME DEFECT', &
         ' PROPORTIONS HAVE BEEN CHANGED FOR ALL SPECIES:', &
          /T12,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2, &
          '; 38.1 CM TREES=',F6.2,/T12,'50.8 CM TREES=',F6.2, &
          '; 63.5 CM TREES=',F6.2,'; 76.2 CM TREES=',F6.2,/T12, &
...
11932       FORMAT(/A8,T12,'PULPWOOD VOLUME DEFECT PROPORTIONS ', &
    ...
    IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.2)THEN
      IF(LKECHO)WRITE(JOSTND,11933) KEYWRD,KARD(2)(1:ILEN), &
                                     IS,(WK6(I),I=1,8)
11933       FORMAT(/A8,T12,'SAWLOG VOLUME DEFECT ', &
...
!
    CASE ('ON')
    IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.1)THEN
      IF(LKECHO)WRITE(JOSTND,11936) KEYWRD,KARD(2)(1:ILEN), &
                                     IS,(WK6(I),I=1,8)
11936       FORMAT(/A8,T12,'GROSS TOTAL VOLUME DEFECT PROPORTIONS ', &
           'HAVE BEEN CHANGED FOR SPECIES:',A,' (CODE=',I3,')', &
           /T12,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2, &
           '; 38.1 CM TREES=',F6.2,/T12,'50.8 CM TREES=',F6.2, &
           '; 63.5 CM TREES=',F6.2,'; 76.2 CM TREES=',F6.2,/T12, &
    ...
    ENDIF
    IF(IDTYPE.EQ.0 .OR. IDTYPE.EQ.2)THEN
      IF(LKECHO)WRITE(JOSTND,11937) KEYWRD,KARD(2)(1:ILEN), &
                                     IS,(WK6(I),I=1,8)
11937       FORMAT(/A8,T12,'GROSS MERCHANTABLE VOLUME DEFECT ', &
           'PROPORTIONS HAVE BEEN CHANGED FOR SPECIES:',A, &
           ' (CODE=',I3,')', &
           /T12,' 12.7 CM TREES=',F6.2,'; 25.4 CM TREES=',F6.2, &
           '; 38.1 CM TREES=',F6.2,/T12,'50.8 CM TREES=',F6.2, &
...
GO TO 10
!
!  ==========  OPTION NUMBER 120: CRUZFILE ==========================CRUZFILE
!
12000 CONTINUE
IDT=1
IF (LNOTBK(1)) IDT=IFIX(ARRAY(1))
...
12301   CONTINUE
  ...
  IF(LKECHO)WRITE(JOSTND,12320) KEYWRD,KARD(1)(1:ILEN), &
       ISPC,CAPDBH*INtoCM,CAPPRP,IDFLG,CAPHT*FTtoM
!----------
!     IF (ISPC=0) ALL SPECIES WILL BE CHANGED IN FOLLOWING CODE:
!----------
ELSEIF(ISPC .EQ. 0)THEN
...
ELSE
  ...
  IF(LKECHO)WRITE(JOSTND,12922)PRMS(2),KARD(4)(1:ILEN), &
                        IS,PRMS(4)*INtoCM, PRMS(5)*INtoCM
12922   FORMAT(T12, &
      'PROPORTION OF SELECTED TREES REMOVED= ',F6.3,';',/, &
      T12,'SPECIES=',A,' (CODE=',I3,');', &
      ' DBH OF REMOVED TREES WILL RANGE FROM ',F5.1,' TO ', &
...
13000 CONTINUE
!
!  IF WE DONT HAVE A KODFOR VALUE YET SET REGION BASED ON VARIANT 'VAR'
!
IF(KODFOR.LE.0)THEN
  SELECT CASE(VARACD)
  CASE('EM','KT','NI','IE','BC')
...
IF(LKECHO)WRITE(JOSTND,13710) KEYWRD,QMDMSB*INtoCM,SLPMSB, &
     &FFMSB,DLOMSB*INtoCM,DHIMSB*INtoCM,MFLMSB
...
14000 CONTINUE
  ...
  ELSE
    ...
    CALCSDI="REINEKE"
  ENDIF
  IF(LKECHO)WRITE(JOSTND,14010) KEYWRD,CALCSDI, &
        DBHSTAGE*INtoCM, &
        DBHZEIDE*INtoCM
14010   FORMAT (/A8,'   THE ',A7,' METHOD WILL BE ', &
                 'USED TO CALCULATE SDI.'/ &
     &X,'   THE MINIMUM DIAMETER USED IN REINEKE SDI ', &
                 'CALCULATIONS = ',F6.2,' CM'/ &
     &X,'   THE MINIMUM DIAMETER USED IN ZEIDE SDI CALCULATIONS = ', &
     &6.2,' CM')
  GO TO 10
CALL ERRGRO (.TRUE.,16)
GO TO 10
...
!
IF (LNOTBK(2))PRMS(1)=ARRAY(2)*CMtoIN ! LOWER DBH
IF (LNOTBK(3))PRMS(2)=ARRAY(3)*CMtoIN ! UPPER DBH
IF (LNOTBK(5))PRMS(4)=ARRAY(5)        ! Q FACTOR
IF (LNOTBK(6))PRMS(5)=ARRAY(6)*CMtoIN ! DIA. CLASS WIDTH
!
...
IRECNT=IRECNT+1
!
!     USE 'X' to hold original value pre-metrification, to simplify printing later
!
IF(ISETQFA.LE.0)THEN                  ! BA TARGET
  IF (LNOTBK(7)) THEN
    X=ARRAY(7)
    PRMS(6)=ARRAY(7)*M2pHAtoFT2pACR
  ENDIF
  CTARGET='TARGET BA='
  PRMS(7)=0
...
IF (KODE.GT.0) GOTO 10
IF(LKECHO)WRITE(JOSTND,41110)KEYWRD,IDT, &
     &RMS(1)*INtoCM, &
     &RMS(2)*INtoCM
...
GOTO 10
!
!  ==========  OPTION NUMBER 49: SPLEAVE ============================SPLEAVE
!
14400 CONTINUE
IDT=1
IF(LNOTBK(1)) IDT=IFIX(ARRAY(1))
