...
! EM $Id$
!----------
!     PREDICT THE PROBABILITY OF SUBSEQUENT SPECIES.
!     PLANTING COEFFICIENTS NOT INCLUDED IN THIS SUBROUTINE.
!----------
!
!OMMONS
!
...
!----------
!
!     PROB OF SUBSEQUENT WHITEBARK PINE (USE NI WP)
!
...
PSUB(1)= (1.0/(1.0+EXP(-PN))) * OCURHT(IHAB,1)  *XESMLT(1) &
        *OCURNF(IFO,1)
!
!     PROB OF SUBSEQUENT WESTERN LARCH (USE NI WL)
!
...
PSUB(2)= (1.0/(1.0+EXP(-PN))) * OCURHT(IHAB,2)  *XESMLT(2) &
        *OCURNF(IFO,2)
!
!     PROB OF SUBSEQUENT DOUGLAS-FIR (USE NI DF)
!
...
IF(IFO.EQ.7.OR.IFO.EQ.9.OR.IFO.EQ.10.OR.IFO.EQ.11.OR.IFO .EQ. &
      12.OR.IFO.EQ.3) PN=PN+0.6195183
PSUB(3)= (1.0/(1.0+EXP(-PN))) * OCURHT(IHAB,3)  *XESMLT(3) &
        *OCURNF(IFO,3)
!
!     PROB OF SUBSEQUENT LIMBER PINE (USE IE LM)
!
PSUB(4)=0.
!
!     PROB OF SUBSEQUENT SUBALPINE LARCH (USE IE AF)
!
...
PSUB(5)= (1.0/(1.0+EXP(-PN))) * OCURHT(IHAB,5)  *XESMLT(5) &
        *OCURNF(IFO,5)
!
!     PROB OF SUBSEQUENT ROCKY MTN JUNIPER (USE IE JU)
!
PSUB(6)=0.
!
!     PROB OF SUBSEQUENT LODGEPOLE PINE (USE NI LP)
!
...
PSUB(7)= (1.0/(1.0+EXP(-PN))) * OCURHT(IHAB,7)  *XESMLT(7) &
        *OCURNF(IFO,7)
!
!     PROB OF SUBSEQUENT ENGELMANN SPRUCE (USE NI ES)
!
...
PSUB(8)= (1.0/(1.0+EXP(-PN))) * OCURHT(IHAB,8)  *XESMLT(8) &
        *OCURNF(IFO,8)
!
!     PROB OF SUBSEQUENT SUBALPINE FIR (USE NI AF)
!
...
PSUB(9)= (1.0/(1.0+EXP(-PN))) * OCURHT(IHAB,9)  *XESMLT(9) &
        *OCURNF(IFO,9)
!
!     PROB OF SUBSEQUENT PONDEROSA PINE (USE NI PP)
!
...
PSUB(10)= (1.0/(1.0+EXP(-PN))) * OCURHT(IHAB,10)*XESMLT(10) &
        *OCURNF(IFO,10)
!
!     PROB OF SUBSEQUENT ANY OTHER SPECIES
!
PSUB(11)=0.0
PSUB(12)=0.0
PSUB(13)=0.0
