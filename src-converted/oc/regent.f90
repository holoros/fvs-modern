...
! OC $Id$
!----------
!  THIS SUBROUTINE COMPUTES 5-YR HEIGHT AND DIAMETER INCREMENTS FOR
!  SMALL TREES.
!
!  FOR SPECIES/TREES USING FVS GROWTH EQUATIONS:
!    THE HEIGHT INCREMENT MODEL IS APPLIED TO TREES THAT ARE LESS THAN 4
!    INCHES DBH, AND THE DBH INCREMENT MODEL IS APPLIED TO TREES THAT ARE
!    LESS THAN 3 INCHES DBH.  FOR TREES THAT ARE GREATER THAN 2 INCHES
!    AND LESS THAN 4 INCHES DBH, HEIGHT INCREMENT PREDICTIONS
!    ARE AVERAGED WITH THE PREDICTIONS FROM THE LARGE TREE MODEL.
!    DIAMETER IS ASSIGNED FROM A HEIGHT-DIAMETER FUNCTION FROM HEIGHT AT
!    THE BEGINNING AND END OF THE PROJECTION CYCLE. DIAMETER INCREMENT IS
!    COMPUTED BY SUBTRACTION AND ADJUSTED FOR BARK RATIO. DIAMETER
!    INCREMENT PREDICTIONS ARE AVERAGED WITH THE PREDICTIONS FROM THE
!    LARGE TREE MODEL OVER THE DIAMETER RANGE 1.5-3.0 INCHES.
!
!  FOR SPECIES/TREES USING ORGANON GROWTH EQUATIONS:
!    USE THE LARGE TREE DIAMETER AND HEIGHT GROWTH ESTIMATES
!
...
!    HEIGHT INCREMENT IS DERIVED FROM A HEIGHT-AGE CURVE AND IS
!    BASED ON A 5-YEAR GROWTH PERIOD.
!    SCALE IS USED TO CONVERT THE 5-YR HTG ESTIMATE TO A FINT-YR BASIS.
!
!    DIAMETER INCREMENT IS PREDICTED FROM CHANGE IN HEIGHT. SO THE
!    DIAMETER INCREMENT IS INITIALLY ON A FINT-YR BASIS.
!    SCALE2 IS USED TO COVERT THE FINT-YR DG ESTIMATE TO A 5-YR BASIS.
!    DIAMETER INCREMENT IS CONVERTED BACK TO A FINT-YEAR BASIS IN
!    **UPDATE**.
!
!  FOR SPECIES/TREES USING ORGANON GROWTH ESTIMATES:
!    HEIGHT INCREMENT IS BASED ON A 5-YEAR GROWTH PERIOD AND HAS ALREADY
!    BEEN SCALED TO A FINT-YR BASIS IN **HTGF**.
!
!    DIAMETER INCREMENT IS BASED ON A 5-YEAR GROWTH PERIOD, SO IT DOES
!    NOT NEED TO BE SCALED. DIAMETER INCREMENT WILL BE CONVERTED TO A
!    FINT-YR BASIS IN **UPDATE**.
!----------
FNT=FINT
IF(LESTB) THEN
  IF(FINT.LE.5.0) THEN
...
!----------
!  SET ORGANON TREE FLAG TO NON-VALID TREE BECAUSE THIS IS A NEW SEEDLING
!----------
  IORG(I) = 0
!----------
