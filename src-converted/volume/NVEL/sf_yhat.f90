...
!                                        STEP 6 of the 2-pt application.

!            calculate the predicted  dib, given relative height
!            this is based on Eqns 1 thru 6 of the draft report.
!
!            This routine may also takes the optional step of
!            calculating 1st derivatives.
!  REVISED TO CORRECT NUMERICAL UNDERFLOW PROBLEM (30SEP99)
!
  REAL SF_YHAT
...
!                                                     middle segment
  if (rh.ge. rhi2) then
      ...
      if (x.gt.0.0) then
         IF(JSP.EQ.22) THEN
!----------
!  TEST FOR POTENTIAL NUMERICAL UNDERFLOW FROM EXPONENTAL TERM b.havis 9/99
!  if x^(b1+2) < E-20, set to zero
!----------
...
!
!----------
!  TEST FOR POTENTIAL NUMERICAL OVERFLOW FROM EXPONENTAL TERM b.havis 9/99
!  if x^(b1+1) < E-20, set to zero
!----------
           IF (B1*LOG10(X) .LE. -20.) THEN
                SUS3= 0.0
           ELSE
...
100 SF_YHAT=F*Y
  ...
  if (INEEDSL .eq. 1) then
!                   the derivatives were calculated based on y and x
!                   lets also calculate them based on dib and H.
