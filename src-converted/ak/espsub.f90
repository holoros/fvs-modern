...
! AK $Id$
!----------
!     PREDICTS THE PROBABILITY OF SUBSEQUENT SPECIES.
!----------
!
!OMMONS
!
...
!  PSUB     -- SPECIES PROBABILITY, PASSED TO ESCOM2.F77
!----------
...
!----------
!  PREDICT PROBABILITY OF SUBSEQUENT SPECIES
!
!  PN = B1 + B2 * XPRD + B3 * BAPER + B4 * ADJELV + B5 * ADJSLO +
!  B6 * TLAT
!
!  VARIABLES DEFINED IN SECTION ABOVE
!  PN IS CONVERTED TO PROBABILITY OF SUBSEQUENT SPECIES BY
!  EXP(PN)/(1 + EXP(PN))
!----------
...
!           MAKE SURE SPECIES PROBABILITIES ARE BETWEEN 0 AND 1.
      IF(PSUB(J) .LT. 0.0) PSUB(J) = 0.0
      IF(PSUB(J) .GT. 1.0) PSUB(J) = 1.0
    ENDIF
  ENDDO
ENDIF
