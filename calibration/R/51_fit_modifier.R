...
## 51_fit_modifier.R
##
## Phase 3 Stage B. Reads the residual RDS produced by
## 50_extract_base_residuals.R and fits modifier_common.stan to recover
## plantation, disturbance, and treatment modifier coefficients on the
## same scale as the base model's eta. The posterior-mean modifier
## coefficients are the drop-in correction for the projection engine:
##
##   eta_total_i = eta_base_i + delta_i(alpha_hat)
##
## Usage (DG ORGANON):
##   Rscript calibration/R/51_fit_modifier.R \
##       --residuals calibration/output/conus/dg/dg_organon_residuals.rds \
##       --stan_file calibration/stan/modifier_common.stan \
##       --lambda    10 \
##       --chains    2 \
##       --warmup    500 \
##       --sampling  500 \
##       --out       calibration/output/conus/dg/dg_organon_modifier
##
## --lambda chooses which precomputed decay envelope to use: 5, 10, or
## 20 years. Comparing runs at each lambda is how we pick the best
## time constant for each base model family.
## =============================================================================
...
opts <- OptionParser(option_list = list(
  ...
  make_option("--stan_file", type = "character",
              default = "calibration/stan/modifier_common.stan"),
...
cat("==============================================================\n")
cat("FVS-CONUS Phase 3 Stage B modifier fit\n")
...
cat(sprintf("Residual bundle: model=%s  family=%s  %s rows\n",
...
# Drop rows with non-finite residuals (numerical edge cases in eta_base)
before <- nrow(d)
d <- d[is.finite(residual)]
if (nrow(d) < before)
  cat(sprintf("Dropped %s rows with non-finite residuals\n",
