...
! Determines ECON and PRETEND status each cycle.
!  Pretend mode is "on" only if active in the first year of a cycle.

! Called after each call to EVMON in GRINC to obtain activities (econStart and pretend) triggered by events during a cycle

! 10/12/2012 Removed saved variables & created new commons

! Variables from FVS
!   beforeCuts - 0 = ECSTATUS being called before CUTS, 1 = ECSTATUS being called after CUTS
!   ICYC       - current cycle number.
!   NCYC       - number of cycles in simulation.
!   IDT        - eventYear, year an Event Monitor activity is scheduled to take place.
!   IY         - array of actual simulation years, 1=inventory year, 2=end 1st cycle, 3=end 2nd cycle, etc.
