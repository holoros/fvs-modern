...
REAL TOTATT
REAL X
!...  The following added 6/28/99 for the new stand density rating value
!        calculation below.  AJM.
REAL DNCF1
REAL DNCF2
...
!     First zero out the variable used in printing to the driving variables
!     output file:
...
!     Begin calculation by looping over sizeclass-level GRF factors.
...
!...     GRF from DM (near zero is DMR=6)
...
!
!...  GRF from fire (differs between beetle species)
!
   GRFDV = 1.0
...
!
   ...
   IF (GRFDV .LE. 0.0) GRFDV = .01
!*******************************************
!...troubleshoot new compiling 3/00 ajm
!
!        WRITE (81,110) ISTD, ICLS, RVDSC(ISTD,ICLS), GRFDV,
!     >   BAH(ISTD,ICLS), DVRV(ISTD,3), GRF(ISTD,ICLS)
!  110    FORMAT (I12, I4, 2F6.3, 2X, F7.3, 2X, F9.3, 2X, F10.3)
!**************************************************************
   GRF(ISTD,ICLS)= GRF(ISTD,ICLS) * GRFDV
...
!  GRF from defoliators (notice the first line so that the GRF
!                        will not be 0 in the first year)
!
...
!...  GRF from density effects

!   The following two lines commented out 6/25/99, AJM.
!   GRF due to stand density effects will be calculated at  the
!   stand level (not by size class as is done here).  See added lines below.
!
!        GRF(ISTD,ICLS)= GRF(ISTD,ICLS) * DNSTD(ISTD,ICLS)
!        DVRV(ISTD,9) = DVRV(ISTD,9) + DNSTD(ISTD,ICLS) * BAH(ISTD,ICLS)

!     GRF can be at most 0.01
   GRF(ISTD,ICLS)= MAX(1.0E-2, GRF(ISTD,ICLS))
! *********IF CHANGING this use of 0.01, change it also in BMDRGT and BMATCT.
...
ENDIF
! *************************************************************************
!...The following section of code calculates stand-level GRF due to
!   stand density effects, and then updates the total stand GRF accordingly
!...Modified 6/25/99; and again 7/9/99... and again 7/19/99, and again 7/20
...
!...the following logical flag added so that keyword RVDENSE still works
!   with this new stand-density-RV function.  GFRFSTD will use the newly-
!   calculated RVDNST if keyword RVDENSE is NOT used (if RVDENSE  is used
!   LCDENS is set to false in BMPPIN.) AJM 1/31/00.
...
ENDIF
! *************************************************************************
!     temporarily disable grfstd()   may 4, 93
!
!      GRFSTD(ISTD)= 1.0
