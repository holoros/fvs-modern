...
IMPLICIT NONE
!   OLDGRO COMPUTES VOLUMES OF OLD GROWTH TREES WITH
!   (D GT 35  OR H GT 190).   32-foot log rule.
!
!   FROM DON DEMARS --- PNW, JUNEAU AK
!----------
!OMMONS
!
!
...
!     FOR LIMD(1) ONLY (I.E., DIB TOP=6.0 INCHES).
!----------
2 CONTINUE
DO 50 N=1,15
...
CL=HH(1)-16.3*KBD-ST
!----------
!     WE NOW KNOW (OR AT LEAST WE THINK WE KNOW) DBH, HEIGHT, DIAMETER
!       OUTSIDE BARK AT 6-INCH TOP, MERCH. HEIGHT TO 6-INCH TOP,
!       DIAMETER OUTSIDE BARK AT 4-INCH TOP, MERCH. HEIGHT TO A 4-INCH
!       TOP, NUMBER OF WHOLE LOGS IN THE TREE TO A 6-INCH TOP, AND
!       THE CHUNK LENGTH.
!
...
!
!     INTEGRAL OF SPRUCE-HEMLOCK PROFILE EQUATION
!
FF=.4*R**2.5+(-.0052554*H +.000034947*H**2+.104477* &
   H/D)*(.4*R**2.5-.25*R**4.)+(7.76807/D**2-.0000094852*H**2 &
   -.011351*H/D)*(.4*R**2.5-(1./33.)*R**33)
VS(JJ)=FF*FV
!
78 CONTINUE
!----------
...
XINTT=0.0
!----------
!     NOW ADD SCRIBNER FORMULA AND INTERNATIONAL 1/4-INCH SCALE.  START
!     WITH DIB(3) AND GO TO DIB(NNN-1).  DIB(NNN-1) IS 6.0 INCH TOP.
!----------
SCFT=0.0
NM=NNN-1
DO 90 K=3,NM
!----------
! SCRIBNER 32 FOOT LOG RULE
!----------
FK = FLOAT(K) * 0.5
ICK = INT(FK)
...
GO TO 92
91 CONTINUE
IF(CL.GT.0.0.AND.CL.LT.4.075)XINT(K)=CL/4.075*(0.199*DIB(K)**2 &
   -0.642*DIB(K))
IF(CL.GE.4.075.AND.CL.LT.8.15)XINT(K)=CL/8.15*(0.398*DIB(K)**2 &
   -1.086*DIB(K)-.271)
IF(CL.GE.8.15.AND.CL.LT.12.225)XINT(K)=CL/12.225*(0.597*DIB(K)**2 &
   -1.330*DIB(K)-.715)
IF(CL.GE.12.225.AND.CL.LT.16.3)XINT(K)=CL/16.3*(.796*DIB(K)**2 &
   -1.375*DIB(K)-1.230)
92 CONTINUE
XINTT=XINTT+XINT(K)
90 CONTINUE
...
!----------
!     AK VARIANT:
!     SPRUCE-HEMLOCK PROFILE EQUATION.
!     CALLED FROM OLDGRO.  USED FOR DIAMETER CALCULATIONS.
!----------
REAL H,D,R,PROF1
!
PROF1=R**1.5 + (-.0052554*H +.000034947*H**2 + .104477 &
   *H/D) * (R**1.5-R**3) +(7.76807/D**2 -.0000094852*H**2 - &
   .011351*H/D)*(R**1.5-R**32)
