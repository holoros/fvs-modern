...
!
   IF(IWHO.EQ.1)THEN &
     &ALL OPDONE (ITODO,JYR-1)
   ELSEIF (IWHO.EQ.2)THEN &
     &ALL OPDONE (ITODO,JYR)
   ELSE &
     &ALL OPDONE (ITODO,JYR)
   ENDIF
ENDIF
...
ENDIF
!
!     MAKE SURE THE OUTPUT FILE IS OPENED
!
...
!
  SELECT CASE (VARACD)
!----------
!  EASTERN AND CANADIAN ONTARIO VARIANTS
!----------
  CASE ('CS','LS','NE','SN','ON')
...
!----------
!  WESTERN AND CANADIAN BRITISH COLUMBIA VARIANTS
!----------
  CASE DEFAULT
...
11     FORMAT(/'COMPLETE ',A4,' LIST -- STAND: ',A26,T58,'MGMTID: ', &
        ...
        'POINT TOT CU MCH CU         MC BF TRC',/, &
        '  NUMBER INDX CD NO CL CD NUM   PER HA   PER HA  ', &
        'DIAM  INCR    HT INCR CR   CW MS ', &
        '%-TILE    BA  M VOL  M VOL     N/A DF DF  HT',/, &
        '-------- ---- -- -- -- -- --- -------- -------- ----- ', &
        '----- ----- ---- -- ---- -- ------ ----- ------ ', &
        '------ ------- -- -- ---')
  END SELECT
