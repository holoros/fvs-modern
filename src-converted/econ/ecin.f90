SUBROUTINE ECIN(IRECNT, IREAD, JOSTND, NSP, ICYC, LKECHO, ISPGRP) &
   esses FVS/ECON keywords for each simulation. &
   tains entry ECKEY (activityId, keyword) returns an activity code given a keyword name &

   ed from initre.f whenever an ECON keyword is read. &

   ables from FVS &
   C   - current cycle number &
   AD  - logical unit number from which keyword records are read &
   CNT - counter for number of keyword records processed &
   GRP - array holding species that have been defined as a group, from CONTRL.F77 &
   TND - logical unit number for writing standard output file &
   CHO - echo keywords & parameter values to the main output file, true by default. Controled with echok and noecho keywords. &
   DE  - logical true if ECON scheduled via Event Monitor and false if options scheduled normally, set in call to addEvent &
   SP  - maximum number of species codes used by a specific variant, from PRGPRM.F77 &
       - array of species character codes used by a specific variant, includes both species and tree-value-class codes
...
LFLAG    = .FALSE. &
   ead keywords, first keyword expected to be ECON
readKeyWd: do                                                      !Exit on keyword=END or KEYRDR=EOF, errCode=2
   isEconToBe = .TRUE.
   CALL KEYRDR(IREAD, JOSTND, .FALSE., KEYWRD, isNotBlank, &
           REAL Fields, IRECNT, errCode, charFields, LFLAG, LKECHO)    !KEYRDR fills realField w/ 0.0 unless actual number
   if (errCode == 2) then
...
return &
   iven an activityId, return the keyword name
ENTRY ECKEY(activityId, keyword)                                   !activityId keyword index
   keyword = KEYWORD_TABLE(activityId)
return

contains &
   hecks that correct units-of-measure is found in the specified column
logical function isCorrectUnit(keyword, units)
   ...
   etermines if a supplemental record indicator is in the given field
logical function isSupplemental(field)
   ...
   eads appreciation/depreciation rates and durations from a supplemental record.
subroutine ratesAndDurations(supplemental, numRates, rateCnt, &
  ...
  Set rates and durations on input arrays
   ...
   oads keyword parameters submitted via PARMS on the Event Monitor
subroutine loadParms(activityCode, field)
   ...
   egisters a keyword on the Event Monitor
subroutine addEvent(activityCode, minDate, NPRMS, prmsIndex)
   ...
   rovides for repeatable error messages
subroutine errMsg(msg)
   ...
   ssigns keyword values to revenue variables
