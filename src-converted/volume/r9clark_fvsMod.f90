!  Region 9 Profile Model Volume Calculation
!  revised TDH 01/12/2010  Added code for calculating log volumes and
!                          added the definfe type CLKCOEF for passing
!                          coefficients
!  revised RNH 11/19/2009  Added numerical error trap in equation
!                          for stmDib, subroutine r9dib
!  revised TDH 05/12/2009  Added debug code to first three subroutines.
!  revised TDH 08/12/2010  Fixed Log/BdFt error - initialize NOLOGP,
!                          NOLOGS to zero.
!  revised MGV 12/10/2010  Fixed order of code in R9PREP so top DIB is
!                          read before assigning coefficients a17, b17
!  revised TDH 01/21/2011  Trying to fix bug that doesn't give volumes
!                          for trees with merch ht only. addded tcfvol
!                          code on line 194
!  revised TDH 02/04/2011  Fixed bug listed above by moving the merch
!                          initialization above the coeffs/ht logic
!                          in R9PREP.
!
!  revised TDH 03/03/2011  Add ABS(R) on line 904 since negative
!                          values for R would cause problems.

!  revised TDH 5/24/2011   Added comments and an ISNAN check to final
!                          stemHt in r9Ht
!  revised RNH 5/24/2011   Added subroutine IISNAN to replace ISNAN check
!                          which isn't available in Lahey Fortran
!
!  revised RNH 07/21/2011  reassigned sawHT to ht1prd
!
!  revised YW  11/15/2011  Added upsHt1 to r9clark, r9prep, and r9dia417.
!                          Also added logVol to r9bdft and cType to r9clark.
!
!          YW  12/08/2011  Added check number of logs not greater than 20.
!
!          YW  02/28/2012  Changed to calc HT for prod 1 when ht1prd = 0
!          YW  08/21/2012  Added error flag check and reset vol array.
! YW 01/18/2013 Added vol calculation for stump (VOL(14)) and tip VOL(15)
! YW 04/15/2015 Added to make prod 14 to be same as prod 01 for volume calculation.
! YW 10/06/2015 Added TLOGS,NOLOGP and NOLOGS to the R9CLARK subroutine
! YW 01/22/2016 Added extra NaN check in r9ht routine for stemHt.
! YW 03/02/2016 Removed the error check for small tree (<17.3) and calculation using small tree logic
! YW 04/13/2017 Moved the stump and tip volume calc to volinit subroutine.
! YW 12/20/2017 NCrookston modified R9HT on Sept 2017 to deal with degenerate
!               math when trying to exponentiate negative numbers with real-valued powers.
! YW 01/25/2018 Removed the broken top calculation, which cause problem for CP.
! YW 07/13/2018 Modified R9cor to remove the adjustment factor for yellow-poplar (621).
! YW 02/14/2019 Check height greater than merchL (it was checking minLen before) for volume calculation
! YW 09/24/2019 R08 prod 08 uses MTOPP (DIB), so changed to use COEFFS to get sawHt (same as PROD8 subroutine)
   021/06/21 Comment out the subroutine iisnan(x,res). It is not used and cause compile error on some system (Mac).
   023/06/12 Modified r9cuft to avoid the number is off the limit
!-------------------------------------------------------------------------
!  This subroutine is designed for use with the VOLLIB routines in
!  the National Volume Estimator Library.  It returns arrays filled
!  with different types of volumes (vol), log lengths (logLen),
!  and log volumes (logVol).  It uses a standard stump height (stump)
!  if none was passed in.  An error flag (errFlg) is also returned
!  indicating any specific error found.
!
!  Variable definitions are located at the end of this file.
...
do 130,i=1,15
   vol(i)=0.0
130 continue &
   /02/25 The following height check looks not right. I comment out
...
!     Region 9
 ...
 ENDIF
comment out
!  added on 04/15/2015 for prod 14 to be same as prod 01
...
!       check height greater than merchL (20190214)
...
NOLOGS = 0 &
   ******************************************************************
!-----Get height to sawtimber top
sawHt=0.0
!Added R08 prod 08 here (2019/09/23)
if(iProd.eq.1.OR.(VOLEQ(1:1).EQ.'8'.AND.iprod.EQ.8)) then
!       cType is checked to let Volume Tester program to calculate boardfoot
!       volume for FVS if only total height is provided. 11/15/2011 (yw)
   ...
   R8 OUTSIDE BARK VOLUME (EQ 8*1CLKO***)
    ...
    ELSE
      ...
      IF(VOLEQ(1:1).EQ.'8'.AND.iprod.EQ.8)THEN
        TEMPVOL = 0.0
!             Check if HTTOT is entered (2020/10/28)
...
I think it should use cfvol (5/21/2015)
cfvol = VOL(4) + VOL(7)
...
675    FORMAT (A, F7.2) &
   ave no idea why the following statement causes some volumes
not be caclulated but it does.  wtf?
...
!   calc Tip Vol(15) and stump vol(14)
...
!  the specified measurements are reasonable.  Get coefficients and
!  merchantability rules for the specified species and product.
...
!  DW 08/22 initializations of variables otherwise uninitialized prior to MRULES call
OPT = 0
EVOD = 0
MINLENT = 0
BTR = 0
...
!-----Check to determine if the heights and diameters are reasonable.
...
!        errFlg=5
!      elseif(ht2Prd.gt.35.0*sqrt(max(dbhOb,0.0))) then
!        errFlg=5
!      elseif(ht1Prd.gt.35.0*sqrt(max(dbhOb-4.0,0.0))) then
!        errFlg=5
...
!-----Get species group to assign coefficients if there are none
...
!       Get species group index to assign coefficients
  k=grpIdx
...
!-----Get top height and top DIB 10/2022 increase httot to 17.4 from 17.3 DW
short=.false.
...
!       if sawtimber topHt is provided, no recalc topHt. 11/15/2011 (yw)
  if(upsHt1.gt.0) then
...
R9, HT1PRD is not same as UPSHT1. So comment out the foling line
...
!-----Get coefficients for inside-bark calculations.  All are for
!     total height, except a17 and b17, which correspond to the top DIB.
COEFFS%R=0.0
COEFFS%C=0.0
COEFFS%E=0.0
COEFFS%P=0.0
...
!  at 17.3' (dib17), given  species (spp), dbh (dbhOb), and height
!  (topHt) to top diameter inside bark (topDib).  a17 and b17 are
!  inside-bark coefficients corresponding to the specified top diameter.
...
elseif(topHt.gt.17.3) then
  IF(volEq(1:1).EQ.'9')THEN
!       the regression for R9 is using dbhIb
    dib17=dbhIb*(a17+b17*(17.3/topHt)**2)
  ELSE
!       the regression for R8 is using dbhOb
...
!     Make sure DIB  at 17.3 is large enough for product top diameters
...
!       short tree using the topHt (17.4) YW(02/12/2016)
    totHt = topHt
  endif
elseif(topHt.gt.17.3) then
...
!--     For short measured height, use a fixed taper
...
!-----Calculate cubic volume between specified heights
...
! The calculation needs to be done seperately. Otherwise it may cause calculation error
! Replaced this piece code with the code below. (YW 2021/02/26)
!-----Get DIB at specified height
...
!Date 5/24/2011
!REV  TDH added comments and an ISNAN check to final stemHt
USE CLKCOEF_MOD
...
!-----Get height to specified DIB
!     Eqn 2: split since terms are undefined when evaluated together.
!c this section replaced by NCrookston, Sept 2017 to deal with degenerate
!  math when trying to exponentiate negative numbers with real-valued powers.
!  *** note that an original version included the use of testing the results
!  *** for NaN, but that idea isn't valid because it happens after the math
!  *** exception causeing an abnormal termination on some systems.
!      if(Is.eq.1)  then
...
!      endif
!c end of deleted code, start of replacement
...
!     The following line check if stemHt is a NaN
...
!  stump (stump) to the specified sawtimber height (sawHt), given
!  inside-bark dbh (dbhIb), inside-bark diameter at 17.3' (dib17) and
!  total height (totHt).  Intermediate calculations fill arrays for
!  log lengths (logLen), log end diameters (logDia), and height to the
!  log ends (bolHt).  Log specifications are maximum length (maxLen),
!  minimum length (minLen), trim length (trim), minimum sawtimber DBH
!  (minBfD), and default log length (merchL).  r, c, e, p, b, and a are
!  the coefficients for inside-bark calculations.
...
!_______________________________________________________________________
!
!  Correction factors to account for proper merchandising and
!  to bring volumes in line with mill studies and legacy system.

Revised MGV 12/10/10
...
! Added species 621 relow-poplar (YW 07/13/2018)
  cf1=1.0
  cf2=1.0
  cf3=1.0
  cf4=1.0
...
!       GLOBAL
!
!  r,c,e,p,a,b real       Clark coefficients for total height model
!  a4,b4       real       Clark coefficients for DIB at DBH
!  a17,b17     real       Clark coefficients for DIB at 17.3'
!  COEFFS      CLKCOEF    Type(structure) for passing coefficients
!  bfpFlg      integer    flag for sawtimber board foot volume
!  bolHt       real(21)   array of heights of sawlog ends
!  cdpFlg      integer    flag for sawtimber cord volume
!  cfVol       real       calculated cubic foot volume
!  cupFlg      integer    flag for swtimber cubic foot volume
!  cutFlg      integer    flag for total cubic foot volume
!  dbhIb       real       diameter at breast height inside bark
!  dbhOb       real       diameter at breast height outside bark
!  dib17       real       diameter inside bark at a height of 17.3'
!  errFlg      integer    error flag code
!  forst       char*2     forest number
!  geog        integer    geographic area code
!  ht1Prd      real       sawtimber height
!  ht2Prd      real       pulp or topwood height
!  htTot       real       total height to tree tip
!  iProd       integer    numeric product code
!  logDia      real(7,21) array of end diameters for sawlogs
!  logLen      real(20)   array of log lengths for sawtimber
!  logVol      real(7,20) array of volumes for sawlogs
!  maxLen      real       maximum allowable length of a log
!  merchL      real       length of standard logs
!  minBfD      real       minimum board foot diameter
!  minLen      real       minimum allowable length of a log
!  mTopP       real       merchantable top diameter for sawtimber
!  mTopS       real       merchantable top diameter for pulp and topwood
!  numSeg      integer    number of sawlogs in the tree
!  plpDib      real       top DIB for pulp or topwood volume calculation
!  plpHt       real       height for pulp or topwood volume calculation
!  prod        char*2     product code
!  sawDib      real       top DIB for sawtimber volume calculation
!  sawHt       real       height for sawtimber volume calculation
!  short       logical    flag to indicate a short tree
!  shrtHt      real       height used for calculations when tree is short
!  spFlg       integer    flag for topwood volumes
!  spp         integer    species code
!  stump       real       stump height
!  tcfVol      real       calculated cubic foot volume for all products
!  topDib      real       top DIB to which ht2Prd was measured
!  topHt       real       uppermost height measured
!  totHt       real       total tree height for volume calculations
!  trim        real       trim allowance between logs
!  vol         real(15)   array of volumes
!  volEq       char*10    volume equation string
!_______________________________________________________________________
!
!       R9PREP
!
!  lowrHt      real       lower height for a volume calculation
!  match       logical    flag to indicate a species code match
!  sppGrp      integer    species group code for assigning coefficients
!  upprHt      real       upper height for a volume calculation
!_______________________________________________________________________
!
!       R9TOTHT
!
!  Im          real       stem location flag for Clark equations
!  Qa,Qb,Qc    real       intermediate calculations for Clark equation
!_______________________________________________________________________
!
!       R9CUFT
!
!  I1-I6       real       indicator used in Clark volume equation
!  G,W,X,Y,Z,T real       intermediate calculations for Clark equation
!  L1-L3,U1-U3 real       intermediate calculations for Clark equation
!_______________________________________________________________________
!
!       R9DIB
!
!  Is,Ib,It,Im real       stem location flag for Clark equations
!_______________________________________________________________________
!
!       R9HT
!
!  Is,Ib,It,Im real       stem location flag for Clark equations
!  G,W,X,Y,Z   real       intermediate calculations for Clark equation
!  Qa,Qb,Qc    real       intermediate calculations for Clark equation
!_______________________________________________________________________
!
!       R9BDFT
!
!  bdft        real       board foot volume for a log
!  dib         real       DIB at the top end of the current log
!  ht          real       height to the top of the current log
!  iDib        integer    1" diameter class for Scribner calculations
!  leftov      real       length left over above full-length logs
!  len         real       length of a log
!  logV        real       volume calculated for a log
!  scrbnr      real(120)  array of factors used in Scribner calculations
!
!_______________________________________________________________________
!
