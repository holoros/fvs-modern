...
!
!  Purpose :
!     This is the master routine for reading root disease keywords.
!
!     ** There were variant specific versions of this subroutine, due to the
!     ** differing number of species represented in the variants. By changing
!     ** formatted reads to unformatted, multiple versions of this subroutine
!     ** have been eliminated. There is now just one subroutine.
!     **
!     ** Variants once having custom verions of RDIN include:
!     ** BM, CI, CR, EC, EM, IE, KT, NC, PN, SO, TT, UT, WC, WS
!     **
!     ** Format statements 606, 711, 1321, 2921 (11f5.2) and 3963 (11I5) were
!     ** dependent on parameter MAXSP. They have been changed to unformatted
!     ** reads. This makes them a little less user hostile. Values can be
!     ** delimited by space and/or comma and can be on multiple lines. The
!     ** only thing that matters is that a value exist for every species
!     ** in the variant, MAXSP. Lance David 01/08/2016
!
!
!  Called By :
!     INITRE  [PROGNOSIS]
...
!  17-JUL-2007 Lance R. David (FHTET)
!     Removed condition testing IREC1 to determine if tree records had already
!     been processed and terminating to run if they had. The restructuring of
!     damage code processing eliminates this requirement.
!  08/28/14 Lance R. David (FMSC)
!     Added implicit none and declared variables.
!  03/24/15 Lance R. David
!     For implementation of General Report Writer facility and addition
!     of output to database option, BBOUT and RRDOUT keywords modified.
!
!----------------------------------------------------------------------
!
...
! 208 FORMAT(/A8, '   STATIC CARRYOVER MODEL',
!    &     2X,'; PROB. OF NEW INFECTION CENTERS BEING FORMED=',F4.2,
!    &      ';  NEW CENTERS = ',I4,/
!    &        T12,'MODEL CALLED ',I4,' CYCLES AFTER MANAGEMENT')
!     GOTO 90
!
! 207 CONTINUE
!     IF(LKECHO)WRITE(JOSTND,210) KEYWRD,ARRAY(2),PINSET
! 210 FORMAT (/A8,'*** CARRYOVER MODEL KEYWORD, PROBABILITY',
!    & ' OF INFECTION',/,
!    &      T12, '***** ASSIGNED ',F4.2,' OUT OF BOUNDS, SET TO ',F4.2)
!     GOTO 90
!
! 206 CONTINUE
!     IF(LKECHO)WRITE(JOSTND,211) KEYWRD,RRGEN(1,7),IRGEN(1)
! 211 FORMAT (/A8,'   DYNAMIC CARRYOVER MODEL, MINIMUM SPREAD= ',
...
!.... If no date provided, just report default values.
...
!     CHECK THAT ROOT DISEASE TYPE IS ACCEPTABLE
!     Note: IRGEN(9) is set to 3000 in rdinit and is flag that values
!     change at initialization; otherwise, SSSFAC is loaded and applied
!     on specified year in subroutine rdtreg.
...
!.... If no species provided, just report default values.
...
!M  2002 FORMAT (T12,'OUTPUT WILL BE IN LOTUS READABLE FORMAT')
...
!2159 FORMAT (A10,' ISL=',I4, ' ISPC=',I4, '   DEN=',F8.2,
!    &        '   DIAM=',F8.2, '   RTD=',F8.2,
!    &        '   IRRSP=',I4)
...
! 3000 FORMAT (/'BORAX      PROP TREAT:', F4.2, '   MIN DBH:', F6.2)
...
!  12/11/14 implementatin of general report facilty eliminates the need
!  for unit number specification (field 1) and opening of the file
!  which occurs elsewhere. Report will be written to main out file and
!  will be optionally written to output DB, but I have not done the DB
!  part yet. -LD

!X      IBBOUT = 27
!X      IF (ARRAY(1) .GT. 0.0) IBBOUT = INT(ARRAY(1))

!.... OUTPUT FOR BBOUT KEYWORD

!X      IF(LKECHO)WRITE(JOSTND,3501) KEYWRD, IBBOUT
!X 3501 FORMAT (/A8,'   DETAILED BARK BEETLE MORTALITY OUTPUT ON ',
!X     &               'LOGICAL UNIT ',I4)

!     Set IBBOUT as trigger to call output routine.
...
!....       Change all species
!....
!....       Formatted read changed to unformatted for ease of use.
!....       Supplemental record of Annosus type assignment (0, 1, 2) values
!....       can be on one or more lines and delimited by space or coma.
