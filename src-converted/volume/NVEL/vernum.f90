...
! 20160113 Added BTR default value for Region 3 Santa Fe forest DF and PP
! 20160125 Modify r9ht in r9clark.f to check NaN value for stemHt. The old check doesnot work well.
! 20160126 Added weight factor for R3 Santa Fe forest (Modified regdftdata.inc)
! 20160302 Modified r9clark.f to remove the error check for HtTot < 17.3. The calc will use small tree logic
! 20160322 Modified r8clkdib.f DIB calculation for UPSHT1 < 17.3 to avoid Nan value error.
!          Added hardcoded weightfactor for DF in Rogue River - Siskiyou NF based on DBH
!          Added volume rounding to R8CLARK to be same as the one in r8vol rounding
!
! 20160408 Added BTR for R3 F10 WF (91.8) for testing
! 20160505 Added DIST to VOLINIT and VOLLIBCS to set VOL(2) with Scribner or International for Region 8.
! 20160517 Added biomass calculation for sapling adjustment factor; set LOGST to 0 in Profile.f for non variable log cruising
! 20160713 Modified fwinit to add 301FW2W122, 301FW2W202, 301FW2W015, and 301FW2W108 for Santa Fe NF.
! 20160922 (1)Added DRC to DBH conversion and when DRC is measured, convert it to DBH bafore call Jenkins equation for biomass calc
!          (2)Added output variable LOGDIA,LOGLEN,LOGVOL to R4vol subroutine
!          (3)Updated R6 weight factor for GF, NF, PSF, DF and MH in Mt Hood, DF in Siuslaw and WF, PP and DF in Umatilla NF.
! 20160930 Correct the error in CRZSPDFT for searching the weight factor in the last of the array
!          and made correction to LAST = HALF-1 to avoid infinit loop.
! 20161121 Removed the weight factor for douglas-fir in Siuslaw NF. They want to use the regional weight factor for DF.
! 20170209 Added EZVOLLIB subroutine to volumelibrary.f
! 20170214 Added vernum_r, getvoleq_r and vollib_r for use by R program
! 20170227 Modified crzbiomass small tree biomass calculation, i.e. when merch stem vol is calculated, the stem biomass is also calculated.
! 20170414 Removed 532WO2W*** eqn from R5_EQN list, modified CALCDIA to include R4 MAT model, added HT2TOPD to calculate stem HT to a
!          given stem DIB, modified log vol calc for R8 and R9 to also include topwood logs, moved stump and tip vol calc to volinit sunroutine.
! 20170525 Added BIOLIB subroutine to export biomass
! 20170721 Added new equation number for R8 Clark equation using R9 codes. The new R8 equation looks like 8*1CLKE*** and update R6 weight factor.
!          Modified R9clark to recognize broken top input from ht1prd and mtopp ot ht2prd and mtops
! 20170918 Added VOLLIBVB in volumelibrary.f to let VB.net to call the library with MERRULES variable.
! 20171221 Modified R9Clark for NaN error, modified Mrule for custom rule, add R8clkht to r8vol2
! 20180125 Removed R9clark broken top calculation, which cause other problem for cruise processing
! 20180202 modifying R8prep with outside bark coef
! 20180312 Modified r8prep and r9clark for R8 upsHt1
...
! 20190426 Changed volinit.f to set the default stump and MTOPP for FIA equation only
!          Added equation for BIA I16BEHW000 (Behr), I00DVEW000 (Johnson formclass) and C00DVEE***.
!          Added Equation for FVS A02DVEW094 (Malone et al 2013) for statewide white spruce
!          Added equation for blackjack 301HAB0122, 302HAB0122 and yellow pine 300HAB1122
...
!     R program need subroutine name to be all lower case
