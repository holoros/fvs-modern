## =============================================================================
## 52b_fit_modifier_v2_hg_fast.R
##
## Faster v2 modifier fit. Mirrors 52_fit_modifier_v2_hg.R but
## subsamples pairs BEFORE building the data table and computing eta,
## so we don't compute eta for 2M rows just to keep 30K. Also skips the
## full read of fit$summary() until we actually need it.
##
## Loads the 5.2 GB HG fit object once (slow), computes eta only on the
## final subsampled rows, then fits modifier_v2.stan. Total wall ~3 min
## after the initial fit load.
## =============================================================================
...
cat("==============================================================\n")
cat("FVS-CONUS Phase 3 v2 modifier fit (HG, fast variant)\n")
cat("==============================================================\n")
...
## Now derived covariates only on the subsample
...
cat(sprintf("plant prev=%.3f taper_mean=%.3f gamma_mean=%.3f\n",
            mean(stan_data$is_plantation),
            mean(stan_data$plant_age_taper),
            mean(stan_data$gamma_kernel_dstrb)))
