...
! CANADA-ON $Id$
!----------
!  **from LS_NEW: imported to allow age-dubbing for Mowraski volume
!----------
!  THIS SUBROUTINE COMPUTES THE PREDICTED HEIGHT USING SITE INDEX
!  CURVES SHOWN IN:
!  WILLARD H. CARMEAN, JEROLD T. HAHN, & RODNEY D. JACOBS,
!  SITE INDEX CURVES FOR FOREST TREE SPECIES IN THE EASTERN UNITED
!  STATES. USDA FOREST SERVICE, NORTH CENTRAL FOREST EXPERIMENT STATION,
!  GTR: NC-128. 142P.
!
!  IF N .LE. 0 THEN CALCULATE AGE USING HEIGHT = H
!  IF N .GT. 0 AND .LT. 2. THEN CALCULATE HEIGHT H USING AGE AGET
!  IF N .GE. 2 THEN CALCULATE HEIGHT INCREMENT
!
!  CALLED FROM: FINDAG, REGENT
!
!  LOCAL VARIABLES:
!  IVAR = VARIANT INDICATOR
!         1 = LS ** ONLY LS
!  ISPC = SPECIES
!    SI = SITE INDEX FOR THE SPECIES
!    BA = STAND BASAL AREA
!   YRS = PERIOD LENGTH FOR HT GROWTH (E.G. 5 YRS OR 10 YRS)
!  AGET = TREE AGE
!     H = CURRENT TREE HEIGHT
!     N = CALCULATION MODE (DESCRIBED ABOVE)
!     D = TREE DBH
!   PCT1 = PERCENTILE OF TREE
!
!  RETURNED VARIABLES
!   HTG1 = HEIGHT INCREMENT FOR CYCLE
...
!----------
!  CALCULATE MAXIMUM HEIGHT FOR SPECIES, IF CURRENT TREE HEIGHT
!  H >= HTMAX THEN RETURN
!----------
HTMAX=(B1*SI**B2)
...
ENDIF
!----------
!  IF 0 < N < 2 THEN CALCULATE TREE HEIGHT BASED ON AGE
!----------
...
ENDIF &
   W (not used in ON) &
   ------- &
   IGHT GROWTH EQUATION, USING NC128 TREE HEIGHT COEFFS. &
   ALUATED FOR EACH TREE EACH CYCLE (YRS YEARS STANDARD CYCLE LENGTH) &
   -------
 HTG0=  BH + B1*SI**B2*(1.-EXP(B3*AGET))**(B4*SI**B5)
 HTGP5= BH + B1*SI**B2*(1.-EXP(B3*(AGET+YRS)))**(B4*SI**B5)
 HTG1= HTGP5 - HTG0

!... replace the LS height growth calculation method with Penner 2010
!... this is identical to small tree height growth done in RGNTSW for
!... regenerating trees. HTG1 is annual height growh (ft)
