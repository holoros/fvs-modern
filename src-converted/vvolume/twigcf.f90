...
IMPLICIT NONE
!----------
! LS $Id: twigcf.f 2450 2018-07-11 17:28:41Z gedixon $
!----------
! THIS ROUTINE CALCULATES TOTAL CUBIC FOOT VOLUME AND MERCH
! CUBIC FOOT VOLUME FOR A TREE.  CORRECTIONS FOR TOP KILL AND
! DEFECT ARE STILL CALCULATED IN VOLS.
!------------------------------------------------------------------
!  CENTRAL STATES VARIANT
!
!  CENTRAL STATES ACCEPTABLE TREE CLASS VOLUME EQUATION:
!  --SMITH, W. BRAD; WEIST, CAROL A. 1982. A NET VOLUME EQUATION
!    FOR INDIANA. RESOURCE BULL. NC-63. ST. PAUL, MN: US DEPT OF
!    AG, FS, NCFES.
!
!         V = A*SI**B*[1-EXP(-C*(DBH**D)]
!
!  WHERE:
!      V = VOLUME
!     SI = SITE INDEX
!    DBH = DIAMETER BREAST HEIGHT
!    A-D = SPECIES SPECIFIC COEFFICIENTS
!
!----------
!  LAKE STATES VARIANT
!
!  LAKE STATES ACCEPTABLE TREE CLASS VOLUME EQUATION:
!  --RAILE, GERHARD K., W.B. SMITH, C.A.WEIST. 1982. A NET VOLUME
!    EQUATION FOR MICHIGAN'S UPPER AND LOWER PENINSULAS. 12 P.
!    USDA FOREST SERVICE GEN TECH REPORT NC-80.
!
!         V = A*SI**B*[1-EXP(-C*DBH)]**D
!
!  WHERE:
...
!    A-D = SPECIES SPECIFIC COEFFICIENTS
!
!  UNDESIRABLE TREE VOLUME IS CALCULATED WITH THE SAME EQUATION BUT
!  VOLUME IS REDUCED BY A REDUCTION FACTOR.
!
!----------
!  NORTHEAST VARIANT
!
!  NORTHEASTERN FOREST SURVEY VOLUME EQUATIONS:
!  --SCOTT, CHARLES, T. 1981. NORTHEASTERN FOREST SURVEY REVISED
!    CUBIC-FOOT VOLUME EQUATIONS. USDA. FOR. SERV. RES. NOTE NE-304.
!
!  CFVOL=B1+B2(DBH)^B3+(B4(DBH)^B5)*H^B6
!
!  WHERE H = HEIGHT (IN FEET) TO A TOP DIAMETER OUTSIDE BARK (TDOB)
!  DEFAULT TDOB FOR CFVOL = 4 INCHES FOR ALL SPECIES.
!
!  MERCHANTABLE HEIGHT EQUATIONS.
!  --YAUSSY, DAN. E. AND M.E. DALE. 1990.  SAWLOG AND BOLE LENGTH
!    GENERALIZED MERCHANTABLE HEIGHT EQUATIONS FOR THE NORTHEASTERN
!    UNITED STATES.  USDA. FOR. SERV. RES. PAP. NE-___.
!
!    HEIGHT = C1*SI^C2*(1-EXP(C3*(DBH-TDOB)))
!----------
!OMMONS
!
INCLUDE 'PRGPRM.f90'
...
!----------
!  BHB1 --  INTERCEPT COEFFICIENT IN THE MODEL PREDICTING BOLE HEIGHT
!           TO A 4-INCH TOP (ONE COEFFICIENT PER SPECIES).
!  BHB2 --  COEFFICIENT FOR SITE INDEX TERM IN THE MODEL PREDICTING
!           BOLE HEIGHT TO A 4-INCH TOP (ONE COEFFICIENT PER SPECIES).
!  BHB3 --  COEFFICIENT FOR (DBH-TDOB) TERM IN THE MODEL PREDICTING
!           BOLE HEIGHT TO A 4-INCH TOP (ONE COEFFICIENT PER SPECIES).
!  SHB1 --  INTERCEPT COEFFICIENT IN THE MODEL PREDICTING SAWLOG
!           HEIGHT TO A MERCHANTABLE TOP.
!  SHB2 --  COEFFICIENT FOR SITE INDEX TERM IN THE MODEL PREDICTING
!           SAWLOG HEIGHT TO A MERCHANTABLE TOP.
!  SHB3 --  COEFFICIENT FOR (DBH-TDOB) TERM IN THE MODEL PREDICTING
!           SAWLOG HEIGHT TO A MERCHANTABLE TOP.
!  NECF1--  B1 COEFFICIENT IN THE NORTHEASTERN FOREST SURVEY CUBIC-
!           FOOT VOLUME EQUATION.
!  NECF2--  B2 COEFFICIENT IN THE NORTHEASTERN FOREST SURVEY CUBIC-
!           FOOT VOLUME EQUATION.
!  NECF3--  B3 COEFFICIENT IN THE NORTHEASTERN FOREST SURVEY CUBIC-
!           FOOT VOLUME EQUATION.
!  NECF4--  B4 COEFFICIENT IN THE NORTHEASTERN FOREST SURVEY CUBIC-
!           FOOT VOLUME EQUATION.
!  NECF5--  B5 COEFFICIENT IN THE NORTHEASTERN FOREST SURVEY CUBIC-
!           FOOT VOLUME EQUATION.
!  NECF6--  B6 COEFFICIENT IN THE NORTHEASTERN FOREST SURVEY CUBIC-
!           FOOT VOLUME EQUATION.
!
!----------
...
!  CENTRAL STATES VARIANT
!
!  COEFFICIENTS FOR NET CUBIC FOOT VOLUME FOR ACCEPTABLE TREES
!  (TREE CLASS 1)
!----------
DATA CSA1/ &
...
!----------
!  COEFFICIENTS TO CALCULATE NET MERCHANTABLE CUBIC FOOT VOLUME
!  FOR ACCEPTABLE TREES (TREE CLASS 1)
!----------
DATA CSB1/ &
...
!----------
!  COEFFICIENTS FOR NET CUBIC FOOT VOLUME FOR UNDESIRABLE TREES
!  (TREE CLASS 2)
!----------
DATA CSC1/ &
...
!----------
!  COEFFICIENTS TO CALCULATE NET MERCHANTABLE CUBIC FOOT VOLUME
!  FOR ACCEPTABLE TREES (TREE CLASS 1 & 2)
!----------
DATA LSB1/ &
...
DATA LSB4/ &
     ...
     25*5.7524381/
!----------
!  NET CUBIC FOOT REDUCTION FACTOR FOR ACCEPTABLE TREES (TREE CLASS 2).
!----------
DATA LSC1/ &
...
!
!----------
! SET INITIAL VALUES.  IF DIAMETER LIMITS NOT MET THEN RETURN.
!----------
 IF(IMC(I) .EQ. 1)THEN
    D1=CSA1(ISPC)
    D2=CSA2(ISPC)
  ...
  ENDIF
!----------
!  CALCULATE NET CUBIC FOOT VOLUME (VN)
!----------
 VN = D1*(SITEAR(ISPC)**D2)*(1.-EXP(-1.0*D3*(D**D4)))
!----------
!  CALCULATE MERCHANTABLE CUBIC FOOT VOLUME (VM)
!----------
 VM = 0.
...
!----------
!  REDUCE SOUND VOLUME IF TREE IS UNDESIREABLE
!----------
...
!----------
!  CALCULATE MERCH CUBIC FOOT VOLUME (VM)
!----------
 IF ((D .LT. BFMIND(ISPC)) .OR. IMC(I) .NE. 1)GOTO 300
!----------
!  CALCULATE SAWLOG HEIGHT
!----------
