...
!     FIND VARIANT - ROUTINES VARY FOR HT/DBH AND HT-AT-DIAMETER CALCULATIONS
...
IF  ((DX .LT. DBHMIN(SPIYV)) .AND. &
       ((VVER(1:2) .EQ. 'SN') .OR. (VVER(1:2) .EQ. 'LS') .OR. &
        (VVER(1:2) .EQ. 'NE') .OR. (VVER(1:2) .EQ. 'CS') .OR. &
        (VVER(1:2) .EQ. 'ON'))) THEN
...
!       shortleaf pine
  CASE (1:14,69:72) ! ALL THE CONIFERS
...
!       TO INCLUDE ANY BOLEWOOD--JUST BRANCHWOOD
  ...
  CASE (1:14,69:72) ! shortleaf pine
    IF (TTOPW .LT. UMBTW(4)) TTOPW = UMBTW(4)
    XV(0)=FOL
