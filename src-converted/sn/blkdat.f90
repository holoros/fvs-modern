...
DATA (NSP(I,3),I= 1,90) / &
    ...
    'OH3','OT3'/
!----------
!     SPECIES LIST FOR SOUTHERN VARIANT.
!
!    1 = FIR (FR)                      ABIES
!    2 = REDCEDAR (JU)                 JUNIPERUS
!    3 = SPRUCE (PI)                   PICEA
!    4 = SAND PINE (PU)                PINUS CLAUSA
!    5 = SHORTLEAF PINE (SP)           PINUS ECHINATA
!    6 = SLASH PINE (SA)               PINUS ELLIOTTII
!    7 = SPRUCE PINE (SR)              PINUS GLABRA
!    8 = LONGLEAF PINE (LL)            PINUS PALUSTRIS
!    9 = TABLE MOUNTAIN PINE (TM)      PINUS PUNGENS
!    10 = PITCH PINE (PP)              PINUS RIGIDA
!    11 = POND PINE (PD)               PINUS SEROTINA
!    12 = EASTERN WHITE PINE (WP)      PINUS STROBUS
!    13 = LOBLOLLY PINE (LP)           PINUS TAEDA
!    14 = VIRGINIA PINE (VP)           PINUS VIRGINIANA
!    15 = BALD CYPRESS (BY)            TAXODIUM DISTICHUM
!    16 = POND CYPRESS (PC)            TAXODIUM ASCENDENS
!    17 = HEMLOCK (HM)                 TSUGA
!    18 = FLORIDA MAPLE (FM)           ACER BARBATUM
!    19 = BOXELDER (BE)                ACER NEGUNDO
!    20 = RED MAPLE (RM)               ACER RUBRUM
!    21 = SILVER MAPLE (SV)            ACER SACCHARINUM
!    22 = SUGAR MAPLE (SM)             ACER SACCHARUM
!    23 = BUCKEYE (BU)                 AESCULUS
!    24 = BIRCH (BB)                   BETULA
!    25 = SWEET BIRCH (SB)             BETULA LENTA
!    26 = AMERICAN HORNBEAM (AH)       CARPINUS CAROLINIANA
!    27 = HYBRID HICKORY (HI)          CARYA
!    28 = CATALPA (CA)                 CATALPA
!    29 = HACKBERRY (HB)               CELTIS
!    30 = EASTERN REDBUD (RD)          CERCIS CANADENSIS
!    31 = FLOWERING DOGWOOD (DW)       CORNUS FLORIDA
!    32 = COMMON PERSIMMON (PS)        DIOSPYROS VIRGINIANA
!    33 = AMERICAN BEECH (AB)          FAGUS GRANDIFOLIA
!    34 = ASH (AS)                     FRAXINUS
!    35 = WHITE ASH (WA)               FRAXINUS AMERICANA
!    36 = BLACK ASH (BA)               FRAXINUS NIGRA
!    37 = GREEN ASH (GA)               FRAXINUS PENNSYLVANICA
!    38 = HONEYLOCUST (HL)             GLEDITSIA TRIACANTHOS
!    39 = LOBLOLLY BAY (LB)            GORDONIA LASIANTHUS
!    40 = SILVERBELL (HA)              HALESIA
!    41 = AMERICAN HOLLY (HY)          ILEX OPACA
!    42 = BUTTERNUT (BN)               JUGLANS CINEREA
!    43 = BLACK WALNUT (WN)            JUGLANS NIGRA
!    44 = SWEETGUM (SU)                LIQUIDAMBAR STYRACIFLUA
!    45 = TULIPTREE (YP)               LIRIODENDRON TULIPIFERA
!    46 = MAGNOLIA (MG)                MAGNOLIA
!    47 = CUCUMBER TREE (CT)           MAGNOLIA ACUMINATA
!    48 = SOUTHERN MAGNOLIA (MS)       MAGNOLIA GRANDIFLORA
!    49 = SWEETBAY (MV)                MAGNOLIA VIRGINIANA
!    50 = BIGLEAF MAGNOLIA (ML)        MAGNOLIA MACROPHYLLA
!    51 = APPLE (AP)                   MALUS
!    52 = MULBERRY (MB)                MORUS
!    53 = WATER TUPELO (WT)            NYSSA AQUATICA
!    54 = BLACKGUM (BG)                NYSSA SYLVATICA
!    55 = SWAMP TUPELO (TS)            NYSSA BIFLORA
!    56 = HOPHORNBEAM (HH)             OSTRYA VIRGINIANA
!    57 = SOURWOOD (SD)                OXYDENDRUM ARBOREUM
!    58 = REDBAY (RA)                  PERSEA BORBONIA
!    59 = AMERICAN SYCAMORE (SY)       PLATANUS OCCIDENTALIS
!    60 = COTTONWOOD (CW)              POPULUS
!    61 = BIGTOOTH ASPEN (BT)          POPULUS GRANDIDENTATA
!    62 = BLACK CHERRY (BC)            PRUNUS SEROTINA
!    63 = WHITE OAK (WO)               QUERCUS ALBA
!    64 = SCARLET OAK (SO)             QUERCUS COCCINEA
!    65 = SOUTHERN RED OAK (SK)        QUERCUS FALCATA
!    66 = CHERRYBARK OAK (CB)          QUERCUS PAGODA
!    67 = TURKEY OAK (TO)              QUERCUS LAEVIS
!    68 = LAUREL OAK (LK)              QUERCUS LAURIFOLIA
!    69 = OVERCUP OAK (OV)             QUERCUS LYRATA
!    70 = BLACKJACK OAK (BJ)           QUERCUS MARILANDICA
!    71 = SWAMP CHESTNUT OAK (SN)      QUERCUS MICHAUXII
!    72 = CHINKAPIN OAK (CK)           QUERCUS MUEHLENBERGII
!    73 = WATER OAK (WK)               QUERCUS NIGRA
!    74 = CHESTNUT OAK (CO)            QUERCUS PRINUS
!    75 = NORTHERN RED OAK (RO)        QUERCUS RUBRA
!    76 = SHUMARD OAK (QS)             QUERCUS SHUMARDII
!    77 = POST OAK (PO)                QUERCUS STELLATA
!    78 = BLACK OAK (BO)               QUERCUS VELUTINA
!    79 = LIVE OAK (LO)                QUERCUS VIRGINIANA
!    80 = BLACK LOCUST (BK)            ROBINIA PSUEDOACACIA
!    81 = WILLOW (WI)                  SALIX
!    82 = SASSAFRAS (SS)               SASSAFRAS ALBIDUM
!    83 = BASSWOOD (BD)                TILIA
!    84 = ELM (EL)                     ULMUS
!    85 = WINGED ELM (WE)              ULMUS ALATA
!    86 = AMERICAN ELM (AE)            ULMUS AMERICANA
!    87 = SLIPPERY ELM (RL)            ULMUS RUBRA
!    88 = OTHER SOFTWOOD (OS)
!    89 = OTHER HARDWOOD (OH)
!    90 = OTHER SPECIES(OT)
!----------
!   THE HT-DBH COEFFICIENTS ARE ASSIGNED IN THE **SITSET ROUTINE
!----------
