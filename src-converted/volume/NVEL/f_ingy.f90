...
!     BALSAM (GF,WF OR SUBALPINE FIR) B.C.  x
...
!     PONDEROSA PINE  Central Idaho Taper Project
!      DATA (F(I,22),I=10,47)/ 0.59532444D+00, 0.71960574D+00,
!     >              0.00000000D+00, -0.35325066D+00, -0.36096083D+00,
!     >             -0.29617657D+00,  0.00000000D+00,  0.0,
!     >             -0.51250474D+02,  0.10000000D+02,  0.10000000D+02,
!     >              0.99000000D+00, -0.23852919D-03,  0.0,
!     >              0.0,             0.32567984D+01, -0.16274389D+01,
!     >             -0.65209881D+01,  0.15847455D+01,  0.70000000D+01,
!     >             -0.31937474D+02, -0.17812912D+01,  0.80409553D+01,
!     >             -0.14459854D+00,  0.24745882D+01,  0.31064531D+01,
!     >             -0.84611975D+00, -0.42192772D+00, -0.70000000D+01,
!     >              0.00000000D+00,  0.00000000D+00,  0.00000000D+00,
!     >              0.70100000D+01,  0.00000000D+00,  0.0,
!     >              0.12686459D+01, -0.52691593D-01, -0.32305446D-01/
!
!     WHITE FIR/GRAND FIR  Central Idaho Taper Project
...
!                                  And check whether they exist
    IF (JSP.EQ.11.and.(geosub.eq.'15' .or. geosub.eq.'03')) then
!                                     Doug fir
       geotemp = '15'
       JRSP = 10
!                                     Move all coed to JRSP=10
       DO 20 I=10,47
         F(I, JRSP) = F(I, JSP)
20 continue
!                                      Overwrite with regional mods
       DO 40 IP=1,3
...
!     GRAND FIR  x Central Idaho Taper
...
!     PONDEROSA PINE  x Central Idaho Taper
!      DATA (F(I,13),I=10,12)/ 0.59532444D+00, 0.71960574D+00,
!     >                   0.00000000D+00/

!      DATA (V(I,13),I= 1,16)/-0.29870739D+01,-0.11207714D+01,
!     >             -0.12793257D+01, 0.51821007D+01,-0.35197794D+00,
!     >              0.75094517D+00, 0.44774052D-02, 0.74892256D-01,
!     >              0.39386984D+00, 0.42129525D+00,-0.63181585D+01,
!     >             -0.19238006D+00,-0.59970586D+00, 0.16594531D+01,
!     >              0.25120253D+00, 0.85204162D+00/
...
!                Underlying Eqn:
!                     RATIO=DBT/DBHob
!                     Y2 = logit(RATIO) = ln[ ratio/(1-ratio) ]
...
!      ponderosa pine
!      if (jspr .eq. 4 .and. geosub .eq. '15') then
!        FDBT_C2 = dbh-(0.6817382 + 0.9548815*dbh - 0.8475764*LOG(dbh))
!        return
!      endif
!      engelmann spruce
...
!                      Upper-stem bark thickness   - Function A, Coop 2
!**********************************************************************

!      h            in      R    height at which bark thickness is wanted
!      dib          in      R    Diameter outside bark at height h.
!      brk_upa2   returned  R    Ratio of DBT/DOB at selected height
!      IER          OUT     I     0: Normal
!                                  1: the dob-dependent estimate was not
!                                    obtained due to a failure in root
!                                    finding. This represents either an
!                                    error in the upper-stem bark thickness
!                                    coefficients, or an error in programming.
!                                    (note: plausible answer is returned)
!
...
!                 English units only
!
!               PART 1. Iteratively solve for the dob-dependent RATIO.
!
!                            Construct STARTING  guess as to DOB:
  RATIO_0 = BRK_UPH2(JSP,DBH,TOTALH,DBT_BH,h)
  RATIO1  = RATIO_0
  DOB1    = dib /(1.0 - RATIO_0)

!                            Apply the GUESS to see what we get
  RATIO  = BRK_UPD2(JSP,DBH,DBT_BH,h, DOB1)
  ERR1 = (1.0-RATIO)*DOB1 - DIB

!                           Establish upper and lower bounds for RATIO
  RATIO2 = RATIO_0
  if (err1 .gt. 0.0) Then
...
!                     Use a root-finder to get exact ratio
!                     (DIB will be within .01% of goal when DOB is evaluated)
50 MODE=0
  TOL = -.0001*DIB
...
!                                  answer (RATIO1) is within tolerance
         go to 300
  else
         IER=1
!             if(ier_unit .ne. 0) write(ier_unit,56) 'BRK_UPA2',IER,MODE
!56           format(1x,A8,i3,' MODE=',i3,
!     &              '( from M7SZERO),  bad (HTOP , SI)?'  )
  end if

!                   NO REASONABLE RATIO.  USE starting estimate
200 RATIO1= RATIO_0
...
!**********************************************************************
!         Upper (actually, non BH) stem bark thickness   - Function B, Coop 2
!         THIS is the COMBINED result (-dob and h- dependent)
!         THIS function  REQUIRES that dob is known.

!      h            in      R    height at which bark thickness is wanted
!                                  TOTHT >  h >= 0 ;  h NE 4.5
!      dob          in      R    Diameter outside bark at height h.
!                                  dob > 0
!      brk_upb2   returned  R    Ratio of DBT/DOB at selected height
!
!      LOGICAL METRIC
  INTEGER JSP
  ...
  REAL BRK_UPH2, BRK_UPD2, BRK_UPC2
!                 English units only
!                       obtain the two estimates of RATIO = DBT/DOB
...
!**********************************************************************
!         Upper (actually, non BH) stem bark thickness   - Function C, Coop 2
!         THIS is the COMBINED result (-dob and h- dependent)

!      h            in      R    height at which bark thickness is wanted
!                                  TOTHT >  h >= 0 ;  h NE 4.5
!      ratio_1      in      R    DOB-dependent estimate of dbt/dob
!      ratio_2      in      R    h-dependentt estimate of dbt/dob
!                                  Both ratios MUST be > 0 and  < 1
!      brk_upC2   returned  R    Ratio of DBT/DOB at selected height
!
!
!
  REAL BRK_UPC2
...
!
  JSPR = JSP - 10
  if (H .gt. 4.5) then
!                       convert to estimates of bark area / (o.b. area)
     V1_1 = 1.0 - (1.0-RATIO_1)**2
     V1_2 = 1.0 - (1.0-RATIO_2)**2
!                       make the combined estimator
...
!**********************************************************************
!         Upper (actually, non BH) stem bark thickness   - Function D, Coop 2
!         THIS is the "dob-dependent equations"

!      h            in      R    height at which bark thickness is wanted
!                                  TOTHT >  h >= 0 ;  h NE 4.5
!      dob          in      R    Diameter outside bark at height h.
!                                  dob > 0
!      brk_upd2   returned  R    Ratio of DBT/DOB at selected height
!
!     This incorporates 3 submodels:
!             1)      h < BH
!             2)      h > BH  adjusted hyperbolic  (for spec with b1=0)
!             3)      h > BH  alternative model for species with b1=1
...
!
!                                         One of the two above-BH models
     if (B(1) .eq. 0.0) then
        DR = MIN( DOB/DBH , 1.0 )
...
!**********************************************************************
!         Upper (actually, non BH) stem bark thickness   - Function H, Coop 2
!         THIS is the "height-dependent equation"
!         (was fit to above-BH data;  plausible solutions for H<BH or h>BH)

!      h            in      R    height at which bark thickness is wanted
!                                  TOTHT >  h >= 0 ;  h NE 4.5
!      brk_upH2   returned  R    Ratio of DBT/DOB at selected height
!
!
!
!     This incorporates 3 submodels:
...
!
!
!      Theory: t is a monotonic transformation of h, governed by parm RJ(5)
!              r(t) is a quadratic function that takes on the value 1 at BH.
!              RATIO = r(t) *(DBT_BH/DBHob)
