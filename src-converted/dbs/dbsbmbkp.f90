...
! DBS $Id$
!
!     PURPOSE: POPULATES A DATABASE TABLE WITH DETAILED WWPBM OUTPUT VARS.
!              PERTAINING TO BKP AND RATING VALUES (RVs)
!     AUTH: AJ MCMAHAN -- ITX, INC. SEPT. 2005
!     INPUT:
!
!     OLDBKP:  PRE-DISPERSAL (BUT POST-REPRODUCTION) BKP.
!              SAME AS "MAINOUT"'s PREDBKP
!     NEWBKP:  POST-DISPERSAL (& POST-INFLIGHT MORTALITY) BKP.
!              SAME AS "MAINOUT"'s POSTDBKP
!     SELFBKP: BKP ALLOCATED TO SELF
!     TO_LS:   BKP EMMIGRATING FROM SELF TO OTHER STANDS IN THE LANDSCAPE
!     FRM_LS:  BKP IMMIGRATING FROM OTHER STANDS IN LS
!     BKPIN:   BKP IMMIGRATING FROM OUTSIDE WORLD (OW)
!     BKPOUT:  BKP EMMIGRATING FROM SELF TO OW
!     BKPSV:   PERCENT SURVIVAL OF BKP (IN-FLIGHT MORTALITY RATE = (1-SURVIVAL)
!     STRPBKP: BKP USED IN FINAL STRIPKILL/PITCHOUT TREE
!     SRTP_SC: SIZE CLASS OF FINAL STRIPKILL/PITCHOUT TREE
!     REMBKP:  BKP REMOVED VIA SANITATION CUTS
!     RV:      STAND RATING VALUE
!     DVRV1-10:RATING VALUES OF THE 9 COMPONENT DRIVING VARIABLES (DVs)
!     TPAFAST: TPA KILLED BY WWPBM "FAST" DVs
!     BAFAST:  BASAL AREA KILLED BY WWPBM "FAST" DVs
!     VOLFAST: VOLUME KILLED BY WWPBM "FAST" DVs
!
!
!---
!OMMONS
...
ENDIF
!---------
!     CHECK TO SEE IF THE WWPBM "BKPOUT" TABLE EXISTS IN DATBASE
!---------
...
ENDIF
!
!     ASSIGN REAL VALUES TO DOUBLE PRECISION VARS & ARRAYS
!
 OLDBKPB  = OLDBKP
 NEWBKPB  = NEWBKP
 SELFBKPB = SELFBKP
