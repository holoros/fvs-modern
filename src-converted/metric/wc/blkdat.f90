...
!
!     1 = PACIFIC SILVER FIR (SF)      ABIES AMABILIS                   [BA]
!     2 = WHITE FIR (WF)               ABIES CONCOLOR                   [BC]
!     3 = GRAND FIR (GF)               ABIES GRANDIS                    [BG]
!     4 = SUBALPINE FIR (AF)           ABIES LASIOCARPA                 [BL]
!     5 = CALIFORNIA RED FIR (RF)/     ABIES MAGNIFICA                  [BM]
!         SHASTA RED FIR
!     6 =
!     7 = NOBLE FIR (NF)               ABIES PROCERA                    [BP]
!     8 = ALASKA CEDAR (YC)/           CHAMAECYPARIS NOOTKATENSIS       [YC]
!         WESTERN LARCH                LARIX OCCIDENTALIS               [LW]
!     9 = INCENSE CEDAR (IC)           LIBOCEDRUS DECURRENS             [OA]
!    10 = ENGELMANN SPRUCE (ES)/       PICEA ENGELMANNII                [SE]
!         SITKA SPRUCE                 PICEA SITCHENSIS                 [SS]
!    11 = LODGEPOLE PINE (LP)          PINUS CONTORTA                   [PL]
!    12 = JEFFREY PINE (JP)            PINUS JEFFREYI                   [JP] - unchanged
!    13 = SUGAR PINE (SP)              PINUS LAMBERTIANA                [PS]
!    14 = WESTERN WHITE PINE (WP)      PINUS MONTICOLA                  [PW]
!    15 = PONDEROSA PINE (PP)          PINUS PONDEROSA                  [PY]
!    16 = DOUGLAS-FIR (DF)             PSEUDOTSUGA MENZIESII    [FD]
!    17 = COAST REDWOOD (RW)           SEQUOIA SEMPERVIRENS             [OC]
!    18 = WESTERN REDCEDAR (RC)        THUJA PLICATA                    [CW]
!    19 = WESTERN HEMLOCK (WH)         TSUGA HETEROPHYLLA               [HW]
!    20 = MOUNTAIN HEMLOCK (MH)        TSUGA MERTENSIANA                [HM]
!    21 = BIGLEAF MAPLE (BM)           ACER MACROPHYLLUM                [MB]
!    22 = RED ALDER (RA)               ALNUS RUBRA                              [DR]
!    23 = WHITE ALDER (WA) /           ALNUS RHOMBIFOLIA
!         PACIFIC MADRONE              ARBUTUS MENZIESII                [RA]
!    24 = PAPER BIRCH (PB)             BETULA PAPYRIFERA                [EP]
!    25 = GIANT CHINKAPIN (GC) /       CASTANOPSIS CHRYSOPHYLLA [GC] - unchanged
!         TANOAK                       LITHOCARPUS DENSIFLORUS
!    26 = QUAKING ASPEN (AS)           POPULUS TREMULOIDES              [AT]
!    27 = BLACK COTTONWOOD (CW)        POPULUS TRICHOCARPA              [AC] - (should be ACT)
!    28 = OREGON WHITE OAK (WO) /      QUERCUS GARRYANA                 [QG]
!         CALIFORNIA BLACK OAK         QUERCUS KELLOGGII
!    29 = WESTERN JUNIPER (WJ)         JUNIPERUS OCCIDENTALIS   [J ] - unchanged
!    30 = SUBALPINE LARCH (LL)         LARIX LYALLII                    [LA]
!    31 = WHITEBARK PINE (WB)          PINUS ALBICAULIS                 [PA]
!    32 = KNOBCONE PINE (KP)           PINUS ATTENUATA                  [KP] - unchanged
!    33 = PACIFIC YEW (PY)             TAXUS BREVIFOLIA                 [TW]
!    34 = PACIFIC DOGWOOD (DG)         CORNUS NUTTALLII                 [GP]
!    35 = HAWTHORN (HT)                CRATAEGUS sp.                    [HT] - unchanged
!    36 = BITTER CHERRY (CH)           PRUNUS EMARGINATA                [VB]
!    37 = WILLOW (WI)                  SALIX sp.                                [W ]
!    38 =
!    39 = OTHER (OT)
!
DATA JSP / &
