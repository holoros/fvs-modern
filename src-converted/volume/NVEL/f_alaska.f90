...
!== Test for New Inside Bark Models
!     FILE CONTAINS THE FOLLOWING ROUTINES:
!          SHP_AK
!          COR_AK
!          VAR_AK
!***********************************************************************
!      SUBROUTINE BRK_AK(JSP,geosub,DBHOB,DOB,HT2,DBTBH,DIB,dbt)
!***********************************************************************
!                                       ALASKA YC MODEL
!       CHARACTER*2 GEOSUB
!      REAL DBHOB,HT2,DBTBH,DBT,DIB
!      REAL DOB
!      INTEGER JSP
!c          add upper stem bark model here
!      RETURN (0.0)
!      END
!***********************************************************************
!***********************************************************************
  SUBROUTINE SHP_AK(JSP,GEOCODE,DBHIB,HT,RFLW,RHFW)
!                Stem form  SHAPE parameters for COOP #2 (INGY, east-side)
!***********************************************************************
!
...
!     SPRUCE AND HEMLOCK  JSP 33 OR 34
...
!     SPRUCE AND HEMLOCK Second Growth JSP = 35 OR 36
...
!     Check for spruce hemlock second growth sub-model
...
!     coef 1-16 are coef 61 to 76 in coef file
  VA00 = V(1, JSPR)
  VA01 = V(2, JSPR)
  VA02 = V(3, JSPR)
  VB00 = V(4, JSPR)
...
!                                                below BH
         XU = (BH-H) / BH
         IF (XU.LT. 0.111) THEN
!                                     Special adjustment from 4 ft to BH
            VMOD1 = .TRUE.
            X = XU
            XU = 0.111
         ELSE
...
!                                This is above total ht - meaningless.
         VARHAT = 1.0
         SE_LNX = SQRT(VARHAT)
   end if
...
!     Note:  coefficients are from SF_SET2 (possibly file BARKBHC2.COE)
  REAL FDBT_AK
  INTEGER JSP,JSPR
  INTEGER setopt(6)
  REAL DBHIB,HTTOT,DBHOB

!                                       Use Global or regional Lead coef ?
  JSPR = JSP - 30
...
!      WESTERN RED CEDAR
  ...
  ELSEIF(JSPR == 3) THEN
!      SPRUCE
!         DBHIB = -0.164009 + 0.980249*DBHOB - 0.001646*HTTOT
!         DBHIB = -0.189179 + 0.973778*DBHOB
     DBHIB = -0.0171289+0.9511897*dbhob+0.000507*dbhob*dbhob
  ELSEIF(JSPR == 4) THEN
!      HEMLOCK
        DBHIB = -0.054306 + 0.916145*DBHOB + 0.004821*HTTOT
!            DBHIB = 0.040770 + 0.933471*DBHOB

  ELSEIF(JSPR == 5) THEN
!      SPRUCE SECOND GROWTH
        DBHIB = -0.212445 + 0.983230*DBHOB

  ELSEIF(JSPR == 6) THEN
!      HEMLOCK SECOND GROWTH
