...
! CANADA-ON $Id$
!----------
!  THIS SUBROUTINE CALCULATES THE SPECIAL VOLUME EQUATIONS SUBMITTED
!  BY ONTARIO: ZAK AND HONER.
!
!  IT CONTAINS THREE ENTRY POINTS:
!     HONER    - CALCULATES VOLUME USING HONER EQUATIONS
!     ZAKVOL   - CALCULATES VOLUME USING ZAKRZWSKI EQUATIONS
!     ZAKHT    - CALCULATES HEIGHT FOR MERCH VOL ZAKVOL EQUATION
!
!  FOR EASTERN VARIANTS:
!    VN = TOTAL MERCH CUBIC (PULPWOOD AND SAWLOG)
!    VM = NET MERCH VOLUM (NMV) - MOWRASKI CULL, M.PENNER 2010
!    BBFV = MERCH SAWLOG BOARD FOOT VOLUME
!
!  THIS SUBROUTINE IS CALLED IF THE USER ENTERS METHOD 8 IN
!  THE VOLUME KEYWORD.
!
!OMMONS
!
...
!     VARIABLES USED BY ZAK CALCULATIONS
...
!     DEFINE SPECIES MAPPING ARRAYS
!     MAP FROM FVS SPECIES TO DATA VALUES
!       ISPMPZ = SPECIES MAPPING FOR ZAK EQUATIONS
!       ISPMPH = SPECIES MAPPING FOR HONER EQUATIONS
!       ISPMPM = SPECIES MAPPING FOR MOWRASKI CULL EQUATIONS
...
!     PARAMETERS OF THE HONER EQUATIONS
!     =================================
!     B1 here is B2 in documentation
!     1=white pine, 2=red pine, 3=jack pine, 4=Scotch pine
!     5=white spruce, 6=black spruce, 7=red spruce, 8=Eastern hemlock
!     9=Balsam fir, 10=Eastern/northern white cedar, 11=tamarack
!     12=Sugar maple, 13=red maple, 14=yellow birch,
!     15=white/paper birch, 16=red oak, 17=American beech, 18=black ash
!     19=white ash, 20=American elm, 21=basswood
!     22=Ironwood/Eastern hophornbeam & mountain ash & rock elm,
!     23=black/pin cherry,
!     24=largetooth/bigtooth aspen, 25=balsam poplar, 26=quaking aspen
!
!     Note equal parameters for (12, 13, 18, 19), (6,11)
...
!     PARAMETERS OF THE ZAK EQUATIONS
!     ===============================
!     These are the current TCM/PAM coefficients
!     1=white pine, 2=red pine, 3=jack pine,
!     4=black spruce, 5=white spruce, 6=Balsam fir,
!     7=Eastern white cedar, 8=hard maple, 9=aspen,
!     10=white birch, 11=other hardwoods,12=Other softwoods
...
!     PARAMETERS OF THE MOWRASKI CULL EQUATIONS
!     Cull = (1-exp(-AM*age))^BM
!     SEE ISPMPM FOR SPECIES MAPPINGS
!     =========================================
DATA AM / &
...
!     CHANGE TREE INFORMATION INTO METRIC FOR VOLUME CALCULATIONS
...
!     VN=0.0043891*DBH*DBH*(1-0.04365*B2)**2/(C1 + 0.3048*C2/HT)
...
!
!     Vnm = Vm *(r1+r2x+r3x**2)
!     x=tm**2 * dm**(-2)*(1-.04365b2)**-2 * (1+sm/hm)
!     tm = top diameter inside bark = TOPD
!     dm = dbh outside bark = D
!     sm = stump height = STMP
!     hm = total height = H
...
!     CHANGE VOLUME INFORMATION INTO IMPERIAL
...
!     =================================
!     ENTER ZAK VOLUME CALCULATION HERE
...
!     WHEN LTOTAL IS TRUE, THEN A ZERO-HEIGHT STUMP IS NEEDED.
!     TOTAL VOLUME(GROUND TO TIP = GTV Gross Total Volume)ASSUMES NO STUMP
...
!     CALCULATE TOTAL CUBIC FOOT VOLUME (VN)
...
!     ================================
!     ENTER ZAK HEIGHT CALCULATION HERE

ENTRY ZAKHT (D_IN,H_FT,BARK,ISPC,IT,ZHT)

!     THIS ROUTINE IS USED FOR DETERMINING THE HEIGHT THAT SHOULD BE USED
!     WHEN CALCULATING MERCHANTABLE VOLUME.
!
!     CALLED BY VARVOL
...
!
!     ================================
!     ENTER MOWRASKI CULL CALCULATION HERE

ENTRY MOWRASKI (VM,IT,ISPC,CV)

!     THIS ROUTINE IS USED FOR DETERMINING THE NET MERCHANTABLE
!     VOLUME, BASED ON INPUT VM (=GMV=GROSS MERCH VOL) AND INDIVIDUAL
!     AGE (IF MISSING, THEN STAND-AGE)
!     CALLED BY **VARVOL** (OBFVOL)
