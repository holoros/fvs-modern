!  STAND-BGC MODEL -- A physiological, entity level model derived from Forest-BGC
!  (Running 1986) by Kelsey Milner and Dean Coble.
!
! DATE OF LAST REVISION:
!
! REVISED 6/6/03 Amended format for WRITE(66,994)--"EXPAND" now written to 2
!                decimal places.  AJM
!                Changed the WRITE(74,898) so that reported grass height is
!                in feet instead of meters.  all other heights in this output
!                file are in feet).
!
!...Numerous changes made 10/16/00 ajm.  Added LA_SH and associated variables (sumla_, trans_,
! daytrans_, sumtrans_; see entity.f77).  Changed the three WRITE statements pertaining to file 70
! (daystand.out) that report leaf areas.  Added LA_SH (time zero) and SUMLA_SH/AREA for year end, and
! headers.  Three "WRITES" affected: lines 76, 86, 341.
!
! Added FVS_CYC as a passed argument from here to: (1) BGROWN (for its passing to RESPIRE) and (2)
! BZCOMP.  In both RESPIRE and BZCOMP two calculations were heretofore done (needlessly) at the
! beginning of each cycle instead of only once per simulation.  Now they are done only when FVS_CYC
! is equal to 1.
!
!  Adding FVS_CYC as a passed variable to BSTRUCTN. This addition works hand-
!  in-hand with the new subroutine called from BSTRUCTN.  That new subroutine
! ("REDOLAI") is only called during first year of first cycle (it only
!  affects how entities are initialized [but as an aside, it needs to be called
!  late, as it is,--what it does cannot be done in or from BINITIAL]); hence
!  we need to know if we are in cycle one. ajm 6/03
!
! Added 3 new common block variables for accumulating maintResp by entity by tissue pool
! accumulated in RESPIRE: YRMRLF(i),YRMRST(i),YRMRRT(i).
! Adding them to YRENTY output along with the already-existing
! growth maintResp variables ETYLFGR(I),ETYSTGR(I), and ETYFRGR(I) 8/1/03 ajm
! Also (7/25/03) added SUMRT vars (concurrent w/changes related to making water buckets
!  a function of root biomass instead of leaf area)
!
! 1/26/04 this version "3" identical to version "2R" except I'm changing the writing-out
!  of the "dayentity" file for an NAU-specific analysis.  This is a huge output file---
!  one line per entity per day.  For my current analysis, it will generate output for one
! year only, (2003) and will generate approx 204*365=74460 lines. ajm
! REVISING FOR CVS POSTING VERSION2.
! COMMENTING OUT A WRITE AJM 7/7/05
!
...
INCLUDE 'BGCCOM.f90'
!C------------------------------------------------------------
! BRANCH AROUND INITIALIZATION IF USING BGC INCREMENTS AND
! BGC WAS ALREADY INITIALIZED
!------------------------------------------------------------
!     IF(IBGC.EQ.1 .AND. FVS_CYC.GT.1) GOTO 1
!------------------------------------------------------------
...
! Write out initial values for entities (end of year=0)
...
7   CONTINUE
!
! write out stand level data, labels and year 0, to yrstand.out
! Amended 10/00 ajm.  added fvs_cyc to output table, reformatted table.
  WRITE(70,680)
...
! Read in data from MTCLIM
    READ(60,2000,END=1990,IOSTAT=IEOF) JD,TMAX,TMIN,RH,XRAD,PPT,TR
2000     FORMAT(1X,I3,2X,3(F8.2),F10.2,F8.2,F8.5)
!          WRITE(*,*)'JD= ',JD,'TMAX= ',TMAX,'TMIN= ',TMIN,'RH= ',RH,
!     &              'XRAD= ',XRAD,'PPT= ',PPT,'TR= ',TR
!!!!THE INTERVAL ALGORITHM NOT YET IMPLEMENTED--1/5/94,DWC
!         IF(JD.EQ.365) FLAG=1  !update pools & dimensions yearly
!         IF(JD.EQ.INTERVAL(IB,1)) THEN
!           FLAG=1
!           INTERVAL(IB,1)=INTERVAL(IB,1) + JD
!         ELSE
!           FLAG=0
!         ENDIF
!------------------------------------------------------------
! Call daily timestep subroutines
!------------------------------------------------------------
...
! accumulate across entities daily and expand to unit/area basis
...
! accumulate each entity daily for allocation fraction subroutine
      AFPSN(I)=AFPSN(I) + XPSN
      AFTRANS(I)=AFTRANS(I) + TRANS
      AFGRESP(I)=AFGRESP(I) + TOTGRESP
      AFMRESP(I)=AFMRESP(I) + TOTMRESP
! accumulate for each entity for update subroutine
...
! call update subroutine then zero the accumulators

IF (XPSN.LT.TOTMRESP) THEN
!         STRESSDAYS(I)=STRESSDAYS(I)+1         !*!  # days of stress an entity experiences in a year
!         write (*,*) i,jd,xpsn,totmresp,stressdays(i)
ENDIF
!***************************************************************************
! write out growth loop variables
      !output entity-level daily variables
! Commenting out the writing of dayentity output file for release version
! ajm 11/02
! Adding back for nau project testing
! BYPASSING FOR RELEASE AJM JULY 05
GOTO 12
!****************************************************************************
...
!***************************************************************************
!           WRITE(*,*) JD,I,AFPSN(I),AFTRANS(I),AFMRESP(I),AFGRESP(I)
!           IF(JD.EQ.365) THEN
!             WRITE(*,*) JD, YRPSN, YRTRANS, YRMRESP, YRGRESP
!             WRITE(*,*) JD, I, LEAF(I), STEM(I), ROOT(I)
!             WRITE(*,*) I, D(I), H(I), LEAF(I), STEM(I)
!             WRITE(*,*) JD, I, LEAFCF(I),STEMCF(I),FROOTCF(I)
!           ENDIF
! zero out accumulators. BUCKET is zeroed based on the assumption that
! the entity's water bucket will recharge at night from the surrounding
! soil profile, if water is available. See GROW.FOR for more--12/9/93,DWC
! !*!            BUCKET=0.0
!
!*******************************************************************************
12 CONTINUE
...
! accumulate across days for annual total
    YRPSN=YRPSN + SUMPSN
...
! write out results
!         WRITE(*,998) JD, SUMPSN,SUMTRANS,SUMGRESP,SUMMRESP, SITEWP
!         WRITE(*,998) JD, YRPSN, YRTRANS, YRMRESP, YRGRESP, SITEWP
! output stand-level daily variables for year 1
! Amended 10/00 ajm.  added fvs_cyc to output table, reformatted table.
...
997     FORMAT(/,T2,'FVS',T6,'YR',T11,'JD',T19,'PPT',T27,'TMAX',T36, &
   ...
   '================================================================= &
   ===============================================')
!          IF(Z1(1).EQ.1)   !commented out 10/00 ajm.  Now write for all years.
...
! zero-out daily accumulators for the next day.
...
!
! NEW VARIABLES FOR NEW METHOD OF DETERMINING WATER BUCKETS.
! USE ROOT CARBON INSTEAD OF LEAF AREA. ajm 7/03
!
  SUMRT=0.0
  SUMRT_GR=0.0
  SUMRT_SH=0.0
...
!        stressdays=0.0  !*!  sets array to 0
DO 30 I=1,NB         !entity loop for yearly calculations
!  year-end allocation fraction subroutine
   CALL BUPDATE(I,YEAR)
   CALL BALLOCA(I)
   !sum updated stem carbon pools across tree entities
   IF(ID(I).EQ.'T') TSTEM=TSTEM+STEM(I)*EXPAND(I)
! Call year-end turnover subroutine (doesn't include total plant trnovr).
! Note: structure is updated at the start of the next year to avoid
!   calling STRUCT twice.
   CALL BTRNOVR(I)
! Accumulate total stand turnover (per hectar).
...
!     +        F8.3,1X,F6.2)
!
! Redoing table: adding maintResp and Growth resp by tissue pool.
! ajm 8/03
 WRITE(66,994) FVS_CYC,Z1(1),I, TREENO(I),SPP(I), ETYPSN(I), &
       ETYTRANS(I), ETYMRESP(I), ETYGRESP(I),LEAF(I), STEM(I), &
       ROOT(I), ETYZQ1(I)/1000000.,TOTALTO(I),EXPAND(I), &
...
! zero out entity accumulators
   ...
   ETYFRGR(I)=0.0
   TOTALTO(I)=0.0
! zero the yearly entity allocation fraction accumulators for the next yr.
   AFPSN(I)=0.0
   AFTRANS(I)=0.0
   AFGRESP(I)=0.0
   AFMRESP(I)=0.0
   TLEAFON(I)=0.0
   SUMPSI(I)=0.0
! zero out new MaintResp accumulators created 8/03 ajm
   YRMRLF(I)=0.0
   YRMRST(I)=0.0
   YRMRRT(I)=0.0
!-------------------------------------------------------
! Accumulate entity increments for passing to FVS and
! write out entity list to file ENTYLIST.OUT (device 74)
!-------------------------------------------------------
! Amended 10/00 ajm.  added fvs_cyc to output table, reformatted table.
! Also changed units in heading for 12th column to % (from %/100)
! Added ICOV (beginning-of-year % cover for understory to output
!  (Heretofore only cover increment reported)  ajm 10/26/00
...
896    FORMAT(/,T2,'FVS',T7,'YR',T10,'INDX',T16,'ID',T22,'TRENO',T33, &
   ...
   =====')
   IF(ID(I).EQ.'T') THEN
! Set expansion factor to zero for trees killed in BTRNOVR and pass to FVS.
!          IF(IDEADFLG(I).EQ.1) EXPAND(I)=0.0
...
!     &     O_CR(I),FVSCW,HTINC,DGIB,CRINC,SUMHG(I),SUMDG(I),SPP(I)
!  897      FORMAT(I4,2(I4,1X),2X,A1,1X,I8,4(1X,F8.2),9X,5(1X,F8.2)
!     &            T131,A2)
! adding radial growth in mm, printing to %cover column for now
! (which is blank for trees anyway.)ajm 2/10/04
     WRITE(74,897)FVS_CYC,YEAR,I,ID(I),TREENO(I),FVSHT,FVSDBH, &
...
! Define and write out increments in height and cover for grass and shrubs
     HTINC=(H(I)-O_H(I))*3.28  ! Converts to feet
...
! write out stand level data for the year to yrstand.out
! Amended 10/00 ajm.  added fvs_cyc to output table, reformatted table.
...
! Call year-end mortality subroutine (total plant death only)
  CALL BKILL(FVS_CYC)
! zero the yearly accumulators for the next yr.
  YRPSN=0.0
  YRTRANS=0.0
  YRZQ1=0.0
  YRMRESP=0.0
  ...
  SUMWP=0.0
  YRSTNDTO=0.0
! zero the BASE(I,J), LLA(I,J), & LAI(I,J) arrays
  DO 40 M=1,SIZE
    DO 50 N=1,SIZE*2
      LAI(M,N)=0.0
      LLA(M,N)=0.0
...
! ZERO OUT THE INCREMENT ACCUMULATORS
DO 900 I=1,SIZE
  SUMDG(I)=0.0
  SUMHG(I)=0.0
900 CONTINUE
