...
! VCOVR $Id$
!----------
!   COMPUTES TOTAL SHRUB COVER, AND PROBABILITY OF OCCURRENCE, HEIGHT,
!   AND PERCENT COVER FOR 31 SHRUB, FORB, GRASS, AND FERN SPECIES
!   IN GRAND FIR, CEDAR, HEMLOCK, DOUGLAS-FIR, AND SUBALPINE FIR
!   HABITAT TYPES.  TOTAL SHRUB COVER, SPECIES PERCENT COVER, AND
!   SPECIES HEIGHT ARE FROM:
!
!      LAURSEN, STEVE.  1984.  MODELING UNDERSTORY DEVELOPMENT IN
!         INTERMOUNTAIN FORESTS: SHRUB HEIGHT AND COVER
!         DYNAMICS.  PH.D THESIS, UNIV. OF IDAHO, COLLEGE
!         OF FORESTRY, WILDLIFE, AND RANGE SCIENCES, MOSCOW.
!         (IN PREP.)
!
!   PROBABILITY OF OCCURRENCE BY SPECIES IS FROM:
!
!      SCHAROSCH, STEVE.  1984.  PREDICTING THE PROBABILITY OF
!         OCCURRENCE FOR SELECTED SHRUB SPECIES IN THE UNDERSTORY OF
!         NORTH AND CENTRAL IDAHO FORESTS.  M.S. THESIS, UNIV. OF IDAHO,
!         COLLEGE OF FORESTRY, WILDLIFE, AND RANGE SCIENCES, MOSCOW.
!         (IN PREP.)
!
!   THESE RELATIONSHIPS REPLACE PROB, COVER, AND HEIGHT EQUATIONS OF
!   IRWIN AND PEEK, AS OF SPRING, 1984.
!
!   TOTAL SHRUB BIOMASS AND TWIG PRODUCTION FOR 16 SPECIES IN GF/CLUN,
!   WRC/CLUN, AND WH/CLUN HABITAT TYPES ONLY, ARE RETAINED FROM:
!
!         IRWIN, L.L. AND J.M. PEEK. 1979. SHRUB PRODUCTION AND BIOMASS
!            TRENDS FOLLOWING LOGGING WITHIN THE CEDAR-HEMLOCK ZONE
!            OF IDAHO. FOR. SCI., VOL. 25, NO.3: PP. 415-426.
!
!   CVBROW CALLS **CVSCON** IN CYCLE 0 TO COMPUTE SHRUB EQUATION
!   CONSTANTS.  IF CALIBRATION IS BEING DONE, **CVBROW** CALLS
!   **CVBCAL** IN CYCLE 0.
!----------
!            ** GENERAL VARIABLES **
!
!-- IHTYPE -- INPUT HABITAT TYPE CODE (MAY BE DIFFERENT FROM
!             STDINFO CODE)
!-- IHCODE(34) -- VECTOR OF ALL VALID HABITAT TYPE CODES
!-- INF, INFOR(11) -- RETURN SUBSCRIPT, AND MAPPING FUNCTION FOR
!             NATIONAL FOREST GROUPINGS
!-- LBROW --  LOGICAL FLAG TO CALL **CVBROW**
!-- SAGE --   THE CURRENT VALUE OF "TIME SINCE DISTURBANCE".
!-- TIMESD(MAXCY1,2) -- TIME SINCE STAND DISTURBANCE, STORED FOR OUTPUT
!             TO SUMMARY DISPLAY
!----------
!     ** VARIABLES ASSOCIATED WITH LAURSEN'S TOTAL SHRUB COVER **
!
!-- ITUN, ITHABU(34) -- RETURN SUBSCRIPT, MAPPING FUNCTION FOR
!             UNDERSTORY HABITAT TYPE GROUPING
!-- TCON(2), TDTSD(4) -- CONSTANT FROM **CVSCON**, AND OTHER
!             VARIABLE COEFFICIENTS
!-- PGT0(MAXCY1,2) --   PROB (COVER > 0)
!-- TCOV --   PREDICTED COVER GIVEN PGT0
!----------
!  ** VARIABLES ASSOCIATED WITH SCHAROSCH'S PROBABILITY OF OCCURRENCE **
!
!-- IOV, IPHABO(34) -- RETURN SUBSCRIPT, MAPPING FUNCTION FOR
!             OVERSTORY HABITAT TYPE GROUPING
!-- IUN, IPHABU(34) -- RETURN SUBSCRIPT, MAPPING FUNCTION FOR
!             UNDERSTORY HABITAT TYPE GROUPING
!-- PCON(31), CSCAL, PDTSD(3,31), POTHER(4,31) -- CONSTANTS FROM
!             **CVSCON**, AND COEFFICIENTS FOR OTHER VARIABLES
!-- PB(31) -- PREDICTED PROBABILITY OF OCCURRENCE
!----------
!  ** VARIABLES ASSOCIATED WITH LAURSEN'S SPECIES HEIGHT **
!
!-- HCON(31), H11DTS(4), H16DTS(4), RESIDC --
!             CONSTANTS FROM **CVSCON**, AND COEFFICIENTS FOR
!             OTHER VARIABLES
!-- SH(31) -- PREDICTED SPECIES HEIGHT
!----------
!  ** VARIABLES ASSOCIATED WITH LAURSEN'S SPECIES COVER **
!
!-- CCON(31), ALNHT(31), RESIDH(31), -- CONSTANTS
!             FROM **CVSCON**, AND COEFFICIENTS FOR OTHER VARIABLES
!-- CV(31) -- PREDICTED SPECIES COVER, UNWEIGHTED BY PB(I)
!-- HTINDX(31), CABHT(31) -- SPECIES SUBSCRIPTS SORTED BY HEIGHT, AND
!             CUMULATIVE COVER ABOVE CURRENT HEIGHT
!-- PBCV(31) -- PREDICTED COVER WEIGHTED BY PROBABILITY OF
!             OCCURRENCE (PB(I)*CV(I)).  REPORTED AS SPECIES COVER
!-- TOTLCV(MAXCY1,2) -- REPORTED TOTAL COVER, EQUAL TO THE SUM OF
!             INDIVIDUAL SPECIES COVER WEIGHTED BY PROB. OF
!             OCCURRENCE (PBCV(I))
!----------
!     ** VARIABLES ASSOCIATED WITH IRWIN AND PEEK EQUATIONS **
!
!-- ICEHAB, IGFHAB -- HABITAT TYPE DUMMY VARIABLES
!-- SBMASS(MAXCY1,2) - PREDICTED SHRUB BIOMASS
!-- TWIGS(MAXCY1,2) -- PREDICTED TWIG PRODUCTION
!----------
!     ** VARIABLES ASSOCIATED WITH CALIBRATION **
!
!-- LCALIB -- .TRUE. IF SHRUB CALIBRATION FACTORS ARE STILL IN
!             EFFECT.  CALIBRATION STOPS AFTER THINNING
!-- SHRBHT(31), SHRBPC(31) -- INPUT SPECIES HEIGHT AND COVER
!-- BHTCF(31), BPCCF(31) -- CORRECTION FACTORS COMPUTED BY **CVBCAL**
!----------
!  ** CODES AND SUBSCRIPTS USED IN SHRUB MODELS **
!
!     N.FOREST             REGION 1   FOREST
!     GROUP                CODE       NAME
!     (INF)      (IFOR)
!     --------   ------    --------   ---------------
!        N/A       N/A          1     PAYETTE
...
!
!
!  OVERSTORY  UNDERSTORY  UNDERSTORY
!  SERIES     UNION FOR   UNION FOR
!             PB          TCOV,SH,CV
!  (IOV)      (IUN)       (ITUN)       (IHCODE)  CODE    HABITAT TYPE
!  ---------  ----------  ----------   --------  ----    ------------
!    1          5           6             1       210     DF / AGSP
!    1          5           6             2       220     DF / FEID
!    1          2           2             3       260     DF / PHMA
...
!    1          3           4             8       380     DF / SYOR
!    1          2           2             9       390     DF / ACGL
!    1          3           4            10       395     DF / BERE
!    2          3           4            11       505     GF / SPBE
!    2          1           1            12       510     GF / XETE
!    2          1           1            13       511     GF / COOC
!    2          2           2            14       515     GF / VAGL
!    2          1           1            15       520     GF / CLUN
!    2          2           2            16       525     GF / ACGL
!    3          1           3            17       530     WRC/ CLUN
!    3          1           3            18       540     WRC/ ATFI
!    3          1           3            19       550     WRC/ OPHO
!    4          4           5            20       570     WH / CLUN
!    2          1           1            21       590     GF / LIBO
!    5          4           5            22       620     AF / CLUN
!    5          1           1            23       635     AF / STAM
!    5          2           2            24       645     AF / ACGL
!    5          5           6            25       650     AF / CACA
!    5          2           2            26       670     AF / MEFE
!    5          1           1            27       690     AF / XETE
!    5          3           4            28       705     AF / SPBE
!    5          1           1            29       710     MH / XETE
!    5          2           2            30       720     AF / VAGL
!    5          2           2            31       721     AF / VAGL
!    5          2           1            32       730     AF / VASC
!    5          5           6            33       790     AF / CAGE
!    5          1           1            34       830     AF / LUHI
!
!  (IPHYS)   PYHSIOGRAPHY
!  -------   ------------
!     1      BOTTOM
...
!  ISP CODE    SPECIES
!  --- ---- ------------------------
!   1) ARUV:ARCTOSTAPHYLOS UVA-URSI
!   2) BERB:BERBERIS SPP.
!   3) LIBO:LINNAEA BOREALIS
!   4) PAMY:PACHISTIMA MYRSINITES
!   5) SPBE:SPIRAEA BETULIFOLIA
!   6) VASC:VACCINIUM SCOPARIUM
!   7) CARX:CAREX SPP.
!
!         MEDIUM SHRUBS
!  ---------------------------------
...
!  --- ---- ------------------------
!   8) LONI:LONICERA SPP.
!   9) MEFE:MENZIESIA FERRUGINEA
!  10) PHMA:PHYSOCARPUS MALVACEUS
!  11) RIBE:RIBES SPP.
!  12) ROSA:ROSA SPP.
!  13) RUPA:RUBUS PARVIFLORUS
!  14) SHCA:SHEPHERDIA CANADENSIS
!  15) SYMP:SYMPHORICARPOS SPP.
!  16) VAME:VACCINIUM MEMBRANACEUM
!  17) XETE:XEROPHYLLUM TENAX
!  18) FERN:FERNS
!  19) COMB:OTHER SHRUBS COMBINED
!
!         TALL SHRUBS
!  ---------------------------------
!  ISP CODE    SPECIES
!  --- ---- ------------------------
!  20) ACGL:ACER GLABRUM
!  21) ALSI:ALNUS SINUATA
!  22) AMAL:AMELANCHIER ALNIFOLIA
!  23) CESA:CEANOTHUS SANGUINEUS
!  24) CEVE:CEANOTHUS VELUTINUS
!  25) COST:CORNUS STOLONIFERA
!  26) HODI:HOLODISCUS DISCOLOR
!  27) PREM:PRUNUS EMARGINATA
!  28) PRVI:PRUNUS VIRGINIANA
!  29) SALX:SALIX SPP.
!  30) SAMB:SAMBUCUS SPP.
!  31) SORB:SORBUS SPP.
!
!-----------------------------------------------------------------------
!OMMONS
...
!----------------------------------------------------------------------
!  VARIABLE GROUPINGS FOR TOTAL SHRUB COVER, SPECIES HEIGHT AND COVER
DATA RESIDC /0.0/
DATA ITHABU /2*6,2,4,2*6,2*4,2,2*4,2*1,2,1,2,3*3, &
                5,1,5,1,2,6,2,1,4,1,2*2,1,6,1/
!                  TIME*NONE    TIME*MECH    TIME*BURN    ROAD
...
!----------
! 1/SAGE*DISTURBANCE TYPE COEFFICIENTS
! SP      NONE       MECH       BURN    ROAD
DATA PDTSD1 / &
...
!----------
!  SET NATIONAL FOREST GROUPING SUBSCRIPT.
!  IF THE MODEL IS BEING USED OUTSIDE THE AREA IT WAS CALIBRATED FOR,
!  SET TO THE BOISE/PAYETTE GROUP (FOR GROUPINGS, SEE PG 33, INT-190).
!----------
!
SELECT CASE (VARACD)
!
...
CASE('EM')
  INF=3
!
!  OTHER VARIANTS USE THE BOISE/PAYETTE GROUPING
!
CASE DEFAULT
  INF=1
!
...
IF (LCOV) GO TO 2
!----------
!  CALL THE OPTION PROCESSOR TO FIND WHETHER THE COVER KEYWORD
!  HAS BEEN BEEN SCHEDULED THIS CYCLE.
!----------
CALL OPFIND (1,MYACT(2),NTODO)
IF (NTODO .EQ. 0) GO TO 2
!----------
!  GET THE ACTIVITY, AND SIGNAL THAT IT IS DONE.
!----------
DO 1 I=1,NTODO
...
2 CONTINUE
!----------
!  RETURN IF THE SHRUBS KEYWORD HAS NOT BEEN SPECIFIED.
!----------
IF (.NOT. LBROW) RETURN
!----------
!  IF COVER NOT ACTIVE IN THE FIRST CYLE, DO THE SHRUB CALCULATIONS
!  ANYWAY TO COMPUTE CALIBRATION VALUES.
!----------
!
LSAGE = .FALSE.
!======================================================================
! *** START OF PROJECTION (CYCLE=0) ***
!----------
IF (.NOT. LSTART) GO TO 30
!----------
!  SET HABITAT TYPE TO STAND HABITAT TYPE IF NOT INPUT ON
!  SHRUBS KEYWORD.
!----------
IF (IHTYPE .EQ. 0) IHTYPE = ICL5
!------------------
!  CHECK FOR VALID HABITAT TYPE.
!  SET OVERSTORY UNION, UNDERSTORY UNION SUBSCRIPTS.
!------------------
DO 5 I=1,34
...
ITUN = ITHABU(ICODE)
!----------
!  SET HAB. TYPE DUMMY VARIABLES  FOR IRWIN AND PEEK RELATIONSHIPS, IF
!  APPROPRIATE:  520=GF/CLUN  530=WC/CLUN  570=WH/CLUN.
!----------
IGFHAB=0
ICEHAB=0
...
!----------
!  SET TIME SINCE DISTURBANCE:
!   1) TIME SINCE DISTURBANCE =  STAND AGE IF NOT READ FROM SHRUBS
!      KEYWORD.
!   2) IF SAGE AND IAGE BOTH ARE LESS THAN 3.0, SET SAGE = 3.0 YEARS,
!      AND FLAG WITH LSAGE.
!----------
...
20 CONTINUE
!======================================================================
!  CALL **CVSCON TO COMPUTE CONSTANTS WHICH HOLD FOR THE
!  DURATION OF THE PROJECTION.  **CVSCON** RETURNS PIECES OF
!  EQUATIONS OF THE FORM X'B FOR SPECIES PROBABILITY (PCON), TOTAL SHRUB
!  COVER (TCON), SPECIES HEIGHT (HCON), AND SPECIES COVER (CCON).
!----------
CALL CVSCON
30 CONTINUE
!======================================================================
!  *** START PROJECTION CYCLE CALCULATIONS ***
!----------
!  INCREMENT "TIME SINCE DISTURBANCE".
!
!  1) RESET SAGE TO IAGE IF RESETAGE HAS BEEN SCHEDULED THIS CYCLE.
!  2) IF THE SECOND CALL IN THE CYCLE (AFTER TREES HAVE BEEN UPDATED),
!     INCREMENT BY THE LENGTH OF THE CYCLE.
!----------
!  CALL THE OPTION PROCESSOR TO SEE IF RESETAGE HAS BEEN
!  SCHEDULED THIS CYCLE.
!----------
IF ((LTHIN).OR.(LSTART)) GO TO 35
CALL OPFIND (1,MYACT(1),NTODO)
!----------
!  IF NOT, SKIP TO STATEMENT 35.
!----------
IF (NTODO .EQ. 0) GO TO 35
!----------
!  SET TIME SINCE DISTURBANCE TO THE NEW STAND AGE.
!----------
DO 32 I=1,NTODO
CALL OPGET (I,0,KDT,IACT,NP,A)
32 CONTINUE
...
!----------
!  USE POST-THIN DENSITY STATISTICS IF THINNING HAS JUST OCCURRED.
!  CALL **CVCW** TO CALCULATE CRAREA FOR THINNING TEST.
!  IF THINNING HAS LOWERED CANOPY CLOSURE BELOW 50%, AND IF THE
!  VOLUME REMOVED IS AT LEAST 20% OF THE VOLUME BEFORE REMOVAL,
!  SET "TIME SINCE DISTURBANCE" BACK TO 3.0 YEARS, AND FLAG
!  THAT IT HAS BEEN RESET.
!  TURN OFF CALIBRATION FOLLOWING A THINNING.
!----------
ITHN = 1
...
!----------
! SAVE TIME SINCE DISTURBANCE FOR OUTPUT.
!----------
TIMESD(IP1,ITHN) = SAGE
!----------
!  DON'T COMPUTE SHRUBS IF TIME SINCE DISTURBANCE IS GREATER THAN
!  40 YEARS.
!----------
IF (.NOT. LCOV) GO TO 80
IF (SAGE .GT. 40.) RETURN
!----------------------------------------------------------------------
!  COMPUTE TOTAL COVER OF SHRUBS. (LAURSEN 1984)
!----------------------------------------------------------------------
!  1)  PROBABILITY THAT TOTAL SHRUB COVER ON THE PLOT EXCEEDS ZERO.
!        P(COV>0) = 1/(1+EXP(-XB))
!----------
PGT0(IP1,ITHN) = 1/(1 + EXP(-(TCON(1) + .0138*SAGE - &
                    0.00412*BA - .000213*BA*SAGE)))
!----------
!  2) TOTAL COVER GIVEN THAT COVER >0
!  (CORRECTED FOR BIAS BY ADDING .5*MSE=.3238953).
!  LN(TCOV) = A+BX
!----------
...
!----------------------------------------------------------------------
!  SHRUB SPECIES PROBABILITY OF OCCURRENCE.  (SCHAROSCH 1984)
!----------------------------------------------------------------------
DO 42 ISPI=1,31
...
42 CONTINUE
!----------------------------------------------------------------------
!  SHRUB SPECIES HEIGHT (FEET).   (LAURSEN 1984)
!  (LOG-LINEAR EQNS ARE CORRECTED FOR BIAS BY ADDING .5*MSE)
!----------------------------------------------------------------------
!  IF CALIBRATION BY HEIGHT LAYER IS BEING PERFORMED, COMPUTE
!  RESIDUAL COVER (OBSERVED - PREDICTED).  DO IN CYCLE 0, AND
!  CARRY THE VALUE THROUGH THE PROJECTION.
!----------
...
SH(31)= EXP(HCON(31)+.004520*TCOV+0.001510*RESIDC- &
           0.173325/SAGE+.08724)
!----------------------------------------------------------------------
!  SHRUB SPECIES PERCENT COVER   (LAURSEN 1984)
!  (LOG-LINEAR EQNS ARE CORRECTED FOR BIAS BY ADDING .5*MSE)
!----------------------------------------------------------------------
!  FIRST, COMPUTE LOG OF PREDICTED SPECIES HEIGHT, AND RESIDH(I)
!  (OBS - PRED) IF SHRUB HEIGHT CALIBRATION BY SPECIES IS BEING
!  PERFORMED. DO IN CYCLE 0, AND CARRY THROUGH THE PROJECTION.
!----------
TOTLCV(IP1,ITHN) = 0.0
DO 48 ISPI = 1,31
SHH = SH(ISPI)
...
48 CONTINUE
!----------
!  SORT THE SHRUBS BY PREDICTED HEIGHT TO DETERMINE PERCENT
!  COVER ABOVE THE CURRENT HEIGHT.
!----------
...
50 PBCV(K) = PB(K)*CV(K)
!----------
!  TOTAL COVER, REPORTED FOR ALL SPECIES, IS THE SUM OF INDIVIDUAL
!  SPECIES PREDICTED COVER TIMES PROBABILITY OF OCCURRENCE.
!----------
TOTLCV(IP1,ITHN) = TOTLCV(IP1,ITHN) + PBCV(K)
501 CONTINUE
!------------------
!  COMPUTE IRWIN AND PEEK STUFF IF APPROPRIATE HABITAT TYPE.
!------------------
IF ((IHTYPE .NE. 520).AND.(IHTYPE .NE. 530).AND. &
        (IHTYPE .NE. 570)) GO TO 62
!----------------------------------------------------------------------
!  SHRUB BIOMASS
!  SBMASS = BIOMASS IN LB/AC
! ( 1 KG/HA )( 2.2046226 LB/KG )( 1 HA/2.4710538 AC ) = 0.8921791 LB/AC
!----------------------------------------------------------------------
SBMASS(IP1,ITHN) = EXP(5.52+1.29*ALOG(SAGE)-0.1*SAGE-.01*RELDEN)
SBMASS(IP1,ITHN) = SBMASS(IP1,ITHN)*0.8921791
!----------------------------------------------------------------------
!  TWIG PRODUCTION  (TWIG/SQFT)     (10.76387 SQFT/SQM)
!----------------------------------------------------------------------
...
!----------
!  IF THIS IS START OF PROJECTION, AND CALIBRATION IS BEING
!  DONE, PASS SPECIES SUBSCRIPTS SORTED BY HEIGHT TO **CVBCAL**
!  TO COMPUTE THE SHRUB SCALING FACTORS.
!----------
IF (LSTART .AND. LCALIB) CALL CVBCAL(HTINDX)
!----------
!  APPLY SCALING FACTORS TO PREDICTIONS IF THEY ARE STILL IN EFFECT.
!----------
...
!----------
!  IF SAGE HAS BEEN SET TO 3.0 IN CYCLE 0, OR AFTER A THIN,
!  SUBTRACT 3.0 HERE, TO CORRESPOND TO NEXT CYCLE BOUNDARY.
!----------
80 CONTINUE
