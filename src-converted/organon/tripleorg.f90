...
SUBROUTINE XTRIP(K,J,M,ON,IWQ,WOODQ,IB,BIG6,POINT,TREENO, &
                    TDATAI,PRAGE,BRCNT,BRHT,BRDIA,JCORE,NPR,PRLH, &
                    PRDBH,PRHT,PRCR,PREXP,SCR,VOLTR,SYTVOL)
IMPLICIT NONE
INTEGER*4 K,J,M,ON,IWQ,IB,BIG6,POINT(2000),TREENO(2000), &
             TDATAI(2000,3),PRAGE(2000,3),BRCNT(2000,3), &
             BRHT(2000,40),BRDIA(2000,40),JCORE(2000,40), &
             NPR(2000),ISPGRP,I
REAL*4 PRLH(2000,3),PRDBH(2000,3),PRHT(2000,3),PRCR(2000,3), &
          PREXP(2000,3),SCR(2000,3),VOLTR(2000,4),SYTVOL(2000,2)
LOGICAL*2 WOODQ
ISPGRP=TDATAI(K,2)
!
!     DO TRIPLING OF VALUES OTHER THAN BASIC TREE ATTRIBUTES
!
IF(J .GT. 2000)THEN
...
DATA DGLRES/ &
               -0.440    , -0.4246576, -0.3934351, -0.4750919, &  !  DF,GW,PP,SP
               -0.3819283, -0.4629461, -0.3733441, -0.2690408, &  !  IC,WH,RC,PY
               -0.3155211, -0.3281635, -0.3407614, -0.2963305, &  !  MD,GC,TA,CL
               -0.3663556, -0.38681203,-0.2698731, -0.4947154, &  !  BL,WO,BO,RA
               -0.3679208, -0.3679208/                              !  PD,WI
!
DATA DGURES/ &
                0.487    ,  0.4822343,  0.4619852,  0.4977667, &  !  DF,GW,PP,SP
                0.4446445,  0.5300564,  0.4250894,  0.3367567, &  !  IC,WH,RC,PY
                0.3843792,  0.3975281,  0.4190495,  0.3788400, &  !  MD,GC,TA,CL
                0.4157338,  0.46098924, 0.3292178,  0.6171431, &  !  BL,WO,BO,RA
                0.4444550,  0.4444550/                              !  PD,WI
LRES=DGLRES(ISPGRP)
URES=DGURES(ISPGRP)
RETURN
END
...
DATA DGLRES/ &
                -0.440     , -0.60689712, -0.51832840, -0.3733441, &  !  DF,GF,WH,RC
                -0.2690408 , -0.3155211 , -0.3663556 , -0.38681203, &  !  PY,MD,BL,WO
                -0.4947154 , -0.3679208 , -0.3679208/               !  RA,PD,WI
DATA DGURES/ &
                0.487     , 0.66701064, 0.7452303 , 0.4250894, &  !  DF,GF,WH,RC
                0.3367567 , 0.3843792 , 0.4157338 , 0.46098924, &  !  PY,MD,BL,WO
                0.6171431 , 0.4444550 , 0.4444550/                  !  RA,PD,WI
LRES=DGLRES(ISPGRP)
URES=DGURES(ISPGRP)
RETURN
END
...
REAL*4 LRES,URES,DGLRES(11),DGURES(11)
DATA DGLRES/ &
                -0.440     , -0.60689712, -0.50104700, -0.3733441, &  !  DF,GF,WH,RC
                -0.2690408 , -0.3155211 , -0.3663556 , -0.38681203, &  !  PY,MD,BL,WO
                -0.4947154 , -0.3679208 , -0.3679208/               !  RA,PD,WI
DATA DGURES/ &
                0.487     , 0.66701064, 0.59381592, 0.4250894, &  !  DF,GF,WH,RC
                0.3367567 , 0.3843792 , 0.4157338 , 0.46098924, &  !  PY,MD,BL,WO
                0.6171431 , 0.4444550 , 0.4444550/                  !  RA,PD,WI
LRES=DGLRES(ISPGRP)
URES=DGURES(ISPGRP)
...
DATA DGLRES/ &
                -0.20610321, -0.1967740 , -0.22407503, -0.1669646, &  !  RA,DF,WH,RC
                -0.16383920, -0.16453918, -0.16453918/              !  BL,PD,WI
DATA DGURES/ &
                0.21278486, 0.2177930 , 0.26556256, 0.19010576, &  !  RA,DF,WH,RC
                0.18592181, 0.19876632, 0.19876632/                 !  BL,PD,WI
LRES=DGLRES(ISPGRP)
URES=DGURES(ISPGRP)
RETURN
END
...
!
DATA HGLRES &
              / -0.51303, -0.46026,  0.00000,  0.00000,  0.00000, &  !  DF,GW,PP,SP,IC
                -0.35755, -0.31335, -0.53151, -0.59648, -0.80800, &  !  DF,GW,PP,SP,IC
                 0.01357,  0.00000,  0.02492,  0.03244,  0.07429/   !  DF,GW,PP,SP,IC
!
DATA HGURES/ &
                 1.09084,  1.32726,  1.20195,  0.00000,  1.29032, &  !  DF,GW,PP,SP,IC
                 0.31540,  0.17331,  0.11159,  0.84916,  0.12008, &  !  DF,GW,PP,SP,IC
                -0.02196,  0.00000,  0.00000, -0.07683,  0.00000/   !  DF,GW,PP,SP,IC
!
A0=HGLRES(ISPGRP,1)
A1=HGLRES(ISPGRP,2)
A2=HGLRES(ISPGRP,3)
...
!
DATA HGLRES/ &
               0.0    ,  0.0    ,  0.0    , &  !  DF,GF,WH
              -0.4961 , -0.4156 , -0.35487, &  !  DF,GF,WH
               0.02173,  0.00997,  0.0    /                         !  DF,GF,WH
DATA HGURES/ &
               0.4956 ,  0.0   ,  0.0      , &  !  DF,GF,WH
               0.5758 ,  0.9094,  0.2682203, &  !  DF,GF,WH
              -0.03378, -0.0554,  0.0       /                       !  DF,GF,WH
!
A0=HGLRES(ISPGRP,1)
A1=HGLRES(ISPGRP,2)
A2=HGLRES(ISPGRP,3)
...
DATA HGLRES/ &
               0.0    ,  0.0    ,  0.0    , &  !  DF,GF,WH
              -0.4961 , -0.4156 , -0.4392 , &  !  DF,GF,WH
               0.02173,  0.00997,  0.02751/                         !  DF,GF,WH
DATA HGURES/ &
               0.4956 ,  0.0   ,  0.4388, &  !  DF,GF,WH
               0.5758 ,  0.9094,  0.5098, &  !  DF,GF,WH
              -0.03378, -0.0554, -0.02991/                          !  DF,GF,WH
!
A0=HGLRES(ISPGRP,1)
A1=HGLRES(ISPGRP,2)
A2=HGLRES(ISPGRP,3)
...
!
DATA HGLRES/ &
              -0.565597958    ,  0.0    ,  0.0    , &  !  RA,DF,WH
              -0.259956282    , -0.4961 , -0.4392 , &  !  RA,DF,WH
               0.0276968137   ,  0.02173,  0.02751/                         !  RA,DF,WH
DATA HGURES/ &
               0.866609308    ,  0.4956 ,  0.4388, &  !  RA,DF,WH
               0.100265352    ,  0.5758 ,  0.5098, &  !  RA,DF,WH
              -0.0135706039   , -0.03378, -0.02991/                         !  RA,DF,WH
!
A0=HGLRES(ISPGRP,1)
A1=HGLRES(ISPGRP,2)
A2=HGLRES(ISPGRP,3)
