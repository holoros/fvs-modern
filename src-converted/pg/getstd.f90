...
!----------
!
!     READS A STAND FROM MASS STORAGE, AND THE ALL
!     DATA CHARACTER FILE, INTO MEMORY.
!
!
!OMMONS
!
...
!
!
!     READ ALL INTEGER VARIABLES WITH IFREAD, ALL REAL VARIABLES WITH
!     BFREAD, AND ALL LOGICAL VARIABLES WITH LFREAD.  ONE EXCEPTION IS
!     IOSUM, WHICH MUST BE LONG INTEGER, AND IS READ USING
!     BFREAD VIA AN EQUIVALENCE TO ROSUM.  THE OTHER TWO EXCEPTIONS
!     ARE THE RANDOM NUMBER SEEDS, WHICH ARE EQUIVALENCED TO REAL
!     ARRAYS OF LENGTH 2.
!
...
!     IT IS NECESSARY TO GET THE INTEGER SCALARS FIRST IN ORDER TO SET UP
!     LIMITS FOR THE REST OF THE VARIABLE LENGTH READS.
!
...
13 CONTINUE
14 CONTINUE
CALL IFREAD (WK3,IPNT,ILIMIT,DECAYCD, ITRN, 2)
CALL IFREAD (WK3,IPNT,ILIMIT,WDLDSTEM,ITRN, 2)
!
...
CALL BFREAD (WK3,IPNT,ILIMIT,ABVGRD_BIO,ITRN,   2)
CALL BFREAD (WK3,IPNT,ILIMIT,ABVGRD_CARB,ITRN,  2)
...
CALL BFREAD (WK3,IPNT,ILIMIT,CARB_FRAC, ITRN,   2)
...
CALL BFREAD (WK3,IPNT,ILIMIT,CUBSAW_BIO,ITRN,   2)
CALL BFREAD (WK3,IPNT,ILIMIT,CUBSAW_CARB,ITRN,  2)
CALL BFREAD (WK3,IPNT,ILIMIT,CULL,    ITRN,     2)
...
CALL BFREAD (WK3,IPNT,ILIMIT,FOLI_BIO,ITRN,     2)
CALL BFREAD (WK3,IPNT,ILIMIT,FOLI_CARB,ITRN,    2)
...
17 CONTINUE
CALL BFREAD (WK3,IPNT,ILIMIT,MCFV,   ITRN,      2)
CALL BFREAD (WK3,IPNT,ILIMIT,MERCH_BIO,ITRN,    2)
CALL BFREAD (WK3,IPNT,ILIMIT,MERCH_CARB,ITRN,   2)
...
CALL BFREAD (WK3,IPNT,ILIMIT,OSCCUR, 7,         2)
CALL BFREAD (WK3,IPNT,ILIMIT,OSCREM, 7,         2)
...
CALL BFREAD (WK3,IPNT,ILIMIT,OSPSC,  4,         2)
CALL BFREAD (WK3,IPNT,ILIMIT,OSPSR,  4,         2)
...
CALL BFREAD (WK3,IPNT,ILIMIT,PSCFV,  ITRN,      2)
...
CALL BFREAD (WK3,IPNT,ILIMIT,SCFMIND,MAXSP,     2)
CALL BFREAD (WK3,IPNT,ILIMIT,SCFSTMP,MAXSP,     2)
CALL BFREAD (WK3,IPNT,ILIMIT,SCFTOPD,MAXSP,     2)
CALL BFREAD (WK3,IPNT,ILIMIT,SCFV,   ITRN,      2)
...
CALL BFREAD (WK3,IPNT,ILIMIT,TPAAINV,IPTINV,    2)
CALL BFREAD (WK3,IPNT,ILIMIT,TSTV1,  MXTST1,    2)
CALL BFREAD (WK3,IPNT,ILIMIT,TSTV2,  MXTST2,    2)
CALL BFREAD (WK3,IPNT,ILIMIT,TSTV3,  MXTST3,    2)
...
!
!     GET THE COVER VARIABLES...IF THE COVER MODEL IS BEING USED
!     IN THIS STAND.
!
IF (LCVGO) CALL CVGET (WK3,IPNT,ILIMIT)

!
!     GET THE MISTLETOE DATA...IF THE MISTLETOE MODEL LINKED TO
...
!
!     GET THE ROOT DISEASE & STEM RUST DATA...IF THE QUICK RR MODEL
!     IS LINKED TO THE SYSTEM (WHICH IT IS FOR THE WESTWIDE PINE
!     BEETLE MODEL).
!
!     CALL RRATV (LRR1,LRR2)
!     IF(LRR1) CALL RRPPGT (WK3,IPNT,ILIMIT)
!
!     GET THE WESTERN ROOT DISEASE MODEL INFORMATION
!
IF(LWRD) CALL RDPPGT (WK3,IPNT,ILIMIT)

!
!     GET THE WESTWIDE PINE BEETLE MODEL STAND-LEVEL DATA THAT
!     IS NEEDED.
!
!     CALL BMPPGT (IPNT,ILIMIT)
!
!     GET THE GENDEFOL/BUDWORM MODEL NUMERIC VARIABLES, IF ACTIVE.
!
!     CALL BWEPPATV (LBWE)
!     IF (LBWE) CALL BWEPPGT (WK3,IPNT,ILIMIT,1)
!
!     GET THE FIRE MODEL VARIABLES
!
IF (LFM) CALL FMPPGET (WK3,IPNT,ILIMIT)

!
!     GET THE ECONOMIC MODEL VARIABLES
!
...
!
!     GET THE CLIMATE-FVS VARIABLES
!
CALL CLGET (WK3,IPNT,ILIMIT)

!
!     GET THE LAST REAL VARIABLE.
!
CALL BFREAD (WK3,IPNT,ILIMIT,XSTORE, MAXPLT,    3)

!
!     END OF NUMERIC AND LOGICAL READ.  READ IN CHARACTERS.
!
CALL CHGET
!
