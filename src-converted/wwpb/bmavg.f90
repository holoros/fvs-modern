...
!     Loop through all the stands in the beetle model, finding the
!     required weighted sums.  Sums are required for each variable which
!     has a non-zero coefficient in the drought model (e.g. the weighted
!     sum of HIGH values across stands is needed if KHIGH <> 0).  Do not
!     include non-stockable stands in the averages.
...
!       find the sum of HIGH
  IF (KHIGH .NE. 0.0) THIGH = THIGH + SAREA*HIGH(SINDX)
!       find the sum of WET
!
!     Renamed WET to WTSC to use data read in WETSCORE (RNH 1july98)
!
!        IF (KWET .NE. 0.0) TWET = TWET + SAREA*WET(SINDX)
  IF (KWET .NE. 0.0) TWET = TWET + SAREA*WTSC(SINDX)
!
!
!       calculate SCDBH and find AVDBH
...
!           find the average DBH of trees in the size class from their
!           average basal area (basal area = pi * (dbh/2)^2), and convert
...
!       calculate BMSDI and find the sum of SDI
  IF (KSDI .EQ. 0.0) GOTO 400

!         find the total density of trees in the stand, and total basal area
!         (which will already be available if AVDBH has been calculated)
    TOTDEN = 0.0
    BA = 0.0
    DO 300 ISIZ = 1,NSCL
...
!         find ASD from average basal area = pi * (ASD/2)^2.  This gives ASD
!         in square feet - convert it to inches.
    ASD = 12.0 * SQRT(1.273239545 * TBA(SINDX) / TOTDEN)

!         find SDI.  This equation is the one used by ANCSD.FOR in the
!         Annosus Bark Beetle model, and has been checked against the SDI
!         graph in Forest Mensuration (Husch et al.).  TOTDEN in this equation
!         is in trees/acre and ASD is in inches.
    SLOPE = -1.605
...
!     Find the required averages (AVDBH is already done).
...
!     Loop through all the stands in the beetle model, finding the
!     required weighted sums of squared deviations.  Again, skip any
!     non-stockable stands.
THIGH = 0.0
TSDI = 0.0
TWET = 0.0
...
!       find the sum of the HIGH deviations
  IF (KHIGH .NE. 0.0) THIGH = THIGH +SAREA*(AVHIGH-HIGH(SINDX))**2
!       find the sum of the WET deviations
!
!     Renamed WET to WTSC to use data read in WETSCORE (RNH 1july98)
!
  IF (KWET .NE. 0.0) TWET = TWET + SAREA*(AVWET-WTSC(SINDX))**2
!        IF (KWET .NE. 0.0) TWET = TWET + SAREA*(AVWET-WET(SINDX))**2
!       find the sum of the SDI deviations
...
!     Find the required standard deviations (SDDBH is already done).
