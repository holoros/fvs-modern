...
!----------
!
!     STASH A STAND TO THE MASS STORAGE
!
!   Local variable definitions:
!     MXI      Maximum number of integer scalars to be written.
!     MXL      Maximum number of logical scalars to be written.
!     MXR      Maximum number of real scalars to be written.
!
!OMMONS
!
!
...
!     WITH LFWRIT, AND REAL VARIABLES WITH BFWRIT.  ONE EXCEPTION
!     IS IOSUM, WHICH MUST BE LONG INTEGER, AND IS WRITTEN WITH
!     BFWRIT VIA AN EQUIVALENCE TO ROSUM.  THE OTHER EXCEPTIONS ARE
!     THE RANDOM NUMBER SEEDS, WHICH ARE EQUIVALENCED TO REAL ARRAYS
!     OF LENGTH 2.
!
INTEGER MXL,MXR,MXI,IRECLN
PARAMETER (MXR=137,MXL=40,MXI=120,IRECLN=1024)
INTEGER ILIMIT,IPNT,K,I,II
INTEGER INTS(MXI)
!      LOGICAL LBWE,LRR1,LRR2
...
INTS(120) =   ISTDORG
!
!     BEGIN THE WRITE TO THE DIRECT ACCESS DATA FILE, AND WRITE THE
!     INTEGER SCALARS.
!
...
13 CONTINUE
14 CONTINUE
CALL IFWRIT (WK3,IPNT,ILIMIT,DECAYCD, ITRN, 2)
CALL IFWRIT (WK3,IPNT,ILIMIT,WDLDSTEM,ITRN, 2)
!
!     STORE THE LOGICAL SCALARS IN THE ARRAY LOGICS.
!
LOGICS ( 1) = LAUTAL
LOGICS ( 2) = LAUTON
LOGICS ( 3) = LBKDEN
...
CALL BFWRIT (WK3,IPNT,ILIMIT,ABVGRD_BIO,ITRN,   2)
CALL BFWRIT (WK3,IPNT,ILIMIT,ABVGRD_CARB,ITRN,  2)
...
CALL BFWRIT (WK3,IPNT,ILIMIT,CARB_FRAC, ITRN,   2)
...
CALL BFWRIT (WK3,IPNT,ILIMIT,CUBSAW_BIO,ITRN,   2)
CALL BFWRIT (WK3,IPNT,ILIMIT,CUBSAW_CARB,ITRN,  2)
CALL BFWRIT (WK3,IPNT,ILIMIT,CULL,    ITRN,     2)
...
CALL BFWRIT (WK3,IPNT,ILIMIT,FOLI_BIO,ITRN,     2)
CALL BFWRIT (WK3,IPNT,ILIMIT,FOLI_CARB,ITRN,    2)
...
17 CONTINUE
CALL BFWRIT (WK3,IPNT,ILIMIT,MCFV,   ITRN,      2)
CALL BFWRIT (WK3,IPNT,ILIMIT,MERCH_BIO,ITRN,    2)
CALL BFWRIT (WK3,IPNT,ILIMIT,MERCH_CARB,ITRN,   2)
...
CALL BFWRIT (WK3,IPNT,ILIMIT,OSCCUR, 7,         2)
CALL BFWRIT (WK3,IPNT,ILIMIT,OSCREM, 7,         2)
...
CALL BFWRIT (WK3,IPNT,ILIMIT,OSPSC,  4,         2)
CALL BFWRIT (WK3,IPNT,ILIMIT,OSPSR,  4,         2)
...
CALL BFWRIT (WK3,IPNT,ILIMIT,PSCFV,  ITRN,      2)
...
CALL BFWRIT (WK3,IPNT,ILIMIT,SCFMIND,MAXSP,     2)
CALL BFWRIT (WK3,IPNT,ILIMIT,SCFSTMP,MAXSP,     2)
CALL BFWRIT (WK3,IPNT,ILIMIT,SCFTOPD,MAXSP,     2)
CALL BFWRIT (WK3,IPNT,ILIMIT,SCFV,    ITRN,     2)
...
CALL BFWRIT (WK3,IPNT,ILIMIT,TPAAINV,IPTINV,    2)
CALL BFWRIT (WK3,IPNT,ILIMIT,TSTV1,  MXTST1,    2)
CALL BFWRIT (WK3,IPNT,ILIMIT,TSTV2,  MXTST2,    2)
CALL BFWRIT (WK3,IPNT,ILIMIT,TSTV3,  MXTST3,    2)
...
!
!     WRITE THE COVER VARIABLES...IF THE COVER MODEL IS BEING USED
!     IN THIS STAND.
!
IF (LCVGO) CALL CVPUT (WK3,IPNT,ILIMIT,ICYC,ITRN)

!
!     WRITE THE MISTLETOE DATA...IF THE MISTLETOE MODEL LINKED TO
!     TO THE SYSTEM (WHICH IS GENERALLY TRUE FOR MOST VARIANTS).
!
CALL MISACT (LMORED)
IF (LMORED) CALL MSPPPT (WK3,IPNT,ILIMIT)

!
!     WRITE THE ROOT DISEASE & STEM RUST DATA...IF THE QUICK RR MODEL
!     IS LINKED TO TO THE SYSTEM (WHICH IT IS FOR THE WESTWIDE PINE
!     BEETLE MODEL).
!
!      CALL RRATV (LRR1, LRR2)
!      IF (LRR1) CALL RRPPPT (WK3,IPNT,ILIMIT)
!
!     WRITE THE WESTERN ROOT DISEASE MODEL INFORMATION
!
IF (LWRD) CALL RDPPPT (WK3,IPNT,ILIMIT)

!
!     WRITE THE WESTWIDE PINE BEETLE MODEL STAND-LEVEL DATA THAT
!     IS NEEDED.
!
!      CALL BMPPPT (IPNT,ILIMIT)
!
!     WRITE THE GENDEFOL/BUDWORM MODEL NUMERIC VARIABLES, IF ACTIVE.
!
!     CALL BWEPPATV (LBWE)
!     IF (LBWE) CALL BWEPPPT (WK3,IPNT,ILIMIT,1)
!
!     WRITE THE FIRE MODEL VARIABLES
!
IF (LFM) CALL FMPPPUT (WK3,IPNT,ILIMIT)

!
!     WRITE THE ECONOMIC MODEL VARIABLES
!
CALL ECNPUT (WK3,IPNT,ILIMIT)

!
!     WRITE THE DATABASE VARIABLES
!
CALL DBSPPPUT (WK3,IPNT,ILIMIT)

!
!     WRITE THE CLIMATE-FVS VARIABLES
!
CALL CLPUT (WK3,IPNT,ILIMIT)

!
!     WRITE THE LAST REAL VARIABLE.
!
CALL BFWRIT (WK3,IPNT,ILIMIT,XSTORE, MAXPLT,    3)

!
!     CALL CHPUT TO STORE THE CHARACTER DATA
!
CALL CHPUT
!
RETURN
