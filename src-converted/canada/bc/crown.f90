...
REAL     PCR,DCR,XCR,BACHLO !,CRLN,CRMAX

!     THESE USE NI-OTHER (11) FOR 12,13,15, AND NI-FD (3) FOR 14
DATA PARM/ &
...
!  DATA CRSD/6.35/ ! assigned by species below

DATA MYACTS/81/
!
!       ZONE COEFFICIENTS FOR ICH
!
DATA CRKONST(1) / &
     CR_STR ( &
       'ICH ', &
...
!----------
!  BRANCH TO STATEMENT 58 TO HANDLE TREES WITH DBH LESS THAN
!  3IN (V2) OR 2CM (V3)
!----------
    IF (LV2ATV) THEN
...
!  COMMENTED OUT: ADD RANDOM ERROR ON FIRST USE ONLY
!----------
    XRAN = 0.0
    IF (LV2ATV) THEN
...
!----------
50     CONTINUE
    IF(DEBUG) WRITE(JOSTND,9802) I,ICR(I),EXPPCR,EXPDCR,D,H, &
         PCT(I),OLDPCT(I)
...
!----------
    CHG=EXPPCR-EXPDCR
    ICRI = ICR(I) + INT((EXPPCR-EXPDCR)*100.0+0.5)
...
!----------
  ...
  XCR=ICRI
  ICRI=INT(BACHLO(XCR,CRSD(ISPC),RANN))
51   CONTINUE &
   ------ &
   MPUTE THE CHANGE IN CROWN RATIO &
   LC THE DIFFERENCE BETWEEN THE MODEL AND THE OLD(OBS) &
   IS SECTION ADDED FOR SPECIES EXPANSION &
   UND CROWN CHANGE TO 1% PER YEAR &
   ------
 CONTINUE
 ...
 IF(.NOT.LSTART.OR.(ICR(I).GT.0))THEN
   ...
   FORMAT(/'  IN CROWN 9020 I,CRNEW,ICR,PDIFPY,CHG =',
&  I5,F10.3,I5,3F10.3)
   IF(DBH(I).GE.DLOW(ISPC).AND.DBH(I).LT.DHI(ISPC))THEN
...
&  ICRI = INT(REAL(ICRI)*CRNMLT(ISPC))
 ENDIF &
   ----- &
   C CROWN LENGTH NOW FOR CR, UT, TT VARIANTS ??? &
   -----
 IF(LSTART .OR. ICR(I).EQ.0)GO TO 55
 CRLN=HT(I)*REAL(ICR(I))/100. &
   ----- &
   C CROWN LENGTH MAX POSSIBLE IF ALL HTG GOES TO NEW CROWN &
   -----
 CRMAX=(CRLN+HTG(I))/(HT(I)+HTG(I))*100.0
 IF(DEBUG)WRITE(JOSTND,9004)CRMAX,CRLN,ICRI,I,CRNEW(I),CHG &
    FORMAT(/' CRMAX=',F10.2,' CRLN=',F10.2,
&' ICRI=',I10,' I=',I5,' CRNEW=',F10.2,' CHG=',F10.3) &
   -----
NEW CROWN EXCEEDS MAX POSSIBLE LIMIT IT TO MAX POSSIBLE &
   -----
...
GO TO 59
!----------
!  CROWNS FOR TREES WITH DBH <3IN (V2) OR <2CM (V3) ARE DUBBED HERE.  NO CHANGE
!  IS CALCULATED UNTIL THE TREE ATTAINS THESE LIMITS
!----------
58 CONTINUE
...
!---------
!  ADDED FOR CRNMLT ADJUSTMENT OF DUBBED VALUES
!---------
IF(DBH(I).GE.DLOW(ISPC) .AND. DBH(I).LT.DHI(ISPC)) &
     ICRI = INT(REAL(ICRI) * CRNMLT(ISPC))
!----------
...
!  SITE DEPENDENT AND REQUIRE ONE TIME RESOLUTION.
!
!  V2: ITYPE INDEXES HABITAT TYPE (CARRIED IN /PLOT/ COMMON AREA), CRHAB
!  CONTAINS HABITAT INTERCEPTS BY HABITAT TYPE BY SPECIES, AND MAPCR MAPS
!  HABITAT TYPE ONTO HABITAT CLASS FOR EACH SPECIES.
!
!  V3: LOCATE ZONE; DEFAULTS TO ICH (IZN=1) IF NO MATCH
...
IF (LV2ATV) THEN
  DO I = 1,MAXSP
    ...
    CRSD(I) = 6.35 ! copied from GED comment
  ENDDO
ELSE
  IZN = 0
!           MAP THE SBPS ZONE AND ONE SBS SUBZONE ONTO IDF
!               AND THE REST OF THE SBS ONTO ICH
...
!       ZERO ALL COEFFS THEN ASSIGN SITE EFFECTS TO CRCON AND ALL
!       OTHER COEFFS.
...
!         FOR SOME SPECIES IN THE SBS, WE NEED TO CHANGE THE ZONE
!           FOR THE PARAMETERS TO BE USED.
    OLDZN = IZN
...
!     PRIVATE SUBROUTINE TO CALCULATE CROWN PROPORTION
...
!     ADJUST HT, DBH TO APPROXIMATE A 2 CM TREE WHEN DBH<2
...
!     BOUND YCR TO 1E-4 AND 1E+4 AFTER EXP()
