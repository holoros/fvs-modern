SUBROUTINE SMHGDG (IT,ISPC,H,D,HG5,DG5,JCYC,JSTND2,DEBUG,MODE)
IMPLICIT NONE
!----------
! VWC $Id$
!----------
!  THIS ROUTINE CALCULATES THE HEIGHT GROWTH AND DIAMETER GROWTH
!  OF SMALL TREES (D<3.0 IN). BASED ON WORK BY PETER GOULD AND
!  CONNIE HARRINGTON, INITIAL MODELS FOR SMALL-TREE DIAMETER
!  GROWTH (OCTOBER 31, 2011)
!  CALLED FROM **ESSUBH **REGENT
!
!  INPUT VARIABLES
!  IT     - TREE NUMBER
!  ISPC   - SPECIES
...
!
!  RETURN VARIABLES
!  HG5    - 5 YEAR HEIGHT INCREMANT
!  DG5    - 5 YEAR DIAMETER INCREMENT
!OMMONS
!
INCLUDE 'PRGPRM.f90'
!
...
!
LOGICAL DEBUG
INTEGER I,IT,J,JCYC,JSTND2,MODE,ISPC
REAL    H,DGS,HBH,DBHA,D,BAS,AVHT
REAL    DMAX(MAXSP),ALPHA(0:10,MAXSP),BETA(MAXSP),HG5,DG5
REAL    PTBA,PTBA2,PTBAL,PTBAL2,BOOST,CR,RELHT,RELHT2,SI
!----------
!     VARIABLES ADDED DURING RW UPDATES 2021
!----------
...
DATA ((ALPHA(I,J), I=0,9),J=39,39) / &
    ...
    0.00000,-0.18790,-0.01100/
!----------
!  CALCULATE 5 YEAR HEIGHT AND DIAMETER GROWTH
!----------

IF(DEBUG)THEN
  WRITE(JSTND2,*)' ENTERING SMHGDG-IT,H,D,', &
     'AVH,FINT= ',IT,H,D,AVH,FINT
  IF(IT.GT.0)WRITE(JSTND2,*)' ICR(IT),PCT(IT),PTBAA(ITRE(IT))= ', &
      ICR(IT),PCT(IT),PTBAA(ITRE(IT))
  WRITE(JSTND2,*)' IT,ITRE,PTBAA= ',IT,ITRE(IT),PTBAA(ITRE(IT))
ENDIF

!----------
!  MOVED SITE INDEX REFERENCE TO TOP TO DO RW CALCS FIRST AND
!  BYPASS REMAINING CODE FOR PROCESSING EFFICIENCY
!  RW EDITS ONLY CALCULATE HEIGHT GROWTH
!    RW DIA GROWTH FOR SMALL TREES HANDLED BY REGENT.F
!----------
SI= SITEAR(ISPC)
!
IF (ISPC .EQ. 17) THEN
...
!  IF DOUGLAS-FIR IN WC TRANSLATE SI
!  CURTIS TO KING FOR THIS EQN
!----------

IF ((ISPC.EQ.16).AND.(VARACD.EQ.'WC')) THEN
...
!
IF (DEBUG)THEN
  WRITE(JSTND2,*)'BETA(ISPC),DMAXISPC),(ALPHA(I,ISPC),I=0,6)= ', &
     BETA(ISPC),DMAX(ISPC),(ALPHA(I,ISPC),I=0,6)
  WRITE(JSTND2,*)' LEAVING SMHGDG, JCYC,IT,D,DGS,DG5,', &
     'HG5,DBHA,HBH,ISPC= ',JCYC,IT,D,DGS,DG5,HG5,DBHA,HBH,ISPC
  WRITE(JSTND2,*)' H,PTBAL,CR,BAS,PTBA,RELHT,AVH,SI= ', &
