...
IMPLICIT NONE
!----------
! VBASE $Id: twigbf.f 2450 2018-07-11 17:28:41Z gedixon $
!----------
!  CENTRAL STATES ACCEPTABLE TREE CLASS BD FT VOLUME EQUATION:
!  --SMITH, W. BRAD, C.A. WEIST. 1982. A NET VOLUME EQUATION
!    FOR INDIANA. RESOUR. BULL. NC-63. ST PAUL, MN: USDA, FOREST
!    SERVICE, NORTH CENTRAL FOREST EXPERIMENT STATION. 7 P.
!
!          V = B1*SI**B2*[1-EXP(-B3*DBH**B4)]
!
!  WHERE:
!        V = VOLUME
!       SI = SITE INDEX
!      DBH = DIAMETER BREAST HEIGHT
!    B1-B4 = SPECIES SPECIFIC COEFFICIENTS
!
!----------
!  LAKE STATES ACCEPTABLE TREE CLASS VOLUME EQUATION:
!  --RAILLE, GERHARD K., W.B. SMITH, C.A. WEIST. 1982. A NET VOLUME
...
!  --SCOTT, CHARLES, T. 1979. NORTHEASTERN FOREST SURVEY BOARD-FOOT
!    VOLUME EQUATIONS. USDA FOR. SERV RES. NOTE NE-271.
!
!  BFVOL=B1+BS(DBH)^B3+(B4(DBH)^B5)*H^B6
!
!  WHERE H = HEIGHT (IN FEET) TO TOP DIAMETER OUTSIDE BARK (TDOB)
!   TDOB FOR BFVOL = 9 INCHES (HARDWOODS) AND 7 INCHES (SOFTWOODS).
!
!  MERCHANTABLE HEIGHT EQUATIONS:
!  --YAUSSY, D.A. AND M.E. DALE. 1990. SAWLOG AND BOLE LENGTH
!    GENERALIZED MERCHANTABLE HEIGHT EQUATIONS FOR THE NORTHEASTERN
!    UNITED STATES.  USDA FOR. SERV. RES. PAP. NE-??? (IN PRESS).
!
!  HEIGHT=C1*SI^C2*(1-EXP(C3*(DBH-TD)))
!
!------------------------------------------------------------------
!
!OMMONS
...
!  BUT CANNOT BE LESS THAN 2 INCHES.  DEFAULTS ARE 7 IN. FOR SOFTWOODS
!  AND 9 IN. FOR HARDWOODS.  MINIMUM MERCHANTABLE DBH IS
!  SET WITH THE BFVOLUME KEYWORD.
!
!  MERCHANTABLE TOP DIAMETER CAN BE SET WITH THE BFVOLUME KEYWORD
!  AT ANY VALUE BETWEEN 2 IN. AND ACTUAL DBH.  THIS DIAMETER IS
!  OUTSIDE BARK -- IF DIB IS DESIRED, ALLOWANCE FOR DOUBLE BARK
!  THICKNESS MUST BE MADE IN SPECIFICATION OF TOP DIAMETER.
!
!----------
...
!  COEFFICIENTS FOR NET BOARD FOOT VOLUME EQUATION FOR ACCEPTABLE
!  TREES (TREE CLASS 1 & 2).
!----------
DATA CSB1/ &
...
CASE ('CS')
!
!  COMPUTE TOTAL BOARD FOOT VOLUME (VMAX)
!
  VMAX = CSB1(ISPC)*SITEAR(ISPC)**CSB2(ISPC)* &
            (1.0 - EXP(-1.0*CSB3(ISPC)*(D**CSB4(ISPC))))
!
!  COMPUTE MERCH BOARD FOOT VOLUME (BBFV)
!
  BBFV = VMAX
!----------
!  LAKE STATES VARIANT
...
!            HEIGHT (ONE COEFFICIENT PER SPECIES).
!  SHB2   -- COEFFICIENT FOR SITE INDEX TERM IN THE MODEL PREDICTING
!            SAWLOG HEIGHT (ONE COEFFICIENT PER SPECIES).
!  SHB3   -- COEFFICIENT FOR (DBH-TDOB) TERM IN THE MODEL PREDICTING
!            SAWLOG HEIGHT (ONE COEFFICIENT PER SPECIES).
!  NEBF1  -- B1 COEFFICIENT IN THE NORTHEASTERN FOREST SURVEY
!            BOARD FOOT VOLUME EQUATION.
!  NEBF2  -- B2 COEFFICIENT IN THE NORTHEASTERN FOREST SURVEY BOARD-
!            FOOT VOLUME EQUATION.
!  NEBF3  -- B3 COEFFICIENT IN THE NORTHEASTERN FOREST SURVEY BOARD-
!            FOOT VOLUME EQUATION.
!  NEBF4  -- B4 COEFFICIENT IN THE NORTHEASTERN FOREST SURVEY BOARD-
!            FOOT VOLUME EQUATION.
!  NEBF5  -- B5 COEFFICIENT IN THE NORTHEASTERN FOREST SURVEY BOARD-
!            FOOT VOLUME EQUATION
!  NEBF6  -- B6 COEFFICIENT IN THE NORTHEASTERN FOREST SURVEY BOARD-
!            FOOT VOLUME EQUATION
!----------
!
