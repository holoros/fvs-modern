...
!  FILE CONTAINS THE FOLLOWING ROUTINES
!        SHP_W3
!        SHP_W4
!        SHP_W5
!        COR_WS
...
!***********************************************************************
!                      Coefficients for NW TAPER Coop  April, 1994
!                         (REGN  CD17.coe)
...
!              common SF_A provides DBHOB(ob), Total height and DBHOB(ib)
!                                                                Median
...
!             Region 8 (Washington West coast) uses Region 6 (WA. West) coef.
...
!                               Region modifications to coefficients
  FR25=f(25)
  FR34=f(34)
...
!     PUT COEFFICIENTS INTO AN ARRAY FOR EASIER PASSING
  RFLW(1) = R1
  RFLW(2) = R2
  RFLW(3) = R3
  RFLW(4) = R4
...
!             Region 2 (Oregon West Valley) uses Region 3 (Or. EV) coef.
...
!              common SF_A provides DBHOB(ob), Total height and D=DBHOB(ib)
...
!               NW Taper Coop  Western Hemlock (WH41194.coe)
!                      Copyright, 1994  James W. Flewelling
  REAL COR_WS
  INTEGER JSP,JSPR
  REAL BH,HTTOT,HI,HJ, CORR,H1,H2,T3,T4,T2,T1
  REAL QH(3,5),Q1,Q2,Q3,Q4,Q5

!     DATA F80,F81/ 2.013, .4345/
!       Douglas fir
...
!       western red cedar
  ...
  BH = 4.5
!                          any error is completely correlated with itself
  if(hi.eq.hj ) then
       cor_ws=1.0
       return
    endif
!                               There are no errors at breast height (Ha!)
...
!                             define heights h1 and h2 with h2 > h1
  h1=min(hi, hj)
  h2=max(hi,hj)


  if(h1.gt. bh) then
!                                         both heights are above BH
   t3=(h1-bh)/(HTTOT-bh)
   t4=(h2-bh)/(HTTOT-bh)
...
!********************************************************************
!                                                   Find Z, given d.
!        Douglas Fir,  NW Taper Coop   April, 1994  (CD17.coe)
!        Copyright    James W. Flewelling, 1994
!
!      CHANGES_____________________________________________________________
!      23 JUL 1997  Mixed-mode (MU + .0005) arithmetic corrected.
...
!********************************************************************
!                                                   Find Z, given d.
!        Western Hemlock,  NW Taper Coop   April, 1994  (WH41194.coe.coe)
!        Copyright    James W. Flewelling, 1994

!      CHANGES___________________________________________
!      22JUL 1997              Move test of X vs MU.
!                              Note: old postion could have caused a problem
!                                    for non-Microsoft compilers.
!                              Mixed mode arithmetic (R*4 + R*8 corrected).
...
!********************************************************************
!                                                   Find Z, given d.
!        Red Cedar,  NW Taper Coop   April, 1994  (RC41194.coe.coe)
!        Copyright    James W. Flewelling, 1994
!        CHANGES______________________________________________________
!      JUL 23, 1993         Move test of X vs MU
!                            (potential problem for compilers that do not
!                             initilize variables to zero.)
!                           Fix mixed mode arithmetic (R*4 + R*8)
...
!                   NW Taper Coop Doug Fir  (fit on APR 14, 1994)
!                   NW Taper Coop -  Western Hemlock  (fit on APR 14, 1994)
!                   NW Taper Coop -  Red Cedar  (fit on APR 14, 1994)
!                   COPYRIGHT J. Flewelling, 1994.

!     DBHOB        INPUT     R*4  Diameter (o.b.) at Breast height (inches)
!     HTTOT         INPUT     R*4  Total tree height (ft)
!     FDBT_BH3   RETURNED  R*4  Double bark thickness
...
!   Above offsets for   CO    EV    NO    RS    SO    WE    WV   WC(=WE)
!        note: WV has no hemlock data; uses EV coefficient

!      note:   English-only on input and output
...
200 CONTINUE
    ENDIF
!        write(*,*)GEOSUB,DBHOB,ratio,DBHOB*ratio
    FDBT_C1 = RATIO*DBHOB
    RETURN
...
!                      copyright, James Flewelling  1994
  integer jsp
  ...
  IF(JSP.EQ.3) THEN
!      Theory: t is a monotonic transformation of h, goverened by parm B2.
!              r(t) is a quadratic function that takes on the value 1 at BH.
!              t1 is the value of t where r(t) is at a minimum.
!              r1  = r(t1)
!              adjusted r = specified above + correction in upper 25% of stem.
!              DBT/DOB  = [ DBT/DOB at breast height] * [ adjusted r]

!      write(*,*)' ws',DBHOB,HTTOT,h
