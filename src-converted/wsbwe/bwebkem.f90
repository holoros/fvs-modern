...
!----------
!
!     BLOCKDATA FOR THE  BUDWORM MODEL.
!
!     PART OF THE WESTERN SPRUCE BUDWORM MODEL.
!     FORESTRY SCIENCES LAB--MOSCOW, IDAHO--MAY, 1984
!
!   Major revision by K.Sheehan 7/23/96 to strip out
!     BW Pop.Dyn.Model stuff
!   Major revision by K.Sheehan 7/6/96 to add Defoliation Model stuff
!
!     DESCRIPTION :
!
!     MASSIVE INITIALIZATION OF BUDWORM VARIABLES AT LOAD TIME.
!     NON-EXECUTABLE CODE.
!     THE VARIABLES AND ARRAYS INITIALIZED HERE SHOULD BE STATIC AND
!     IF THE VALUES OF ANY OF THESE ARE CHANGED BY KEYWORD OR BY THE
!     MODEL DURING THE SIMULATION, THE INITIALIZATION OF THAT VARIABLE
!     OR ARRAY SHOULD BE MOVED TO SUBROUTINE BWEINT. OTHERWISE, RESIDUAL
!     VALUES OR PARAMETERS WILL BE PASSED TO THE FOLLOWING STAND WHEN
!     PROJECTING MULTIPLE STANDS IN A SINGLE EXECUTION.
!
!  Revision History:
!    01-MAY-00 Lance David (FHTET)
!       Changed keyword BUDLITE to GENDEFOL (General Defoliator).
!    03-MAY-00 Lance David (FHTET)
!       Corrected data statement for TABLE array size.
!       Changed value for MOPT to indicate FVS keyword file.
!    12-MAY-00 Lance David (FHTET)
!       Variable ISIZE changed to TABSZ.
!    31-MAY-00 Lance David (FHTET)
!       Removed data statements that had been commented out during
!       the process of moving initialization of nonstatic variables
!       to the BWEINT routine.
!    04-OCT-00 Lance David (FHTET)
!       Added data statemenst to load weather station names into
!       WSLOOK that had previously been loaded at runtime from
!       stations.dat file.
!    10-NOV-00 Lance David (FHTET)
!       Put Western Larch back into host species list (IBWSPM). It
!       got left behind somewhere in the model update process.
!    09-SEP-06 Lance David (FHTET)
!       Moved non-static variables FOLDVY, FOLWTY and IOUT6A to bweint.f.
!    23-APR-09 Lance R. David (FMSC)
!       New expanded species list for EM variant.
...
!     ****** IBWSPM IS MODIFIED FOR EM VARIANT
!     DEFOLIATION MODEL SPECIES INDICES:
!       1 = WF - White fir
!       2 = DF - Douglas fir
!       3 = GF - Grand fir
!       4 = AF - Subalpine fir
!       5 = ES - Engelmann spruce
!       6 = WL - Western Larch
!       7 = Not a host
!
!     FVS EM VARIANT SPECIES INDICES:
!    AL COMMON                FIA SCIENTIFIC
!  # CD NAME                  CD  NAME
! -- -- --------------------- --- -----------------------------------
!  1 WB WHITEBARK PINE        101 PINUS ALBICALA
!  2 WL WESTERN LARCH         073 LARIX OCCIDENTALIS
!  3 DF DOUGLAS-FIR           202 PSEUDOTSUGA MENZIESII
!  4 LM LIMBER PINE           113 PINUS FLEXILIS
!  5 LL SUBALPINE LARCH       072 LARIX LYALLII
!  6 RM ROCKY MTN JUNIPER     064 JUNIPERUS OCCIDENTALIS
!  7 LP LODGEPOLE PINE        108 PINUS CONTORTA
!  8 ES ENGELMANN SPRUCE      093 PICEA ENGELMANNII
!  9 AF SUBALPINE FIR         019 ABIES LASIOCARPA
! 10 PP PONDEROSA PINE        122 PINUS PONDEROSA
! 11 GA GREEN ASH             544 FRAXINUS PENNSYLVANICA
! 12 AS QUAKING ASPEN         746 POPULUS TREMULOIDES
! 13 CW BLACK COTTONWOOD      747 POPULUS BALSAMIFERA VAR TRICHOCARPA
! 14 BA BALSAM POPLAR         741 POPULUS BALSAMIFERA
! 15 PW PLAINS COTTONWOOD     745 POPULUS DELTOIDES VAR MONOLIFERA
! 16 NC NARROWLEAF COTTONWOOD 749 POPULUS ANGUSTIFOLIA
! 17 PB PAPER BIRCH           375 BETULA PAPYRIFERA
! 18 OS OTHER SOFTWOODS       298
! 19 OH OTHER HARDWOODS       998

!       WB WL DF LM LL RM LP ES AF PP GA AS CW BA PW NC PB OS OH
DATA IBWSPM &
...
!  THE FOLLOWING VARIABLES ARE FOR THE DEFOLIATION MODEL
!
DATA OBTABL/ &
...
!     SET  DEFAULT VALUES (FOLIAGE VARIABLES)
!
...
!     04-OCT-2000 Lance David
!     The data statements after this point are from the stations.dat
!     file dated 17-NOV-1999 that was originally read at runtime.
!
!     Number of weather stations for each state.
!                 AZ,CA,CO, ID,MO,NM,OR,UT,WA,WY
