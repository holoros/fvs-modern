...
! CANADA-ON $Id$
!----------
!  THIS ROUTINE CONTAINS A LIBRARY OF CROWN WIDTH EQUATIONS AVAILABLE
!  FOR USE IN THE EASTERN UNITED STATES.
!  EQUATIONS ARE GROUPED BY SPECIES ACCORDING TO THEIR FIA CODE.
!  THIS ROUTINE COMPUTES LARGEST CROWN WIDTH.
!  IT IS CALLED FROM **CWIDTH** TO PRODUCE CROWTH WIDTH ESTIMATES FOR
!  OF FOREST GROWN TREES, AND IT IS CALLED FROM **CCFCAL** TO PRODUCE
!  CROWN WIDTH ESTIMATES OF OPEN GROWN TREES.
!
!  DEFINITION OF VARIABLES:
! JSP(ISPC) = FVS SPECIES ALPHA NUMBER
!         P = TREES PER ACRE
!         D = TREE DBH
!         H = TREE HEIGHT
!        CR = CROWN RATIO IN PERCENT.
!      IICR = CROWN RATIO - FLOAT
!     HILAT = LATITUDE IN DECIMAL DEGREES
!    HILONG = LONGITUDE (-) IN DECIMAL DEGREES
!    HIELEV = ELEVATION IN FEET
!        HI = HOPKINS BIOCLIMATIC INDEX, SEE BECHTOLD 2003.
!      CWEQ = CW EQUATION NUMBER (FIA # + EQN #)
!               BECHTOLD EQN # = 01
!               BRAGG EQN #    = 02
!               EK EQN #       = 03
!               KRAJICEK EQN # = 04
!               SMITH EQN #    = 05
!        CW = LARGEST CROWN WIDTH
!      IWHO = 1 IF CALLED FROM CCFCAL (USE OPEN GROWN CW EQUATIONS)
!             0 OTHERWISE (USE FOREST GROWN EQUATIONS)
!
!  SOURCES OF FOREST GROWN CROWN WIDTH EQUATIONS:
!  BECHTOLD, WILLIAM A. 2003. CROWN-DIAMETER PREDICTION MODELS FOR
!     87 SPECIES OF STAND-GROWN TREES IN THE EASTERN UNITED STATES.
!     SJAF. 27(4):269-278.
!  BRAGG, DON C. 2001. A LOCAL BASAL AREA ADJUSTMENT FOR CROWN WIDTH
!     PREDICTION. NJAF. 18(1):22-28.
!
!  OPEN GROWN CROWN WIDTH EQUATIONS FOR REGIONS 8 & 9:
!
!  BECHTOLD, WILLIAM A. 2003. CROWN-DIAMETER PREDICTION MODELS FOR
!     87 SPECIES OF STAND-GROWN TREES IN THE EASTERN UNITED STATES.
!     SJAF. 27(4):269-278. MODIFIED FOR CR = 90%
!  EK, ALAN. 1974. DIMENSIONAL RELATIONSHIPS OF FOREST AND OPEN GROWN
!     TREES IN WISCONSIN. UNIV. OF WISCONSIN.
!  KRAJICEK, JOHN E., KENNETH A BRINKMAN, AND SAMUEL F. GINGRICH. 1961.
!     CROWN COMPETITION - A MEASURE OF DENSITY. FOREST SCIENCE,
!     VOL 7(1).
!  SMITH, W.R., R.M. FARRAR, JR, P.A. MURPHY, J.L. YEISER, R.S. MELDAHL,
!     AND J.S. KUSH. 1992. CROWN AND BASAL AREA RELATIONSHIPS OF OPEN-
!     GROWN SOUTHERN PINES FOR MODELING COMPETITION AND GROWTH.
!
!OMMONS
!
!
...
DATA MIND/5./,OMIND/3./
!
!     FOR COMPLIANCE WITH US CROWN WIDTH EQUATIONS, THE 2-CHAR US SPECIES CODES
!     ARE USED HERE, RATHER THAN THE 2-CHAR CDN CODES
!
...
CW = 0.
!----------
! SET VARIABLES FOR CROWN MODELS.
!----------
HILAT=TLAT
HILONG = -1*ABS(TLONG)
HIELEV=ELEV*100
...
!-----------------------------------------------------------------------
!  CASE 01001
!  010         ABIES SPP.                                       FIR SPP.
!         CASE ('01001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 01201 BECHTOLD 2003 MODEL 2
!  012         ABIES BALSAMEA                                 BALSAM FIR
   CASE ('01201')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 34.) CW=34.
!
!  CASE 01202 BRAGG 2001 MODEL NLCW
!  012         ABIES BALSAMEA                                 BALSAM FIR
   CASE ('01202')
...
!-----------------------------------------------------------------------
!  CASE 04301
!  043         CHAMAECYPARIS THYOIDES               ATLANTIC WHITE-CEDAR
!         CASE ('04301')
!           CW =
!-----------------------------------------------------------------------
!  CASE 05701
!  057         JUNIPERUS SPP.                    REDCEDAR / JUNIPER SPP.
!         CASE ('05701')
!           CW =
!-----------------------------------------------------------------------
!  CASE 06801 BECHTOLD 2003 MODEL 2
!  068         JUNIPERUS VIRGINIANA                     EASTERN REDCEDAR
   CASE ('06801')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 33.) CW=33.
! ----------------------------------------------------------------------
!  CASE 07101 BECHTOLD 2003 MODEL 2
!  071         LARIX LARICINA                          TAMARACK (NATIVE)
   CASE ('07101')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 29.) CW=29.
!
!  CASE 07102 BRAGG 2001 MODEL NLCW
!  071         LARIX LARICINA                          TAMARACK (NATIVE)
   CASE ('07102')
...
!-----------------------------------------------------------------------
!  CASE 09001
!  090         PICEA SPP.                                    SPRUCE SPP.
!         CASE ('09001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 09101 BECHTOLD 2003 MODEL 3
!  091         PICEA ABIES                                 NORWAY SPRUCE
   CASE ('09101')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 27.) CW=27.
!
!  CASE 09104 KRAJICEK AND OTHERS 1961
!  091         PICEA ABIES                                 NORWAY SPRUCE
   CASE ('09104')
     IF (D .GE. OMIND) THEN
       CW = 5.0570 + 1.1313*D
     ...
     IF (CW .GT. 47.) CW=47.
!-----------------------------------------------------------------------
!  CASE 09401 BECHTOLD 2003 MODEL 2
!  094         PICEA GLAUCA                                 WHITE SPRUCE
   CASE ('09401')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 30.) CW=30.
!
!  CASE 09402 BRAGG 2001 MODEL NLCW
!  094         PICEA GLAUCA                                 WHITE SPRUCE
   CASE ('09402')
...
!  094         PICEA GLAUCA                                 WHITE SPRUCE
   CASE ('09403')
     ...
     IF (CW .GT. 37.) CW=37.
!-----------------------------------------------------------------------
!  CASE 09501 BECHTOLD 2003 MODEL 2
!  095         PICEA MARIANA                                BLACK SPRUCE
   CASE ('09501')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 27.) CW=27.
!
!  CASE 09502 BRAGG 2001 MODEL NLCW
!  095         PICEA MARIANA                                BLACK SPRUCE
   CASE ('09502')
...
!  095         PICEA MARIANA                                BLACK SPRUCE
   CASE ('09503')
     ...
     IF (CW .GT. 27.) CW=27.
!-----------------------------------------------------------------------
!  CASE 09701 BECHTOLD 2003 MODEL 3
!  097         PICEA RUBENS                                   RED SPRUCE
   CASE ('09701')
     IF (D .GE. MIND) THEN
       IF (D .LT. 30.) THEN
...
!           CW =
!-----------------------------------------------------------------------
!  CASE 10501 BECHTOLD 2003 MODEL 2
!  105         PINUS BANKSIANA                                 JACK PINE
   CASE ('10501')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 25.) CW=25.
!
!  CASE 10502 BRAGG 2001 MODEL NLCW
!  105         PINUS BANKSIANA                                 JACK PINE
   CASE ('10502')
...
!-----------------------------------------------------------------------
!  CASE 10701
!  107         PINUS CLAUSA                                    SAND PINE
!         CASE ('10701')
!           CW =
!-----------------------------------------------------------------------
!  CASE 11001 BECHTOLD 2003 MODEL 3
!  110         PINUS ECHINATA                             SHORTLEAF PINE
   CASE ('11001')
     ...
     ENDIF
       IF (CW .GT. 34.) CW=34.
!
!  CASE 11005 SMITH AND OTHERS 1992
!  110         PINUS ECHINATA                             SHORTLEAF PINE
   CASE ('11005')
     IF (D .GE. OMIND) THEN
     ...
     ENDIF
       IF (CW .GT. 45.) CW=45.
!-----------------------------------------------------------------------
!  CASE 11101 BECHTOLD 2003 MODEL 3
!  111         PINUS ELLIOTTII                                SLASH PINE
   CASE ('11101')
     IF (D .GE. MIND) THEN
       IF (D .LT. 30.) THEN
...
!-----------------------------------------------------------------------
!  CASE 11501
!  115         PINUS GLABRA                                  SPRUCE PINE
!         CASE ('11501')
!           CW =
!-----------------------------------------------------------------------
!  CASE 12101 BECHTOLD 2003 MODEL 3
!  121         PINUS PALUSTRIS                             LONGLEAF PINE
   CASE ('12101')
     IF (D .GE. MIND) THEN
     ...
     ENDIF
       IF (CW .GT. 50.) CW=50.
!
!  CASE 12105 SMITH AND OTHERS 1992
!  121         PINUS PALUSTRIS                             LONGLEAF PINE
   CASE ('12105')
     IF (D .GE. OMIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 12201
!  122         PINUS PONDEROSA                            PONDEROSA PINE
!         CASE ('12201')
!           CW =
!-----------------------------------------------------------------------
!  CASE 12301
!  123         PINUS PUNGENS                         TABLE MOUNTAIN PINE
!         CASE ('12301')
!           CW =
!-----------------------------------------------------------------------
!  CASE 12501 BECHTOLD 2003 MODEL 2
!  125         PINUS RESINOSA                                   RED PINE
   CASE ('12501')
     IF (D .GE. MIND) THEN
       IF (D .LT. 24.) THEN
     ...
     ENDIF
!
!  CASE 12502 BRAGG 2001 MODEL NLCW
!  125         PINUS RESINOSA                                   RED PINE
   CASE ('12502')
...
!  125         PINUS RESINOSA                                   RED PINE
   CASE ('12503')
     ...
     ENDIF
       IF (CW .GT. 39.) CW=39.
!-----------------------------------------------------------------------
!  CASE 12601 BECHTOLD 2003 MODEL 3
!  126         PINUS RIGIDA                                   PITCH PINE
   CASE ('12601')
     ...
     IF (CW .GT. 34.) CW=34.
!-----------------------------------------------------------------------
!  CASE 12801 BECHTOLD 2003 MODEL 1
!  128         PINUS SEROTINA                                  POND PINE
   CASE ('12801')
     IF (D .GE. MIND) THEN
       IF (D .LT. 18.) THEN
     ...
     ENDIF
!-----------------------------------------------------------------------
!  CASE 12901 BECHTOLD 2003 MODEL 2
!  129         PINUS STROBUS                          EASTERN WHITE PINE
   CASE ('12901')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 45.) CW=45.
!
!  CASE 12902 BRAGG 2001 MODEL NLCW
!  129         PINUS STROBUS                          EASTERN WHITE PINE
   CASE ('12902')
...
!  129         PINUS STROBUS                          EASTERN WHITE PINE
   CASE ('12903')
     ...
     IF (CW .GT. 58.) CW=58.
!-----------------------------------------------------------------------
!  CASE 13001 BECHTOLD 2003 MODEL 2
!  130         PINUS SYLVESTRIS                              SCOTCH PINE
   CASE ('13001')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 27.) CW=27.
!-----------------------------------------------------------------------
!  CASE 13101 BECHTOLD 2003 MODEL 2
!  131         PINUS TAEDA                                 LOBLOLLY PINE
   CASE ('13101')
     IF (D .GE. MIND) THEN
...
!  131         PINUS TAEDA                                 LOBLOLLY PINE
   CASE ('13105')
     ...
     IF (CW .GT. 66.) CW=66.
!-----------------------------------------------------------------------
!  CASE 13201 BECHTOLD 2003 MODEL 2
!  132         PINUS VIRGINIANA                            VIRGINIA PINE
   CASE ('13201')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 34.) CW=34.
!-----------------------------------------------------------------------
!  CASE 22101 BECHTOLD 2003 MODEL 2
!  221         TAXODIUM DISTICHUM                            BALDCYPRESS
   CASE ('22101')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 22201
!  222         TAXODIUM ASCENDENS                            PONDCYPRESS
!         CASE ('22201')
!           CW =
!-----------------------------------------------------------------------
!  CASE 24101 BECHTOLD 2003 MODEL 2
!  241         THUJA OCCIDENTALIS                   NORTHERN WHITE-CEDAR
   CASE ('24101')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 27.) CW=27.
!
!  CASE 24102 BRAGG 2001 MODEL NLCW
!  241         THUJA OCCIDENTALIS                   NORTHERN WHITE-CEDAR
   CASE ('24102')
...
!-----------------------------------------------------------------------
!  CASE 26001
!  260         TSUGA SPP.                                   HEMLOCK SPP.
!         CASE ('26001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 26101 BECHTOLD 2003 MODEL 3
!  261         TSUGA CANADENSIS                          EASTERN HEMLOCK
   CASE ('26101')
     ...
     ENDIF
!
!  CASE 26102 BRAGG 2001 MODEL NLCW
!  261         TSUGA CANADENSIS                          EASTERN HEMLOCK
   CASE ('26102')
...
!-----------------------------------------------------------------------
!  CASE 31001
!  310         ACER SPP.                                      MAPLE SPP.
!         CASE ('31001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 31101
!  311         ACER BARBATUM                               FLORIDA MAPLE
!         CASE ('31101')
!           CW =
!-----------------------------------------------------------------------
!  CASE 31301 BECHTOLD 2003 MODEL 3
!  313         ACER NEGUNDO                                     BOXELDER
   CASE ('31301')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 31401
!  314         ACER NIGRUM                                   BLACK MAPLE
!         CASE ('31401')
!           CW =
!-----------------------------------------------------------------------
!  CASE 31501
!  315         ACER PENSYLVANICUM                          STRIPED MAPLE
!         CASE ('31501')
!           CW =
!-----------------------------------------------------------------------
!  CASE 31601 BECHTOLD 2003 MODEL 3
!  316         ACER RUBRUM                                     RED MAPLE
   CASE ('31601')
     ...
     ENDIF
!
!  CASE 31602 BRAGG 2001 MODEL NLCW
!  316         ACER RUBRUM                                     RED MAPLE
   CASE ('31602')
...
!  316         ACER RUBRUM                                     RED MAPLE
   CASE ('31603')
     ...
     IF (CW .GT. 55.) CW=55.
!-----------------------------------------------------------------------
!  CASE 31701 BECHTOLD 2003 MODEL 3
!  317         ACER SACCHARINUM                             SILVER MAPLE
   CASE ('31701')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 45.) CW=45.
!-----------------------------------------------------------------------
!  CASE 31801 BECHTOLD 2003 MODEL 3
!  318         ACER SACCHARUM                                SUGAR MAPLE
   CASE ('31801')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 54.) CW=54.
!
!  CASE 31802 BRAGG 2001 MODEL NLCW
!  318         ACER SACCHARUM                                SUGAR MAPLE
   CASE ('31802')
...
!-----------------------------------------------------------------------
!  CASE 31901
!  319         ACER SPICATUM                              MOUNTAIN MAPLE
!         CASE ('31901')
!           CW =
!-----------------------------------------------------------------------
!  CASE 32301
!  323         ACER LEUCODERME                               CHALK MAPLE
!         CASE ('32301')
!           CW =
!-----------------------------------------------------------------------
!  CASE 33001
!  330         AESCULUS SPP.                  BUCKEYE/HORSECHESTNUT SPP.
!         CASE ('33001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 33101
!  331         AESCULUS GLABRA                              OHIO BUCKEYE
!         CASE ('33101')
!           CW =
!-----------------------------------------------------------------------
!  CASE 33201
!  332         AESCULUS FLAVA                             YELLOW BUCKEYE
!         CASE ('33201')
!           CW =
!-----------------------------------------------------------------------
!  CASE 34101
...
!-----------------------------------------------------------------------
!  CASE 37001
!  370         BETULA SPP.                                    BIRCH SPP.
!         CASE ('37001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 37101 BECHTOLD 2003 MODEL 3
!  371         BETULA ALLEGHANIENSIS                        YELLOW BIRCH
   CASE ('37101')
     IF (D .GE. MIND) THEN
       IF (D .LT. 24.) THEN
     ...
     ENDIF
!
!  CASE 37102 BRAGG 2001 MODEL NLCW
!  371         BETULA ALLEGHANIENSIS                        YELLOW BIRCH
   CASE ('37102')
     CW = 2.374661 + 4.110366*D**0.677280
!-----------------------------------------------------------------------
!  CASE 37201 BECHTOLD 2003 MODEL 2
!  372         BETULA LENTA                                  SWEET BIRCH
   CASE ('37201')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 54.) CW=54.
!-----------------------------------------------------------------------
!  CASE 37301 BECHTOLD 2003 MODEL 1
!  373         BETULA NIGRA                                  RIVER BIRCH
   CASE ('37301')
     IF (D .GE. MIND) THEN
       CW = 11.6634 + 1.0028*D
     ELSE
...
!-----------------------------------------------------------------------
!  CASE 37401
!  374         BETULA OCCIDENTALIS                           WATER BIRCH
!         CASE ('37401')
!           CW =
!-----------------------------------------------------------------------
!  CASE 37501 BECHTOLD 2003 MODEL 3
!  375         BETULA PAPYRIFERA                             PAPER BIRCH
   CASE ('37501')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 42.) CW=42.
!
!  CASE 37502 BRAGG 2001 MODEL NLCW
!  375         BETULA PAPYRIFERA                             PAPER BIRCH
   CASE ('37502')
...
!-----------------------------------------------------------------------
!  CASE 37901
!  379         BETULA POPULIFOLIA                             GRAY BIRCH
!         CASE ('37901')
!           CW =
!-----------------------------------------------------------------------
!  CASE 39101 BECHTOLD 2003 MODEL 3
!  391         CARPINUS CAROLINIANA                    AMERICAN HORNBEAM
   CASE ('39101')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 40101
!  401         CARYA AQUATICA                              WATER HICKORY
!         CASE ('40101')
!           CW =
!-----------------------------------------------------------------------
!  CASE 40201 BECHTOLD 2003 MODEL 1
!  402         CARYA CORDIFORMIS                       BITTERNUT HICKORY
   CASE ('40201')
     IF (D .GE. MIND) THEN
       CW = 8.0118 + 1.4212*D
     ELSE
     ...
     IF (CW .GT. 41.) CW=41.
!-----------------------------------------------------------------------
!  CASE 40301 BECHTOLD 2003 MODEL 2
...
!-----------------------------------------------------------------------
!  CASE 40401
!  404         CARYA ILLINOINENSIS                                 PECAN
!         CASE ('40401')
!           CW =
!-----------------------------------------------------------------------
!  CASE 40501
!  405         CARYA LACINIOSA                         SHELLBARK HICKORY
!         CASE ('40501')
!           CW =
!-----------------------------------------------------------------------
!  CASE 40701 BECHTOLD 2003 MODEL 2
!  407         CARYA OVATA                              SHAGBARK HICKORY
   CASE ('40701')
     IF (D .GE. MIND) THEN
...
!  407         CARYA OVATA                              SHAGBARK HICKORY
   CASE ('40704')
     ...
     IF (CW .GT. 54.) CW=54.
!-----------------------------------------------------------------------
!  CASE 40801 BECHTOLD 2003 MODEL 1
!  408         CARYA TEXANA                                BLACK HICKORY
   CASE ('40801')
     IF (D .GE. MIND) THEN
       IF (D .LT. 15.) THEN
     ...
     ENDIF
!-----------------------------------------------------------------------
!  CASE 40901 BECHTOLD 2003 MODEL 2
!  409         CARYA ALBA                              MOCKERNUT HICKORY
   CASE ('40901')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 42101
!  421         CASTANEA DENTATA                        AMERICAN CHESTNUT
!         CASE ('42101')
!           CW =
!-----------------------------------------------------------------------
!  CASE 45001
!  450         CATALPA SPP.                                 CATALPA SPP.
!         CASE ('45001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 46001
!  460         CELTIS SPP.                                HACKBERRY SPP.
!         CASE ('46001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 46101
!  461         CELTIS LAEVIGATA                               SUGARBERRY
!         CASE ('46101')
!           CW =
!-----------------------------------------------------------------------
!  CASE 46201 BECHTOLD 2003 MODEL 2
!  462         CELTIS OCCIDENTALIS                             HACKBERRY
   CASE ('46201')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 47101
!  471         CERCIS CANADENSIS                          EASTERN REDBUD
!         CASE ('47101')
!           CW =
!-----------------------------------------------------------------------
!  CASE 49101 BECHTOLD 2003 MODEL 2
...
!-----------------------------------------------------------------------
!  CASE 50001
!  500         CRATAEGUS SPP.                              HAWTHORN SPP.
!         CASE ('50001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 52101 BECHTOLD 2003 MODEL 2
!  521         DIOSPYROS VIRGINIANA                     COMMON PERSIMMON
   CASE ('52101')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 36.) CW=36.
!-----------------------------------------------------------------------
!  CASE 53101 BECHTOLD 2003 MODEL 3
...
!-----------------------------------------------------------------------
!  CASE 54001
!  540         FRAXINUS SPP.                                    ASH SPP.
!         CASE ('54001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 54101 BECHTOLD 2003 MODEL 2
!  541         FRAXINUS AMERICANA                              WHITE ASH
   CASE ('54101')
    IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 62.) CW=62.
!
!  CASE 54102 BRAGG 2001 MODEL NLCW
!  541         FRAXINUS AMERICANA                              WHITE ASH
   CASE ('54102')
...
!  541         FRAXINUS AMERICANA                              WHITE ASH
   CASE ('54103')
     ...
     IF (CW .GT. 62.) CW=62.
!-----------------------------------------------------------------------
!  CASE 54301 BECHTOLD 2003 MODEL 1
!  543         FRAXINUS NIGRA                                  BLACK ASH
   CASE ('54301')
     IF (D .GE. MIND) THEN
       CW = 5.2824 + 1.1184*D
     ELSE
     ...
     IF (CW .GT. 34.) CW=34.
!
!  CASE 54302 BRAGG 2001 MODEL NLCW
!  543         FRAXINUS NIGRA                                  BLACK ASH
   CASE ('54302')
     CW = 2.761995 + 2.560977*D**0.742525
!-----------------------------------------------------------------------
!  CASE 54401 BECHTOLD 2003 MODEL 2
!  544         FRAXINUS PENNSYLVANICA                          GREEN ASH
   CASE ('54401')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 54501
!  545         FRAXINUS PROFUNDA                             PUMPKIN ASH
!         CASE ('54501')
!           CW =
!-----------------------------------------------------------------------
!  CASE 54601
!  546         FRAXINUS QUADRANGULATA                           BLUE ASH
!         CASE ('54601')
!           CW =
!-----------------------------------------------------------------------
!  CASE 55201 BECHTOLD 2003 MODEL 2
!  552         GLEDITSIA TRIACANTHOS                         HONEYLOCUST
   CASE ('55201')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 55501
!  555         GORDONIA LASIANTHUS                          LOBLOLLY BAY
!         CASE ('55501')
!           CW =
!-----------------------------------------------------------------------
!  CASE 57101
!  571         GYMNOCLADUS DIOICUS                   KENTUCKY COFFEETREE
!         CASE ('57101')
!           CW =
!-----------------------------------------------------------------------
!  CASE 58001
!  580         HALESIA SPP.                              SILVERBELL SPP.
!         CASE ('58001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 59101 BECHTOLD 2003 MODEL 2
!  591         ILEX OPACA                                 AMERICAN HOLLY
   CASE ('59101')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 60101
!  601         JUGLANS CINEREA                                 BUTTERNUT
!         CASE ('60101')
!           CW =
!-----------------------------------------------------------------------
!  CASE 60201 BECHTOLD 2003 MODEL 2
!  602         JUGLANS NIGRA                                BLACK WALNUT
   CASE ('60201')
     IF (D .GE. MIND) THEN
...
!  602         JUGLANS NIGRA                                BLACK WALNUT
   CASE ('60203')
     ...
     IF (CW .GT. 37.) CW=37.
!-----------------------------------------------------------------------
!  CASE 61101 BECHTOLD 2003 MODEL 3
!  611         LIQUIDAMBAR STYRACIFLUA                          SWEETGUM
   CASE ('61101')
     ...
     IF (CW .GT. 50.) CW=50.
!-----------------------------------------------------------------------
!  CASE 62101 BECHTOLD 2003 MODEL 2
...
!-----------------------------------------------------------------------
!  CASE 64101
!  641         MACLURA POMIFERA                             OSAGE-ORANGE
!         CASE ('64101')
!           CW =
!-----------------------------------------------------------------------
!  CASE 65001
!  650         MAGNOLIA SPP.                               MAGNOLIA SPP.
!         CASE ('65001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 65101 BECHTOLD 2003 MODEL 1
!  651         MAGNOLIA ACUMINATA                           CUCUMBERTREE
   CASE ('65101')
     IF (D .GE. MIND) THEN
       CW = 4.1711 + 1.6275*D
     ELSE
...
!-----------------------------------------------------------------------
!  CASE 65201
!  652         MAGNOLIA GRANDIFLORA                    SOUTHERN MAGNOLIA
!         CASE ('65201')
!           CW =
!-----------------------------------------------------------------------
!  CASE 65301 BECHTOLD 2003 MODEL 1
!  653         MAGNOLIA VIRGINIANA                              SWEETBAY
   CASE ('65301')
     IF (D .GE. MIND) THEN
       CW = 8.2119 + 0.9708*D
     ELSE
...
!-----------------------------------------------------------------------
!  CASE 65401
!  654         MAGNOLIA MACROPHYLLA                     BIGLEAF MAGNOLIA
!         CASE ('65401')
!           CW =
!-----------------------------------------------------------------------
!  CASE 66001
!  660         MALUS SPP.                                     APPLE SPP.
!         CASE ('66001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 68001
!  680         MORUS SPP.                                  MULBERRY SPP.
!         CASE ('68001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 68101
!  681         MORUS ALBA                                 WHITE MULBERRY
!         CASE ('68101')
!           CW =
!-----------------------------------------------------------------------
!  CASE 68201 BECHTOLD 2003 MODEL 1
!  682         MORUS RUBRA                                  RED MULBERRY
   CASE ('68201')
     IF (D .GE. MIND) THEN
       CW = 13.3255 + 1.0735*D
     ELSE
...
!-----------------------------------------------------------------------
!  CASE 69001
!  690         NYSSA SPP.                                    TUPELO SPP.
!         CASE ('69001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 69101 BECHTOLD 2003 MODEL 2
!  691         NYSSA AQUATICA                               WATER TUPELO
   CASE ('69101')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 69201
!  692         NYSSA OCECHE                              OGEECHEE TUPELO
!         CASE ('69201')
!           CW =
!-----------------------------------------------------------------------
!  CASE 69301 BECHTOLD 2003 MODEL 3
!  693         NYSSA SYLVATICA                                  BLACKGUM
   CASE ('69301')
     ...
     IF (CW .GT. 50.) CW=50.
!-----------------------------------------------------------------------
!  CASE 69401 BECHTOLD 2003 MODEL 2
!  694         NYSSA BIFLORA                                SWAMP TUPELO
   CASE ('69401')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 41.) CW=41.
!-----------------------------------------------------------------------
!  CASE 70101 BECHTOLD 2003 MODEL 3
!  701         OSTRYA VIRGINIANA                     EASTERN HOPHORNBEAM
   CASE ('70101')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 39.) CW=39.
!
!  CASE 70102 BRAGG 2001 MODEL NLCW
!  701         OSTRYA VIRGINIANA                     EASTERN HOPHORNBEAM
   CASE ('70102')
     CW = -33.898790 + 38.731332*D**0.152718
!-----------------------------------------------------------------------
!  CASE 71101 BECHTOLD 2003 MODEL 3
...
!-----------------------------------------------------------------------
!  CASE 71201
!  712         PAULOWNIA TOMENTOSA               PAULOWNIA, EMPRESS-TREE
!         CASE ('71201')
!           CW =
!-----------------------------------------------------------------------
!  CASE 72101 BECHTOLD 2003 MODEL 3
!  721         PERSEA BORBONIA                                    REDBAY
   CASE ('72101')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 25.) CW=25.
!-----------------------------------------------------------------------
!  CASE 73101 BECHTOLD 2003 MODEL 2
!  731         PLATANUS OCCIDENTALIS                   AMERICAN SYCAMORE
   CASE ('73101')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 74001
!  740         POPULUS SPP.                   COTTONWOOD AND POPLAR SPP.
!         CASE ('74001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 74101 BECHTOLD 2003 MODEL 1
!  741         POPULUS BALSAMIFERA                         BALSAM POPLAR
   CASE ('74101')
     IF (D .GE. MIND) THEN
       CW = 6.2498 + 0.8655*D
     ELSE
     ...
     IF (CW .GT. 25.) CW=25.
!
!  CASE 74102 BRAGG 2001 MODEL NLCW
!  741         POPULUS BALSAMIFERA                         BALSAM POPLAR
   CASE ('74102')
     CW = 7.522796 + 0.125282*D**1.855258
!-----------------------------------------------------------------------
!  CASE 74201 BECHTOLD 2003 MODEL 1
!  742         POPULUS DELTOIDES                      EASTERN COTTONWOOD
   CASE ('74201')
     IF (D .GE. MIND) THEN
       CW = 3.4375 + 1.4092*D
     ELSE
...
!  742         POPULUS DELTOIDES                      EASTERN COTTONWOOD
   CASE ('74203')
     ...
     IF (CW .GT. 80.) CW=80.
!-----------------------------------------------------------------------
!  CASE 74301 BECHTOLD 2003 MODEL 3
!  743         POPULUS GRANDIDENTATA                      BIGTOOTH ASPEN
   CASE ('74301')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 43.) CW=43.
!
!  CASE 74302 BRAGG 2001 MODEL NLCW
!  743         POPULUS GRANDIDENTATA                      BIGTOOTH ASPEN
   CASE ('74302')
...
!-----------------------------------------------------------------------
!  CASE 74401
!  744         POPULUS HETEROPHYLLA                     SWAMP COTTONWOOD
!         CASE ('74401')
!           CW =
!-----------------------------------------------------------------------
!  CASE 74601 BECHTOLD 2003 MODEL 2
!  746         POPULUS TREMULOIDES                         QUAKING ASPEN
   CASE ('74601')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 39.) CW=39.
!
!  CASE 74602 BRAGG 2001 MODEL NLCW
!  746         POPULUS TREMULOIDES                         QUAKING ASPEN
   CASE ('74602')
...
!-----------------------------------------------------------------------
!  CASE 76001
!  760         PRUNUS SPP.                          CHERRY AND PLUM SPP.
!         CASE ('76001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 76102 BRAGG 2001 MODEL NLCW
!  761         PRUNUS PENNSYLVANICA                           PIN CHERRY
   CASE ('76102')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 52.) CW=52.
!-----------------------------------------------------------------------
!  CASE 76201 BECHTOLD 2003 MODEL 3
!  762         PRUNUS SEROTINA                              BLACK CHERRY
   CASE ('76201')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 52.) CW=52.
!
!  CASE 76202 BRAGG 2001 MODEL NLCW
!  762         PRUNUS SEROTINA                              BLACK CHERRY
   CASE ('76202')
...
!-----------------------------------------------------------------------
!  CASE 76301
!  763         PRUNUS VIRGINIANA                      COMMON CHOKECHERRY
!         CASE ('76301')
!           CW =
!-----------------------------------------------------------------------
!  CASE 80001
!  800         QUERCUS SPP.                                     OAK SPP.
!         CASE ('80001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 80201 BECHTOLD 2003 MODEL 3
...
!-----------------------------------------------------------------------
!  CASE 80401
!  804         QUERCUS BICOLOR                           SWAMP WHITE OAK
!         CASE ('80401')
!           CW =
!-----------------------------------------------------------------------
!  CASE 80601 BECHTOLD 2003 MODEL 2
!  806         QUERCUS COCCINEA                              SCARLET OAK
   CASE ('80601')
     ...
     IF (CW .GT. 66.) CW=66.
!-----------------------------------------------------------------------
!  CASE 80901 BECHTOLD 2003 MODEL 1
!  809         QUERCUS ELLIPSOIDALIS                    NORTHERN PIN OAK
   CASE ('80901')
     IF (D .GE. MIND) THEN
       CW = 4.8935 + 1.6069*D
     ELSE
     ...
     IF (CW .GT. 44.) CW=44.
!-----------------------------------------------------------------------
!  CASE 81201 BECHTOLD 2003 MODEL 2
...
!-----------------------------------------------------------------------
!  CASE 81301
!  813         QUERCUS PAGODA                             CHERRYBARK OAK
!         CASE ('81301')
!           CW =
!-----------------------------------------------------------------------
!  CASE 81701 BECHTOLD 2003 MODEL 1
!  817         QUERCUS IMBRICARIA                            SHINGLE OAK
   CASE ('81701')
     IF (D .GE. MIND) THEN
       CW = 9.8187 + 1.1343*D
     ELSE
     ...
     IF (CW .GT. 54.) CW=54.
!-----------------------------------------------------------------------
!  CASE 81901 BECHTOLD 2003 MODEL 1
!  819         QUERCUS LAEVIS                                 TURKEY OAK
   CASE ('81901')
     IF (D .GE. MIND) THEN
       CW = 5.8858 + 1.4935*D
     ELSE
     ...
     IF (CW .GT. 29.) CW=29.
!-----------------------------------------------------------------------
!  CASE 82001 BECHTOLD 2003 MODEL 1
!  820         QUERCUS LAURIFOLIA                             LAUREL OAK
   CASE ('82001')
     IF (D .GE. MIND) THEN
       CW = 6.3149 + 1.6455*D
     ELSE
...
!-----------------------------------------------------------------------
!  CASE 82201
!  822         QUERCUS LYRATA                                OVERCUP OAK
!         CASE ('82201')
!           CW =
!-----------------------------------------------------------------------
!  CASE 82301 BECHTOLD 2003 MODEL 2
!  823         QUERCUS MACROCARPA                                BUR OAK
   CASE ('82301')
     IF (D .GE. MIND) THEN
...
!  823         QUERCUS MACROCARPA                                BUR OAK
   CASE ('82303')
    ...
    IF (CW .GT. 78.) CW=78.
!-----------------------------------------------------------------------
!  CASE 82401 BECHTOLD 2003 MODEL 2
...
!-----------------------------------------------------------------------
!  CASE 82501
!  825         QUERCUS MICHAUXII                       SWAMP CHESNUT OAK
!         CASE ('82501')
!           CW =
!-----------------------------------------------------------------------
!  CASE 82601 BECHTOLD 2003 MODEL 3
!  826         QUERCUS MUEHLENBERGII                       CHINKAPIN OAK
   CASE ('82601')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 45.) CW=45.
!-----------------------------------------------------------------------
!  CASE 82701 BECHTOLD 2003 MODEL 3
!  827         QUERCUS NIGRA                                   WATER OAK
   CASE ('82701')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 82801
!  828         QUERCUS BUCKLEYI                              NUTTALL OAK
!         CASE ('82801')
!           CW =
!-----------------------------------------------------------------------
!  CASE 83001 BECHTOLD 2003 MODEL 3
!  830         QUERCUS PALUSTRIS                                 PIN OAK
   CASE ('83001')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 63.) CW=63.
!-----------------------------------------------------------------------
!  CASE 83101 BECHTOLD 2003 MODEL 2
!  831         QUERCUS PHELLOS                                WILLOW OAK
   CASE ('83101')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 74.) CW=74.
!-----------------------------------------------------------------------
!  CASE 83201 BECHTOLD 2003 MODEL 2
!  832         QUERCUS PRINUS                                CHESNUT OAK
   CASE ('83201')
     IF (D .GE. MIND) THEN
       IF (D .LT. 50.) THEN
     ...
     ENDIF
!-----------------------------------------------------------------------
!  CASE 83301 BECHTOLD 2003 MODEL 2
!  833         QUERCUS RUBRA                            NORTHERN RED OAK
   CASE ('83301')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 82.) CW=82.
!
!  CASE 83302 BRAGG 2001 MODEL NLCW
!  833         QUERCUS RUBRA                            NORTHERN RED OAK
   CASE ('83302')
...
!-----------------------------------------------------------------------
!  CASE 83401
!  834         QUERCUS SHUMARDII                            SHUMARDS OAK
!         CASE ('83401')
!           CW =
!-----------------------------------------------------------------------
!  CASE 83501 BECHTOLD 2003 MODEL 2
!  835         QUERCUS STELLATA                                 POST OAK
   CASE ('83501')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 83601
!  836         QUERCUS SIMILIS                            DELTA POST OAK
!         CASE ('83601')
!           CW =
!-----------------------------------------------------------------------
!  CASE 83701 BECHTOLD 2003 MODEL 2
...
!  837         QUERCUS VELUTINA                                BLACK OAK
   CASE ('83704')
    ...
    IF (CW .GT. 52.) CW=52.
!-----------------------------------------------------------------------
!  CASE 83801 BECHTOLD 2003 MODEL 1
!  838         QUERCUS VIRGINIANA                               LIVE OAK
   CASE ('83801')
     IF (D .GE. MIND) THEN
       CW = 5.6694 + 1.6402*D
     ELSE
...
!-----------------------------------------------------------------------
!  CASE 84001
!  840         QUERCUS MARGARETTIAE                       DWARF POST OAK
!         CASE ('84001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 84201
!  842         QUERCUS INCANA                               BLUEJACK OAK
!         CASE ('84201')
!           CW =
!-----------------------------------------------------------------------
!  CASE 90101 BECHTOLD 2003 MODEL 2
!  901         ROBINIA PSEUDOACACIA                         BLACK LOCUST
   CASE ('90101')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 92001
!  920         SALIX SPP.                                    WILLOW SPP.
!         CASE ('92001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 92201
!  922         SALIX NIGRA                                  BLACK WILLOW
!         CASE ('92201')
!           CW =
!-----------------------------------------------------------------------
!  CASE 92301
!  923         SALIX BEBBIANA                                BEBB WILLOW
!         CASE ('92301')
!           CW =
!-----------------------------------------------------------------------
!  CASE 93101 BECHTOLD 2003 MODEL 2
!  931         SASSAFRAS ALBIDUM                               SASSAFRAS
   CASE ('93101')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 93501
!  935         SORBUS AMERICANA                    AMERICAN MOUNTAIN ASH
!         CASE ('93501')
!           CW =
!-----------------------------------------------------------------------
!  CASE 95001
!  950         TILIA SPP.                                  BASSWOOD SPP.
!         CASE ('95001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 95101 BECHTOLD 2003 MODEL 3
!  951         TILIA AMERICANA                         AMERICAN BASSWOOD
   CASE ('95101')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 61.) CW=61.
!
!  CASE 95102 BRAGG 2001 MODEL NLCW
!  951         TILIA AMERICANA                         AMERICAN BASSWOOD
   CASE ('95102')
...
!-----------------------------------------------------------------------
!  CASE 95201
!  952         TILIA AMERICANA VAR. HETEROPHYLLA          WHITE BASSWOOD
!         CASE ('95201')
!           CW =
!-----------------------------------------------------------------------
!  CASE 97001
!  970         ULMUS SPP.                                       ELM SPP.
!         CASE ('97001')
!           CW =
!-----------------------------------------------------------------------
!  CASE 97101 BECHTOLD 2003 MODEL 2
!  971         ULMUS ALATA                                    WINGED ELM
   CASE ('97101')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 40.) CW=40.
!-----------------------------------------------------------------------
!  CASE 97201 BECHTOLD 2003 MODEL 3
!  972         ULMUS AMERICANA                              AMERICAN ELM
   CASE ('97201')
     IF (D .GE. MIND) THEN
     ...
     IF (CW .GT. 50.) CW=50.
!
!  CASE 97202 BRAGG 2001 MODEL NLCW
!  972         ULMUS AMERICANA                              AMERICAN ELM
   CASE ('97202')
...
!-----------------------------------------------------------------------
!  CASE 97401
!  974         ULMUS PUMILA                                 SIBERIAN ELM
!         CASE ('97401')
!           CW =
!-----------------------------------------------------------------------
!  CASE 97501 BECHTOLD 2003 MODEL 3
!  975         ULMUS RUBRA                                  SLIPPERY ELM
   CASE ('97501')
     IF (D .GE. MIND) THEN
...
!-----------------------------------------------------------------------
!  CASE 97701
!  977         ULMUS THOMASII                                   ROCK ELM
!         CASE ('97701')
!           CW =
!-----------------------------------------------------------------------
!  CASE 99801
!  998         ---                                   OTHER HARDWOOD SPP.
!         CASE ('99801')
!           CW =
!-----------------------------------------------------------------------
!  CASE 99901
!  999         ---                                  OTHER / UNKNOWN SPP.
!         CASE ('99901')
!           CW =
!-----------------------------------------------------------------------
!
