// ============================================================================
// hg_organon_phase4_candidate.stan
//
// Phase 4 candidate HG architecture: combines all diagnostic findings
// from the Phase 3 preflights into a single specification ready for
// production validation.
//
// Architectural additions on top of hg_organon_fixedK.stan:
//
//   1. Centered ln_cspi_shift to decorrelate linear and quadratic terms
//   2. Quadratic CSPI:        a4b * (ln_cspi - mean)^2
//   3. L1-varying CSPI slope: a_l1_cspi[l] * (ln_cspi - mean)
//      (sigma_a_l1_cspi diagnostic returned 0.17, well-identified)
//   4. Plantation main effect on the base: a_plant * is_plantation
//      (Phase 3 modifier returned +0.15; preflight returned +0.35)
//
...
//       + a4   ln_cspi_c
//       + a4b  ln_cspi_c^2
//       + a_l1_cspi[l1] * ln_cspi_c
//       + a_plant * is_plantation
//       + a5 CCFL/ln(HT+K4h) + a6 sqrt(BA)
//       + a7 BA*RD + a8 CCFL*RD
//
// Note: indexed-vector elementwise multiplications are pre-extracted
// into flat per-obs vectors to avoid the gcc graphite optimizer
// trigger on Cardinal.
// ============================================================================
...
parameters {
  ...
  real<lower=1e-4> sigma_a_l1_cspi;
...
model {
  ...
  a_plant  ~ normal( 0.2, 0.3);    // expectation +0.15 from modifier, preflight +0.35
  ...
  // Pre-extract the L1-varying CSPI slope to a per-obs vector so that
  // the elementwise multiplication doesn't trigger the gcc graphite
  // optimizer on Cardinal.
