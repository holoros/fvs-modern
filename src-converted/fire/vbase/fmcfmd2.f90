...
!                ON JOE SCOTTS LOGIC AND SELECTED FUEL MODEL SET (WHEREAS
...
!
!     ADDITIONAL VARIABLES REQUIRED FOR ENTRY FMCFMD3
!
!
...
!     CHECK WHETHER THE FMODLIST KEYWORD IS SCHEDULED FOR THIS YEAR
...
!     CALCULATE LIVE FRACTION, HERB FRACTION, AND HERB RATIO

DO I = 1, MXFLCL
  CURRCWD(I) = 0.0
ENDDO

!     Sum up CWD categories by size class
...
!     FOR THE LIVE WOODY ESTIMATE, INCLUDE THE FOLIAGE AND HALF THE 0-.25 INCH
!     BRANCHWOOD OF ALL LIVE TREES NOT INCLUDED IN THE CANOPY FUELS CALCULATIONS
!     ALSO INCLUDE THE SHRUBS POOL
...
!     CALCULATE WHETHER YOU ARE IN AN ARID OR HUMID VARIANT

SELECT CASE (VARACD)
!
  CASE ('AK','CS','LS','NE','OP','PN','SN','WC','ON')
    LARID = .FALSE.
!
  CASE DEFAULT
    LARID = .TRUE.
...
!     SET THE FUEL MODEL PICK LIST BASED ON ARID/HUMID AND FIRE CARRYING FUEL TYPE
!     (GR, GS, SH/TU, OR TL/SB).  MAKE SURE THE RIGHT FUEL MODEL SET (13, 40, OR 53)
!     IS BEING USED.
...
!     CALCULATE FUEL BED BULK DENSITY
...
!     CALCULATE SAV FOR THE FUEL BED
...
!      IF (DEBUG) WRITE(JOSTND,*) SAVLIVE, WEIGHTLIVE, SAVDEAD,
!     &           WEIGHTDEAD,USAV(1),USAV(2),USAV(3),
!     &           WTFT(1),WTFT(2),WTFT(3),WTFT(4),WTFT(5),
!     &           SALIVE, SADEAD, SURFAREA(1), SURFAREA(2), SURFAREA(3),
!     &           SURFAREA(4), SURFAREA(5)

IF (DEBUG) WRITE(JOSTND,3) FBSAV,FBBD,FBFFL
3 FORMAT(' FMCFMD2 FBSAV= ',F8.2,' FBBD=',F7.2,' FBFFL=',F7.2)

!     CALCULATE SAV FOR THE FUEL MODEL
...
!     CALCULATE BULK DENSITY AND FINE FUEL LOAD FOR THE FUEL MODELS
!     THEN CALCULATE THE DEPARTURE INDEX
...
!     FIGURE OUT WHICH TWO FUEL MODELS HAVE THE LOWEST DEPARTURE INDEX
...
!     SET NFMODS, FMOD(I), FWT(I), AND FMD, DEPENDING ON WHETHER
!     THE STATIC OR DYNAMIC FUEL MODEL OPTION IS TURNED ON
...
!              FMCFMD2
!
!     NOTE: If desired, this could be made into a seperate file/routine.
!     Comments below will help in that process.
!
!  PURPOSE:
!     THIS SUBROUTINE DETERMINES THE VARIABLES NEEDED FOR DETERMINING THE
!     FUEL MODEL.
!     ALL CODE IS EXTRACTED FROM THE EARLIER FMBURN ROUTINE.
!
!  CALL LIST DEFINITIONS:
!     IYR:  CURRENT YEAR
...
!----------
!OMMONS: For use if turned into its own file.
INCLUDE 'PRGPRM.f90'
INCLUDE 'FMPARM.f90'
INCLUDE 'FMCOM.f90'
INCLUDE 'FMFCOM.f90'
...
INCLUDE 'ARRAYS.f90'
!----------
!     VARIABLE DECLARATIONS: for use if turned into its own file
!----------
...
IF (DEBUG) WRITE(JOSTND,107) ICYC, IYR, IFLOGIC
107 FORMAT(' FMCFMD3 CYCLE=',I2,' IYR=',I5,' IFLOGIC=',I2)
!----------
...
!       Sum up CWD categories by size class and convert from tons/acre to lbs/ft2
...
!       calculate fuel bed depth and moisture of extinction.
!       but first need to calculate fuelbed bulk density.
...
!  (IF BURNYR EQ IYR, THIS WILL HAPPEN IN FMMAIN.)
!----------
IF (BURNYR .NE. IYR) CALL FMPOCR(IYR,1)
!----------
!  WE WANT TO KNOW THE FUEL MODEL THIS STAND WOULD BE, EVEN IF THERE
!  IS NO FIRE SCHEDULED FOR THIS YEAR (SO CAN PRINT TO AN OUTPUT FILE)
!
!  SELECT APPROPRIATE FUEL MODEL OR MODELS AND CORRESPONDING WEIGHTS
!  BASED ON VARIANT-SPECIFIC RULES. THE UT AND CR VARIANT LOGIC
!  EMPLOY WIND SPEED; THE SN/CS VARIANT LOGIC EMPLOYS MOISTURE LEVELS.
!  THE LS VARIANT LOGIC USES MOISTURE AND WIND SPEED.  AS A RESULT, IF
!  YOU ARE USING THESE VARIANTS, YOU NEED TO DETERMINE THE FUEL MODEL
!  MULTIPLE TIMES DURING A YEAR BECAUSE THE WEATHER CONDITIONS (AND
!  ASSOCIATED FUEL MODEL) MAY CHANGE.  IF YOU ARE NOT USING THESE VARIANTS,
!  YOU CAN CALL FMCMFD ONCE HERE AND BE DONE WITH IT.
!
!  IF YOU ARE USING THE NEW FUEL MODEL LOGIC (IFLOGIC = 1), CALL
!  FMCFMD2 INSTEAD, REGARDLESS OF THE VARIANT.
!
!  ALSO, IF YOU ARE USING MODELLED LOADS TO PREDICT FIRE BEHAVIOR
!  (IFLOGIC = 2), YOU CAN BYPASS THIS.
!
!  TT VARIANT ADDED TO THIS CR/UT LOGIC SINCE THE EXPANDED VARIANT MODELS
!  TWO SPECIES OF JUNIPER. 06/03/10.
!----------
IF (IFLOGIC .EQ. 0) THEN
  IF (.NOT.( &
       ...
       VARACD .EQ. 'CS' .OR. &
       VARACD .EQ. 'SN' .OR. &
       VARACD .EQ. 'ON')) CALL FMCFMD (IYR, FMD)
