...
! YW added to also check httot > 20 for prod 08 and changed subroutine PROD8 to return zero volume for height < 17.3
! 01/18/2013 Added stump vol calculation (VOL(14))
! YW 02/07/2014 Modified TOTHT and HT479 subroutine to make it calculate pulpwood broken height volume. 
!               Broken height is entered to the field HT1PRD
! YW 02/26/2014 Modified TOTHT subrouine to have broken height less then HTTWO and also set HTTOT variable when 
!               total height is passed in with UPSHT1(HTTWO) variable befor cal TOTHT subroutine.
! YW 03/12/2014 Fixed HT479 for smal tree problem (HT2<17.3)
! YW 04/13/2017 Modified R8CLARK for the log volume calculation to also include logs for topwood
! YW 07/13/2018 Modified R8CLARK to check number of logs over 20 and return errflag 12
      SUBROUTINE R8VOL2(VOLEQ,VOL,DBHOB,HTONE,HTTWO,MTOPP,HTTOT,CTYPE, &
                        ERRFLAG)
!      *** SUBROUTINE TO CALCULATE NEW VOLUMES ***
!      ***          FOR REGION 8               ***
!      This routine is used for calculating volumes using
!      Clarks profile models.

!      FVS is allowed to call the 4,7,9 models with total height
!      only.  The model calculates merch height and returns
!      appropriate volumes.  FVS calls the pulpwood model to get
!      ht to the 4" and merch cubic vol.  If it is a saw tree
!      it calls the 7/9" model and reports total merch as saw
!      volume plus topwood.

!      *** WRITTEN BY KEN CORMIER AND BRAD JONES ***

!      *** CREATED:  06-18-91 ***
!REV   Revised TDH 06/01/11 Changed error handling/goto 998.
...
!       COMMON /AREA3A/ IPOINT,LSTREC,SRTIN,SRTOUT,ERRFLAG,DAX,TMP1,TMP2,
!     >                 UNTF,LOG,RAW,VAR,PUVOL
!       INTEGER IPOINT,LSTREC
!       INTEGER SRTIN,SRTOUT,ERRFLAG,DAX,TMP1,TMP2,UNTF,LOG,RAW,VAR,PUVOL
!       INCLUDE 'COMM2'
!       INCLUDE 'COMM3'
!       INCLUDE 'COMM8'
...
!      *** SET ALL VARIABLES USED TO SKIP COEF. LOOKUPS TO ZERO ***
...
!      *** ARRAYS FOR HOLDING COEFFICIENTS ***
...
!       EQN  = VOLEQ(2) - (800 + GEOA * 10)
      ...
      ENDIF
!     *********************************************
!        check for httwo equal zero and totht > 0
      ...
      IF (SPEC.EQ.SPECPR .AND. EQN.EQ.EQNPR .AND. GEOA.EQ.GEOAPR) THEN
         ...
         IF(EQN.EQ.0) THEN
!  Modified the total height equation work for broken top
!  The broken height is entered to the field HT1PRD (YW 02/07/2014)
...
!        IF FVS, USE FCLASS TO FIND VOLUME TO 4 INCH TOP
         ENDIF
...
!     BINARY SEARCH FOR CORRECT COEFFICIENTS
      DONEFLAG = 0
      LASTFLAG = 0
      FIRST = 1
      LAST = 182
...
!        IF FVS, USE FCLASS TO FIND VOLUME TO 4 INCH TOP
 ...
 60    IF (INT(SEVEN(I,1)+.5).LT.SPEC .AND. I.LE.15) THEN
         ...
         CALL HT479(EQN,VOL,DBHOB,HTONE,HTTWO,FIXDI,FCLSS,SPEC,SPGRP, &
                      R,C,E,P,Q,AD,BD,AF,BF,TOPRAT)
!        IF FVS, USE FCLSS TO FIND VOLUME TO 4 INCH TOP
...
!      PURPOSE: CALCULATES PULPWOOD VOLUME TO A 4" TOP USING
!               FORMULAS BASED ON TOTAL HEIGHT
...
!      CALCULATE DBH INSIDE BARK

      DIB =  AD + BD*DBH
      IF (DIB.LT.UPPERD) DIB = UPPERD

!      CALCULATE DIAMETER AT 17.3 FEET (FORMCLASS)
...
!      CALCULATE VARIABLES FOR HEIGHT
      IF (UPPERD .LE. 4) THEN
            UPPERD2 = 16
...
!  *** CALCULATE HEIGHT TO 4, 7, OR 9" TOP ***
...
!     Save HTTWO to a tmp variable
      HT2TMP = HTTWO
! -- INITIALIZE SEG* VARIABLES
       IF(IS.EQ.1) THEN
       ...
       ELSE
          ...
          HTTWO = (17.3 + V1*V2)
       ENDIF
! -- When broken height is entered, use the broken height to calc volume
      IF(HT2TMP.GT.0.1.AND.HT2TMP.LT.HTTWO)THEN
        HTTWO = HT2TMP
      ENDIF
!     height to the upper top
...
!      FIND UPPER AND LOWER HEIGHTS OF EACH SEGMENT
...
!      PURPOSE:  CALCULATES CUBIC FOOT VOLUMES FOR PULPWOOD,
!                SAWTIMBER, AND TOPWOOD USING EQUATIONS BASED ON
!                HEIGHT TO A 4 INCH, 7 INCH, OR 9 INCH TOP
...
!     ***Testing variables
      ...
      ENDIF
       ...
       IF (EQN.EQ.4) THEN
         IF (HT2.LE.17.3) THEN
! This fix small tree problem for the 20140227 fix         
           IF (HT1.GT.0.0.AND.THT1.LT.THT2) THEN
!           Broken height recorded
            ...
            ENDIF
         ENDIF
! Modified the 4" height equation to work for broken height
! The broken height is entered to the field HT1PRD (YW 02/07/2014)
...
!         VOL(5) = VOL(4) - (VOL(4)*DEF1)/100
       ...
       ELSE
         VOL(5) = VOL(4)
!--  USE VOL(4) INSTEAD OF VOLM BECAUSE IT HAS BEEN ADJUSTED
!--  FOR SHORT PULPWOOD TREES
!--      VOL(5) = VOLM
...
!      PURPOSE:  CALCULATE THE CUBIC FOOT VOLUME FOR A GIVEN
!                HEIGHT (HT), DIB, AND FORM CLASS USING EQUATIONS
!                FOR A 4, 7, OR 9 INCH DIAMETER TOP
...
!      INITIALIZE VARIABLES FOR THE VOLUME EQUATION
...
!      SET UPPER AND LOWER HEIGHTS FOR EACH SEGMENT
...
!      VOLU = 0.005454 * (I1*DIB2*((1.-V*W)*(U1-L1)+W*((1.-
!     >    L1/HT2)**R*(HT2-L1)-(1.-U1/HT2)**R*(HT2-U1))
!     >    /(R+1))+I2*I3*(T*(U2-L2)+Z*((1-L2/HT2)**P
!     >    *(HT2-L2)-(1-U2/HT2)**P*(HT2-U2))/(P+1.))+
!     >    I4*(N*(U3-L3)+ CAPR*((1-L3/HT2)**Q *(HT2-L3)-(1.-
!     >    U3/HT2)**Q *(HT2-U3))/(Q+1.)))
...
!      PURPOSE: CALCULATES PULPWOOD VOLUME TO A 4" TOP USING
!               FORMULAS BASED ON TOTAL HEIGHT
!               CALCULATES HEIGHT TO MTOPP
!               RETURNS VOLUME TO MTOPP AND TOPWOOD TO 4 INCH TOP
...
!     return Zero volume for tree height less than 17.3 (YW 10/11/2012)
...
!     FIND HEIGHT TO MTOPP USING ITERITIVE APPROACH
...
!     IF HEIGHT MINUS STUMP IS GE 12.0, FIND VOLUME
...
!***********************************************************************
!  Calculates cubic foot volume (volume) from the stump height (lowht) 
!  to the specified height (highht), if 
!  that is lower), given inside-bark dbh (dbhib), inside-bark diameter 
!  at 17.3' (dib17) and height (topht) to the top diameter (topdob), 
!  for the specified species (spp) and  
!  r, c, e, p, b, a and q are the coefficients for inside-bark 
!  volume calculation.
...
!-----Calculate cubic volume stump to specified height------------------
      if(topdob.eq.'0') then
!--     Total height provided - volume is to 4" DOB.
...
!--     Height to a 4", 7", or 9" DOB provided
        volume=0.005454154*(i1*dbhib**2*((1-g*w)*(u1-l1) &
                +w*((1-l1/topht)**r*(topht-l1)-(1-u1/topht)**r &
                *(topht-u1))/(r+1)) &
              +i2*i3*(t*(u2-l2)+z*((1-l2/topht)**p*(topht-l2) &
...
!***********************************************************************
      subroutine R8DIB(dib,dbhib,dib17,topht,highht, &
                       r,c,e,p,b,a,q,errflg)
!***********************************************************************
!  Calculates inside-bark diameter (dib) at any specified height (highht)
!  given inside-bark dbh (dbhib), inside-bark diameter at 17.3' (dib17) 
!  and total height (topht) for the specified 
!  species (spp).   r, c, e, p, b, a, and q 
!  are the coefficients for inside-bark diameter calculation.
...
! Calculate log volume (03/28/2017)
!     MRULES IS EXTERNAL AND CONTAINS THE MERCHANDIZING RULES SETTINGS
        CALL MRULES(REGN,FORST,VOLEQ,DBHOB,COR,EVOD,OPT,MAXLEN,MINLEN, &
                 ...
                 PROD)
! IF USER MODIFIED MRULE, THEN NO NEED TO CALL MRULES
! The following has been added to MRULES, so I comment out (03/22/2017)
...
! CALCULATE LOG BOARDFOOT USING R9 ROUTINE
         IF(BFPFLG.EQ.1)THEN
           CALL R9BDFT(VOL,LOGLEN,NUMSEG,LOGDIA,ERRFLG,LOGVOL)
         ENDIF
! CALCULATE LOG CUBIC VOLUME AND ADJUST LOG VOLUME WITH MERCH CUBIC VOL
! USING R9 ROUTINE
         CALL R9LGCFT(TLOGS, LOGLEN, LOGDIA, LOGVOL,TLOGVOL,CFVOL)
       ELSE  ! For non-sawtimber log volume calc (03/28/2017)
! If pult height is not provided, calculate it       
           IF(HT2PRD.LE.0.0)THEN
              CALL R8PREPCOEF(VOLEQ, COEFFS, ERRFLG)
              DBHIB = COEFFS%A4+COEFFS%B4*DBHOB
...
!Added max number of logs (20) check (20250626)
 ...
 133          continue
             ...
             CALL R8LOGDIB(VOLEQ, FORST, DBHOB, HTTOT, UPSHT1, &
              TLOGS, TRIM, STUMP, BOLHT,LOGLEN, LOGDIA,ERRFLG)
! For non-saw prod, it should not calculate boardfoot volume. This is in R9clark (07/12/2017)
! So I comment out the following lines (07/12/2017)
! CALCULATE LOG BOARDFOOT USING R9 ROUTINE
!            IF(BFPFLG.EQ.1)THEN
!              CALL R9BDFT(VOL,LOGLEN,NUMSEG,LOGDIA,ERRFLG,LOGVOL)
!            ENDIF
...
! USING R9 ROUTINE
           ...
           ENDIF
       ENDIF
! The following is copied fron R8vol (YW 2016/03/10)
!     ROUND THE VOLUMES 10TH CUBIC FOOT, NEAREST BDFT
      VOL(2) = ANINT(VOL(2))
      VOL(4) = ANINT(VOL(4)*10.0)/10.0
      VOL(7) = ANINT(VOL(7)*10.0)/10.0
      VOL(10) = ANINT(VOL(10))
      VOL(12) = ANINT(VOL(12))
! Changed to NOT reset VOL(4) to 0.5 for PROD 08 (09/06/2012)
      IF(PROD.EQ.'08') THEN
...
!        LOGDIA(1,3)= 0.0
        BOLHT(1) = HT
!--     Get DIB at all log ends
        HT=STUMP
        DO 850 I=1,NUMSEG
          HT=HT+TRIM+LOGLEN(I)
! CHECK THE HEIGHT FOR THE LAST LOG. THE HEIGHT MAY BE SLIGHTLY HIGHER THAN
! THE UPSHT1 BECAUSE OF ROUNDING FOR SEGMENT. IN THIS CASE, THE LAST LOGDIB
! WIL USE VOLEQ DEFAULT TOP DIB
! Now R8CLKDIB can calculate DIB above UPSHT1, so I comment out the following lines (03/24/2017)
...
!...          LOGDIA(I+1,3)= 0.0
 ...
 850   CONTINUE
      ...
      END
!-------------------------------------------------------------------------
! This subroutine calculate the height to a given DIB using Clark 4, 7, 9 equation
...
! calculate DBHIB and F17IB      
      ...
      HT = SEG1+SEG2+SEG3
      RETURN
!      HT=IS*Hx*(1-((DIB2/D2-1)/WW+GG)**(1/r))
!     +  +IB*Hx*(1-(XX-(D2-DIB2)/ZZ)**(1/p))
!     +  +IT*Hx*(1-(JJ-(F2-DIB2)/RR)**(1/q))
      END
