...
!***********************************************************************
!     Calculates double bark thickness for PP on the BHNF, PP on the SJNF, ES on the DNF
!     Variables passed:
!         DBHOB     real    in  diameter breash HTTOT
!         THT     real    in  height of measurement point
...
!         Use Coeffs developed using stump and > BH models;
!*********************************************************************
...
!**********************************************************************
!      IF(HT2.LT.(THT-2.0)) THEN
!       HTTOT = THT
!   use wensels bark model
  IF (DOB.GT.0.0) THEN
       DR = DOB/DBHOB
  ELSE
     DOB = 0.0
     DR = 0.0
  ENDIF
!         if DBTBH is missing, then calculate
  IF (DBTBH.LE.0) THEN
    IF(JSPR.LE.1) THEN
!                      Black Hills
...
!         Use Wenzel modified hyperbolic model for greater than BH
!     IF(JSPR.EQ.5)THEN

!ELSE
!  DW 08/22 Propose changing DR to filter values less than 0.01 to prevent underflow
  IF (HT2.GT.4.5) THEN
     IF(DR.GT.0.01) THEN
     ...
     ELSE
        PY = 0.0
     ENDIF
!         at BH, use what was given or calculated
  ELSE IF (HT2 .EQ. 4.5) THEN
     PY = 1
!         Use Wenzel non-lin stump model
  ELSE
     CLX = C1*(DR-1)
     IF (CLX.GE.0) THEN
...
!                         added by K.Cormier March 1997
!                               original code by
!                         J. W. Flewelling, July 1996
!                                   NOTE:  F(49) reference  REMOVED
...
!                                            Set counter for regional adjs
!                               Check for regional modifications requested
  JRSP=JSP-22
...
!                                        (These outcomes are SINGLE precision)
...
!      RHC = RHI2 + (1.0D0 - RHI2) * dexp(U8)/( 1.0d0 + dexp(U8))
!      If (RHC .lt. RHI2 +0.01) then
!          RHC = MIN( RHI2+0.01, (RHI2+1.0)/2.0 )
!        endif
  RHC=U8
...
!100    IF (  abs(CORR) .gt. 1.0  .and.  ier_UNIT.ne.0)
!     1     write(IER_UNIT,102) JSP, H1,H2, CORR
!102        FORMAT(' CORRC2 ERROR on JSP=',i3,'  H1,H2, corr =',3f8.2)
...
!                                   Note: the following assignments are
!                                   for clarity. Faster execution would
!                                   be obtained by using the F() coefients
!                                   directly, and omitting the V%%% variables.
!     COEF: 61-76
...
!                                This is above total HTTOT - meaningless.
         VARHAT = 1.0
      endif
...
!***********************************************************************
!       SF  SHAPE SUBROUTINE   BHNF Ponderosa Pine, English Units
!              coefficients from BHNF model effort JWF & RLE

!                                  This is steps 2 and 3 of application
...
!  Code from COEMOD4, with coefficients from PP14
...
!     This code taken from RF_SHP2 from the REG1 program
  R1 =  dexp(U1) / ( 1.0d0 + dexp(U1) )
  R2 =  dexp(U2) / ( 1.0d0 + dexp(U2) )
  R3 =  dexp(U3) / ( 1.0d0 + dexp(U3) )
  R4 =  dexp(U4) / ( 1.0d0 + dexp(U4) )
...
!        Correlations from prelim model by JWF & RLE
...
!                          any error is completely correlated with itself
  IF(H30.EQ.HT2) THEN
       ...
       RETURN
  ENDIF
!                               there are no errors at breast height
!                               correlations are irrelevant
