...
model {
  ...
  // (This is the PAI approximation; full Cao/Weiskittel iteration is in R)
  {
    ...
    // Log-normal likelihood: log(dg_obs) ~ normal(log(dg_pred), sigma)
    // Mirrors the HG model's likelihood and avoids the scale collapse
    // that the variance-proportional normal had near small dg_pred.
    // dg_obs > 0 is guaranteed by the upstream plausibility filter
    // (DBH2 - DBH1) / YEARS > 0.001.
    for (n in 1:N) {
