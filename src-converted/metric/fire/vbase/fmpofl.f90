...
!               FMCFMD
!
!  PURPOSE:  CALCULATES THE POTENTIAL FLAME LENGTH AND FIRE EFFECTS
!            FOR TWO SETS OF MOISTURE AND WIND CONDITIONS, AND PRINTS
!            AN OUTPUT FILE WITH THE RESULTS.
!---------
!  CALL LIST DEFINITIONS:
!     IYR:  CURRENT YEAR OF SIMULATION
!     FMD:  FUEL MODEL THAT IS USED IN THE STATIC CASE
!  LNMOUT:  FLAG TO SUPPRESS ALL OUTPUT REPORTING.
!
!  LOCAL VARIABLE DEFINITIONS:
!     SFMOD:  SEVERE FUEL MODELS USED.  TRACKED BECAUSE IN SN, DIFFERENT
!             FUEL MODELS MAY BE SELECTED FOR SEVERE AND MODERATE CASES.
!     SFWT:   SEVERE FUEL MODEL WEIGHT.  USED WITH SFMOD.
!     SNFMODS:NUMBER OF FUEL MODELS USED IN THE SEVERE CASE.
!
!OMMONS
!
!
...
!
!     MODIFY WIND SPEED BASED ON CROWN CANOPY CLOSURE (IF CHANGE THIS, ALSO
!     CHANGE THE CALCULATIONS IN FMBURN):
...
!     INITIALIZE SOME VARIABLES USED IN SN-FFE TO ZERO
...
!     START LOOP OVER FIRE TYPES TO GET THE POTENTIAL FLAME LENGTHS
!     AND OTHER INFO.
!     NOTE THAT WE ARE ONLY LOOKING AT TWO TYPES:
!     1=(WILDFIRE/SEVERE), 3=(PRESCRIBED/MODERATE/2)

DO 100 FMOIS = 1,3,2
!
  CALL FMMOIS(FMOIS, MOIS)

!       CHANGE THE DEFAULT WIND, MOISTURE, TEMPERATURE AND SEASON CONDITIONS,
!       IF NECESSARY. NOTE THAT FMOIS=3 IS THE SAME AS ARRAY ELEMENT 2
!       FOR THE WIND AND TEMPERATURE VALUES
  ...
  BURNSEAS = POTSEAS(IDPL)
!----------
!  CALL FIRE MODEL CALCULATIONS AGAIN FOR UT, CR, LS, CS AND SN VARIANTS
!  TT ADDED 06/03/10 SINCE THE EXPANDED VERSION NOW MODELS JUNIPER
!----------
...
!       SAVE THE FLAME LENGTH...IT IMAY BE MODIFIED BELOW TO ACCOUNT
!       FOR CROWN FIRE BEHAVIOR.
...
!       CALL ROUTINE FOR CALCULATING CROWN FIRE INFORMATION IN WESTERN VARIANTS
...
!       CALCULATE THE POTENTIAL MORTALITY
...
!
  ...
  POKILL(FMOIS) = POMORT
!
!       SET SOME COMMON VARIABLES THAT ARE NEEDED FOR THE POTFMORT
!       POTFTYPE, POTSRATE, AND POTREINT EVENT MONITOR FUNCTIONS
...
!       STORE THE POTENTIAL SMOKE PRODUCTION (PM 2.5)

  PSMOKE(IK)=0.
  CALL FMCONS(FMOIS,0,0.,IYR,1,PSMOKE(IK),POTPAB(IK))

!       SAVE THE FUEL MODELS USED IN THE SEVERE CASE, SINCE DIFFERENT ONES
!       MAY BE USED FOR THE MODERATE CASE.  (USED IN SN-FFE ONLY)
...
!     RETURN IF REPORT IS NOT REQUESTED BY KEYWORD
IF (IPFLMB .GT. 0) RETURN

!     COMPUTE CRTFLM (A CRITICAL FLAME LENGTH) AND CRTCBH (A
!     CRITICAL CROWN BASE HEIGHT). THESE VARIABLE ARE NOT CURRENTLY
!     BEING USED OR OUTPUT.
...
!     RETURN IF ALL OUTPUT IS BEING SUPPRESSED

IF (.NOT.LNMOUT) RETURN

!     DBS CALL FOR DUMPING DATA TO THE DATABASE
...
IF(ICYC.EQ.1)THEN
  DO IDPM = 1,2
    ...
    ENDIF
  CALL DBSFMPFC(NPLT,PREWND(IDPM)*MItoKM, &
             INT(POTEMP(IDPM)*FtoC1+FtoC2), &
...
!     PRINT THE OUTPUT FILE.  NOTE THAT PFLAM IS IN FEET and CBD IS IN KG/M3
...
!       WRITE NEW HEADER INFORMATION GIVING THE SEVERE AND MODERATE CONDITIONS
...
!         THESE ARE THE CFIM CALCULATIONS FROM FMCONS.
!         THEY ARE REPRODUCED HERE FOR PRINTING PURPOSES
...
!       FORMAT THE HEADERS SLIGHTLY DIFFERENTLY FOR SN-FFE
!       AND PRINT SLIGHT DIFFERENT INFORMATION IF USING THE CFIM FIRE MODEL
...
!       FORMAT STATEMENTS FOR CFIM FIRE MODEL
!       TEMPORARY DEBUGGING FORMAT STATEMENTS
...
!: ACTUAL FORMAT STATEMENTS FOR CFIM MODEL
!  333   FORMAT (1X,I5,7X,' FLAME LENGTH (M)',2X,
!     &         'FIRE   ROS    FIRE    INTSY ',
!     &         'CNPY CNPY  POTENTIAL MORTALITY POTEN. SMOKE')
!  335   FORMAT (1X,I5,7X,' SURFACE    TOTAL  TYPE M/MIN  ',
!     &         'INTENSTY  CLASS BASE BULK  ',19('-'),1X,12('-'),
!     &         11X,'FUEL MODELS')
!  337   FORMAT (1X,I5,7X,10('-'),1X,7('-'),1X,4('-'),1X,6('-'),
!     &         1X,8('-'),2X,5('-'),'  HT DNSTY ',
!     &         ' SEV  MOD   SEV  MOD   SEV  MOD   ',31('-'))
!  340   FORMAT (1X,I5,1X,'YEAR   SEV   MOD SEV MOD S  M  S  M',
!     &         4X,'S   M   S  M   M  ',
!     &         'KG/M3  %BA  %BA  TOT CU VL  (T/H <2.5) ',
!     &         4(' MOD %WT'))

!       FORMAT STATEMENTS FOR THE NEW HEADERS:

144   FORMAT (1X,I5,1X,'FIRE        WIND  TEMP ----- FUEL MOISTURE', &
            ' CONDITIONS (PERCENT) ------')
145   FORMAT (1X,I5,1X,'CONDITION (KM/H)   (C)   <0.6    .6-2.5 ', &
...
!     WRITE OUT DATA TO FILE.  THE SN VARIANT IS PRINTED DIFFERENTLY,
!     BECAUSE CROWN FIRE IS NOT MODELLED AND DIFFERENT FUEL MODELS CAN BE
!     CHOSEN FOR THE SEVERE AND MODERATE SCENARIOS
...
!     CFIM FIRE MODEL ALSO WRITTEN DIFFERENTLY
ELSE IF (CFIM_ON) THEN

  WRITE (JPOTFL,69) IDPFLM, IYR, PFLAM(2)*FTtoM, PFLAM(4)*FTtoM, &
         INT((PFLAM(1)*FTtoM)+.5), INT((PFLAM(3)*FTtoM)+.5), &
         CFTYPE(1)(1:1),CFTYPE(3)(1:1), &
         PROS(1), PROS(3), INT(PBYRAM(1)), INT(PBYRAM(3)), &
         PFCLS(1), PFCLS(3), &
         ...
         PSMOKE(1)*P2T*CONV,PSMOKE(2)*P2T*CONV, &
         (FMOD(I),INT((FWT(I)*100.)+0.5),I=1,NFMODS)
69   FORMAT (1X,I5,1X,I4,2(1X,F5.1),2(1X,I3), &
          1X,A1,2X,A1,2(1X,F3.1),2(1X,I4),2(1X,I1),1X,I3,2X,F5.3, &
          2(2X,I3),2X,I4,1X,I4,2X,F4.2,1X,F4.2,2X,8(1X,I3))

   TEMPORARY CFIM WRITE STATEMENTS
...
&      INT(CRB(3)*100.),POTCONS(1,1), POTCONS(1,2), POTCONS(1,3)
 &      POTCONS(3,1), POTCONS(3,2), POTCONS(3,3) &
      FORMAT (1X,I5,1X,I4,2(1X,I2),1X,A1,2X,A1,2(1X,F3.1),
...
ELSE ! write like usual
  WRITE (JPOTFL,49) IDPFLM, IYR, PFLAM(2)*FTtoM, PFLAM(4)*FTtoM, &
         INT((PFLAM(1)*FTtoM)+.5), INT((PFLAM(3)*FTtoM)+.5), &
         CFTYPE(1)(1:1), &
         CFTYPE(3)(1:1),PTORCH(1), PTORCH(2), &
         OINIT1(1)*MItoKM, OACT1(1)*MItoKM, &
         INT(ACTCBH*FTtoM), CBD, &
         INT(POKILL(1)*100.),INT(POKILL(3)*100.), &
         INT(POVOLK(1)*FT3pACRtoM3pHA),INT(POVOLK(2)*FT3pACRtoM3pHA), &
...
IMPLICIT NONE
!----------
!  **FMPTRH FIRE-BASE-DATE OF LAST REVISION:  05/18/2004
!----------
!     SERVICE ROUTINE TO FMPOFL TO COMPUTE PTORCH FOR EACH OF TWO
!     FLAME LENGTHS.  NOT DESIGNED TO BE CALLED FROM OTHER PARTS
!     OF THE PROGRAM.
!
!     NL CROOKSTON -- RMRS MOSCOW -- MAY 2004
!
!OMMONS
!
!
...
!OMMONS
!
!     SET THE MAXIMUM NUMBER OF REPLICATIONS.

INTEGER MXREPS
PARAMETER (MXREPS=30)

!     SET THE VERTIAL PLOT SIZE
...
!     SAVE THE RANDOM NUMBER STATUS

CALL RANNGET(SAVES0)

!     DROP SOME RANDOM NUMBERS SO THAT CALLS FROM DIFFERENT YEARS HAVE
!     DIFFERENT SEQUENCES.
...
!     COMPUTE THE TOP HEIGHT OF THE STAND...AND SET THE CRITICAL HEIGHT
!     THAT THE FLM1S MUST REACH.

AVHT=0.
SSUM=0.
DO I=1,MXI
   P=PRB(I)
   IF(SSUM+P.GT.40.47) P=40.57-SSUM
   SSUM=SSUM+P
   AVHT=AVHT+HT(I)*P
   IF(SSUM.GE.40.47) EXIT
ENDDO
IF (SSUM.GT.0.) AVHT=AVHT/SSUM
CRIT=MAX(5.,MIN(.5*AVHT,50.))

!     COMPUTE THE PROBABILITY OF TORCHING FOR EACH REP.

DO IREP=1,MXREPS

!        FIRST, DECIDE WHICH TREES ARE ON THE VIRTUAL PLOT...
...
!              SET ITOP TO 1 IF THERE IS LEAST 1 TREE THAT IS
!              OVER THE CRITICAL HEIGHT.

         IF (HT(I).GE.CRIT) ITOP=1
      ENDIF
   ENDDO

!        FIND THE LOWEST CROWN THAT MUST BE IGNITED FOR THE PLOT
!        TO TORCH.
...
!     FIND THE AVERAGE LOWEST POINT...NOT THAT CURRENTLY WE DON'T
!     USE IF FOR ANYTHING.
...
ELSE IF (II.GT.1) THEN &
   t or not, as you wish)         CALL RDPSRT(II,MINCB,INDX,.FALSE.)
   AVCBHT=0.
   DO I=1,II
...
!     FIND THE PROBABILITY OF TORCHING FOR BOTH IGNITION HEIGHTS.
...
!      PRINT *,'IYR=',IYR,' MXREPS=',MXREPS,' II=',II,' FLM=',FLM1,FLM2,
!     >        ' PTR=',PTR1,PTR2,' AVCBHT=',AVCBHT

!     RESTORE THE RANDOM NUMBER STATUS
