...
! AK $Id$
!----------
!     PREDICTS THE PROBABILITIES FOR THE NUMBER OF SPECIES
!     ON STOCKED PLOTS
!----------
...
!  ITPP     -- NUMBER OF TREES PER STOCKED PLOT (INTEGER)
!  RITPP    -- NUMBER OF TREES PER STOCKED PLOT (REAL)
!  NSSPB1 AND B1 -- B1 COEFFICENT BY FOREST TYPE AND NUMBER OF SPECIES (6)
!  NSSPB2 AND B2 -- B2 COEFFICENT BY NUMBER OF SPECIES (6)
!  PSPA     -- X VARIABLE IN EQN FOR PSPE
!  PSPE     -- PROBABILITY OF 1-6 SPECIES ON A PLOT
!----------
...
!----------
!  PREDICT PROBABILITIES FOR NUMBER OF SPECIES ON PLOT
!
!  PSPA = B1 + B2 * RITPP
!
!  VARIABLES DEFINED IN SECTION ABOVE
!  PSPA IS CONVERTED TO PROBABILITY OF NUMBER OF SPECIES ON PLOT BY
!  EXP(PN)/(1 + EXP(PN))
!----------

!     P(1 SPECIES ON STOCKED PLOTS)
B1 = NSSPB1(1,IFT)
B2 = NSSPB2(1)
PSPA = B1 + B2 * RITPP
PSPE(1) = EXP(PSPA)/(1+EXP(PSPA))
!     P(2 SPECIES ON STOCKED PLOTS)
IF(ITPP.LT.2) GO TO 78
B1 = NSSPB1(2,IFT)
B2 = NSSPB2(2)
PSPA = B1 + B2 * RITPP
PSPE(2) = EXP(PSPA)/(1+EXP(PSPA))
!     P(3 SPECIES ON STOCKED PLOTS)
IF(ITPP.LT.3) GO TO 78
B1 = NSSPB1(3,IFT)
B2 = NSSPB2(3)
PSPA = B1 + B2 * RITPP
PSPE(3) = EXP(PSPA)/(1+EXP(PSPA))
!     P(4 SPECIES ON STOCKED PLOTS)
IF(ITPP.LT.4) GO TO 78
B1 = NSSPB1(4,IFT)
B2 = NSSPB2(4)
PSPA = B1 + B2 * RITPP
PSPE(4) = EXP(PSPA)/(1+EXP(PSPA))
!     P(5 SPECIES ON STOCKED PLOTS)
IF(ITPP.LT.5) GO TO 78
B1 = NSSPB1(5,IFT)
B2 = NSSPB2(5)
PSPA = B1 + B2 * RITPP
PSPE(5) = EXP(PSPA)/(1+EXP(PSPA))
!     P(6 SPECIES ON STOCKED PLOTS)
